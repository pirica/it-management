...
- **Migration runner:** `schema_migrations` (`filename`, `checksum`, `applied_at`) tracks files applied via `scripts/migrate.php`. Fresh `db/` imports already satisfy many migration files — runner shows **Applied** (not false PENDING) and `--apply` can record them without re-running destructive SQL. Canonical DDL in `01_schema.sql`; live bootstrap: `db/migrations/schema_migrations.sql`. Helpers: `includes/itm_database_migrations.php`.
...
- **Hotel booking:** tables `hotel_booking_*`, `booking_rooms_types`, lifecycle status lookups (`hotel_bookings_future|present|history`), photo child tables (`hotel_booking_hotel_photos` + `booking_rooms_type_photos` seeds — disk under `booking/images/1/hotel_photos/` and `booking/images/1/room_types_photos/`), `hotel_booking_portal_users`, `hotel_booking_settings`, **room-type calendar** (`hotel_booking_room_type_rate_overrides`, `hotel_booking_room_type_blocks`). `hotel_booking_special_rates` has `trg_hotel_booking_special_rates_audit_*` in `03_triggers.sql`. Tenant uniques: `bank_accounts` (`company_id`, `institution_name`, `account_name`); `appointment_visit_reasons` (`company_id`, `name`). Fresh install: DDL/DML/triggers in `01_schema.sql` / `02_data.sql` / `03_triggers.sql`. Live DB backfill: `php scripts/seed_hotel_booking_sample_photos.php --apply`. Live DB: `db/migrations/hotel_booking.sql`, `db/migrations/hotel_booking_room_type_calendar.sql`. Module doc: `modules/hotel_bookings/AGENT_NOTES.md`, public `booking/AGENT_NOTES.md`.
