...
- **Command palette (phase 2 index):** `search_index` denormalized rows (`company_id`, `module_slug`, `record_id`, `title`, `subtitle`, `keywords`, FULLTEXT on text columns). Populated on CRUD via `includes/itm_search_index.php` and bulk backfill `scripts/apply_search_index_backfill.php`. Palette queries index first when rows exist, else phase 1 SQL `LIKE` via `includes/itm_command_palette_search.php`. Live migration: `db/migrations/search_index.sql`.
...
- **Hotel booking:** tables `hotel_booking_*`, `booking_rooms_types`, lifecycle status lookups (`hotel_bookings_future|present|history`), photo child tables (`hotel_booking_hotel_photos` + `booking_rooms_type_photos` seeds — disk under `booking/images/1/hotel_photos/` and `booking/images/1/room_types_photos/`), `hotel_booking_portal_users`, `hotel_booking_settings`, **`hotel_bookings.auth2`** (guest manage PIN; migration `hotel_bookings_auth2.sql`), **room-type calendar** (`hotel_booking_room_type_rate_overrides`, `hotel_booking_room_type_blocks`). `hotel_booking_special_rates` has `trg_hotel_booking_special_rates_audit_*` in `03_triggers.sql`. **Distribution tenant uniques** (`01_schema.sql`): `hotel_booking_distribution_mappings`, `hotel_booking_distribution_reservations`, `hotel_booking_distribution_rate_plan_mappings`, and `hotel_booking_distribution_ari_restrictions` lead composite UNIQUE keys with `company_id` + `channel_id` (audit skips `hotel_booking_distribution_ari_events` and `hotel_booking_distribution_webhook_queue` — many rows per channel). **Seeds:** `02_data.sql` includes one `appointments` row, `hotel_bookings` (`HBSEED001`), and `hotel_booking_distribution_reservations` (booking.com channel id `2` on fresh import) before SELECT-based distribution mapping blocks so `itm_parse_database_sql_inserts()` can extract sample templates. Tenant uniques: `bank_accounts` (`company_id`, `institution_name`, `account_name`); `appointment_visit_reasons` (`company_id`, `name`). Fresh install: DDL/DML/triggers in `01_schema.sql` / `02_data.sql` / `03_triggers.sql`. Live DB backfill: `php scripts/seed_hotel_booking_sample_photos.php --apply`. Live DB: `db/migrations/hotel_booking.sql`, `db/migrations/hotel_booking_room_type_calendar.sql`, `db/migrations/hotel_bookings_auth2.sql`. Module doc: `modules/hotel_bookings/AGENT_NOTES.md`, public `booking/AGENT_NOTES.md`.
