...
- **Command palette (phase 2 index):** `search_index` denormalized rows (`company_id`, `module_slug`, `record_id`, `title`, `subtitle`, `keywords`, FULLTEXT on text columns). Phase 1 palette uses live SQL `LIKE` via `includes/itm_command_palette_search.php` — no rows required in `search_index` yet. Live migration: `db/migrations/search_index.sql`.
