...
- **Migration runner:** `schema_migrations` (`filename`, `checksum`, `applied_at`) tracks files applied via `scripts/migrate.php`. Canonical DDL in `01_schema.sql`; live bootstrap: `db/migrations/schema_migrations.sql`. Helpers: `includes/itm_database_migrations.php`.
