...
- **Hotel booking:** tables `hotel_booking_*`, `booking_rooms_types`, lifecycle status lookups (`hotel_bookings_future|present|history`), photo child tables (`hotel_booking_hotel_photos` + `booking_rooms_type_photos` seeds — disk under `booking/images/1/hotel_photos/` and `booking/images/1/room_types_photos/`), `hotel_booking_portal_users`, `hotel_booking_settings`. Fresh install: DDL/DML/triggers in `01_schema.sql` / `02_data.sql` / `03_triggers.sql`. Live DB backfill: `php scripts/seed_hotel_booking_sample_photos.php --apply`. Live DB: `db/migrations/hotel_booking.sql`. Module doc: `modules/hotel_bookings/AGENT_NOTES.md`, public `booking/AGENT_NOTES.md`.
