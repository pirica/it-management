...
- **Hotel booking:** tables `hotel_booking_*`, `booking_rooms_types`, lifecycle status lookups (`hotel_bookings_future|present|history`), photo child tables (`hotel_booking_hotel_photos` seeds four `hb_seed_*.jpg` files for company 1 / hotel 1 — disk copies under `images/hotel_booking/1/hotel/1/` and portal demos under `booking/images/hotel-sample-*.jpg`), `hotel_booking_portal_users`, `hotel_booking_settings`. Fresh install: DDL/DML/triggers in `01_schema.sql` / `02_data.sql` / `03_triggers.sql`. Live DB backfill: `php scripts/seed_hotel_booking_sample_photos.php --apply`. Live DB: `db/migrations/hotel_booking.sql`. Module doc: `modules/hotel_bookings/AGENT_NOTES.md`, public `booking/AGENT_NOTES.md`.
