...
- **`ticket_inbound_email_messages`:** dedupe/thread table (`company_id`, `message_id` UNIQUE per tenant, optional `ticket_id`, `email_log_id` → `emails.id`). Canonical DDL in `db/01_schema.sql`.
- **`ticket_inbound_email_routing_rules`:** keyword → `priority_id` / `category_id` / `assigned_to_employee_id` for inbound new tickets (`urgent`, `critical`, `billing`, `support` seeds on company 1). Live migration: `db/migrations/ticket_inbound_email_routing_rules.sql`.
