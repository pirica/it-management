...
- **QR share (private-data exempt):** unified `share_sessions` (`module_slug`, `record_id`, optional `scope_path` / `scope_path_hash` for Explorer). Company enable/disable matrix: `company_module_share` + `modules/share_modules/`. Capable slug list: `itm_qr_share_capable_module_slugs()` in `includes/itm_qr_share.php` (32 slugs — 9 vault/explorer + 23 CRUD record modules). `company_module_share` seeded in `db/02_data.sql` for those slugs only; live DB backfill: `db/migrations/company_module_share_capable_seed.sql`. No rows in `02_data.sql` for `share_sessions`; no audit triggers on `share_sessions` in `03_triggers.sql`. Canonical agent doc: `docs/CRUD_RECORD_SHARE.md`. Live migration from per-module `*_share_sessions` tables: `db/migrations/share_sessions_unified.sql`.
