...
- **`audit_logs` BIGINT keys:** `id` and `record_id` are `BIGINT` in `01_schema.sql` (append-only log + generic audited-row pointer). Live migration with row preserve: `db/migrations/audit_logs_bigint.sql`. Review probe: `php scripts/verify_bigint_table_review.php`.
