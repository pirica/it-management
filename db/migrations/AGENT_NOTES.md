...
**Pruned repo state (September 2026):** all runner-scoped `*.sql` files were removed after a fresh `db/` import showed **0 Pending** on `php scripts/migrate.php --status` and `php scripts/verify_db_migrations.php` passed for every file. Applied history remains in the `schema_migrations` audit table and in git history. **On disk:** `schema_migrations.sql` (bootstrap DDL only) plus `index.html`.
...
- **Stale live DBs:** when incremental files are pruned from the repo, operators on old databases must re-import `db/` after backup (`bash scripts/import_database_split.sh`) or restore from a backup taken after the last successful `migrate.php --apply` — there is no on-disk migration path for drifted installs until a new `{module}_{subject}.sql` is added again.
...
| File | Role |
|------|------|
| `schema_migrations.sql` | Bootstrap `CREATE TABLE` for migration audit history (`filename`, `checksum`, `applied_at`) — excluded from the runner apply loop |
| `index.html` | Directory listing prevention |

When shipping schema for **existing** databases again, add a new `{module}_{subject}.sql` here and mirror the final shape in `db/01_schema.sql` in the same PR.
