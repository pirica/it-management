...
$stmt = mysqli_prepare($conn, "
    SELECT id, company_id, workstation_mode_id
    FROM employees
    WHERE id = ?
    AND company_id = ?
");
...
$stmt = mysqli_prepare($conn, "
    SELECT workstation_mode_id, assignment_type_id
    FROM employees
    WHERE id = ?
    AND company_id = ?
");
...
$workstation_mode_id = (int)($row['workstation_mode_id'] ?? 0);
$assignment_type_id = (int)($row['assignment_type_id'] ?? 0);
...
$stat_definitions = [
    ...
    ['table' => 'events', 'field' => 'assigned_to_employee_id', 'label' => 'Events', 'slug' => 'events'],
...
foreach ($all_stats as $index => $s) {



    if ($s['table'] === 'equipment' && $s['field'] === 'assigned_to_employee_id') {
        
        $assigned_assets_count = $s['count'];
    } 
...
/* 7) BIND RESULT DINÂMICO */
$meta = mysqli_stmt_result_metadata($stmt);
if (!$meta) die("RESULT METADATA FAILED: " . mysqli_stmt_error($stmt));
...
/* 8) FETCH - copiando valores */
$system_access_overview = [];
while (mysqli_stmt_fetch($stmt)) {
    $temp = [];
    foreach($row as $key => $val) {
        $temp[$key] = $val; // copia valor, não referência
    }
    $system_access_overview[] = $temp;
	//print_r($temp);
}
...
//    echo "Com acesso: $count_1 | Sem acesso: $count_0";
}
