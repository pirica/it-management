...
* **Testing/reporting guardrail (mandatory):**
  ...
  * **PHPUnit suite / HTML coverage:** `php scripts/run_tests.php` or browser `scripts/run_tests.php?run=1&mode=coverage` — report at `phpunit/coverage/html/coverage.html` (Xdebug or PCOV). On **Dunebox** (`D:\dunebox-v1.0.6`), run once: `powershell -ExecutionPolicy Bypass -File scripts/setup_dunebox_php_from_laragon.ps1` (copies **Xdebug** from Laragon portable into Dunebox `php/ext` and writes `php.ini` for **mbstring** + coverage). Authoring rules: **`scripts/SCRIPTS.md` → PHPUnit test runner**.
