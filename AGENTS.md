...
**Tier A step exceptions:** edit `mbqa_runner_module_step_exceptions()` in `scripts/module_browser_qa_runner.php` (module slug → step → N/A note). The runner **still runs** the normal Tier A pipeline (clear, HTTP, bulk, etc.); the map only overrides **recorded** results for listed steps via `mbqa_step_result_for_module()`. Example: `user_companies` → `create`, `add`, `import_db` (not `sample_data` — that module has `add_sample_data` and must seed/restore normally). The map is also written to the JSON report as `module_step_exceptions`.
