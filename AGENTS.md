...
7a. **Module file paths → local browser URL (mandatory in agent replies):** when you cite a defect, regression line, or manual check for a path under `modules/` (for example `modules/news/index.php — browser title not canonical`), always add the **Laragon URL** on the next line and tell the user to **open it in a new browser tab** (Admin session unless the page is public). Base: `http://localhost/it-management/` (see **Setup & Debugging**). Pattern: `http://localhost/it-management/modules/<slug>/<file>.php`. Use a markdown link, for example `[Open modules/news/index.php](http://localhost/it-management/modules/news/index.php)`. Browser script failures should use `itm_script_format_modules_file_local_dev_link()` (`scripts/lib/script_browser_nav.php`; `verify_module_page_chrome.php` prints **Open in new tab →** under each `[FAIL]`).
