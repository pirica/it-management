...
* **Import bundle (`db/`):** Import in **one MySQL session** in order **01_schema → 02_data → 03_triggers** (`bash scripts/import_database_split.sh` or `bash scripts/verify_database_sql_import.sh`). CLI scripts honour **`MYSQL_PORT`** (default **3307** on Dunebox; **3306** on GitHub Actions **database-import** — set in `.github/workflows/smoke.yml`). Match application **`.env` `DB_PORT`** to your MySQL listener. See `db/AGENT_NOTES.md`.
...
* **Incremental migrations (`db/migrations/`):** schema changes for **existing** databases belong in `db/migrations/{module}_{subject}.sql` (for example `todo_vault.sql`). Copy the table’s `CREATE TABLE` block from `db/01_schema.sql`, paste it into the migration with your changes, and apply with **`DROP TABLE IF EXISTS` + `CREATE TABLE`** (copy/paste replacement) — **no `ALTER TABLE`**, **no `{table}_new` staging tables**. In the **same PR**, mirror the final shape in `db/01_schema.sql` (and `db/02_data.sql` when seeds change). `DROP TABLE` removes existing rows; back up before applying on production. **Runner:** `php scripts/migrate.php --status` probes the **live database** for every file (Applied vs Pending); `schema_migrations` is audit/history only. `php scripts/migrate.php --apply` runs SQL only when the live probe fails and records satisfied migrations without re-executing destructive files (`schema_migrations`: `filename`, `checksum`, `applied_at`). See `db/migrations/AGENT_NOTES.md`.
* **Company Scoping:**
...
* **Audit columns + soft-delete (mandatory for scaffold CRUD):** see inventory lists `docs/list_soft-delete.txt` (in scope) and `docs/list_bespoke_UI.txt` (deferred). Helpers: `includes/itm_crud_audit_fields.php`, `itm_format_audit_timestamp_display()` in `includes/itm_date_format.php`.
    ...
    * **View:** show `active` badges plus all six meta fields. `*_by` → employee full name (`first_name` + `last_name`, username fallback). `*_at` → `d/M/Y - H:i:s` (dd/mmm/yyyy audit stamp, blank when NULL). Soft-deleted rows remain reachable by `view.php?id=` so delete audit can display.
...
* **Multi-company seed admins (mandatory):** Fresh import seeds five companies and one Admin employee per company. Role / access / status FKs and related grants must be **tenant-correct** (subqueries or `INSERT … SELECT` by `company_id` + name — never hardcode `role_id = 1` / `access_level_id = 1` for every company, and never hardcode `employee_roles.id` values that fight auto-increment).
    * Usernames: company 1 → `Admin`; companies 2–5 → `Admin2` … `Admin5` (login is global by username/`LIMIT 1`, so duplicate `admin` usernames are unsafe). Password for all seed admins: `Admin` (bcrypt in `db/02_data.sql`). **`must_change_password = 1`** on seed admins and `demo1`–`demo5` forces rotation on first password login (`force-password-change.php`, `includes/itm_force_password_change.php`). SSO logins skip the gate (`$_SESSION['itm_auth_via_sso']`). Optional `.env` `ITM_SKIP_FORCE_PASSWORD_CHANGE=1` for local dev only.
...
| `share_sessions` | Unified temporary QR/code share snapshots (`module_slug`, `record_id` or `scope_path`, `payload_json` plaintext until expiry); no audit triggers. |
...
5. **Regression scripts:** `php scripts/test_explorer_paths.php` (path ACL logic); `php scripts/verify_explorer_zip_leak.php` (Step 1: blocked roots; Step 2: exact `Private/{username}_{employee_id}` only; Step 3: all other paths blocked); `php scripts/verify_explorer_api_rate_limit.php` (per-employee hourly cap on `api.php`). PoC for malicious `.htaccess` upload: `verify_explorer_rce_htaccess.php`, `verify_explorer_rce_marker.php` (catalog in `scripts/scripts.php`). PHPUnit: `ExplorerTest::testTrashListFiltersAncestorFolders`.
...
3. **Modules:** `modules/ticket_questionnaires/` (CRUD templates), `modules/ticket_surveys/` (read-only issued list), `modules/ticket_survey_dashboard/` (KPI filters). Issue UI on `modules/tickets/view.php`; auto-issue on closed status via `itm_ticket_survey_maybe_issue_on_close()` only when enabled on [modules/tickets/index.php?tab=configuration](http://localhost/it-management/modules/tickets/index.php?tab=configuration) (`ticket_settings`, default off).
...
2. **Dispatcher:** `itm_notify_employee($conn, $employeeId, $payload)` in `includes/itm_employee_notifications.php` (wraps `itm_employee_notification_create()` with dedupe + default `action_url`). Specialized helpers: ticket assign, onboarding approval, warranty expiry, todo/event/alert/appointment assign, note share, email log recipients, ticket comment `@mention`, live chat conversation assign.
3. **Header UI:** `includes/header.php` renders 🔔 + unread badge + dropdown; `js/notifications.js` loads the badge via deferred **`GET ?count_only=1`** (lightweight count only) and polls every **120s** when the panel is closed (**60s** when open). **Do not auto-start SSE** on every page — long-lived `api.php?stream=1` connections tie up Apache/PHP workers and slow saves. Full list loads only when the bell opens (`GET ?unread=0&limit=20`). Click marks read (POST `action=mark_read`) then navigates to `action_url`. **`modules/notifications/api.php` must call `itm_release_session_lock()`** after auth (required if SSE is used manually). **Do not** call `itm_api_enforce_rate_limit_or_exit()` on this internal session UI API — it wrote `ui_configuration` on every poll. **`notifications`** and **`employee_notifications`** slugs are **always allowed** in `itm_module_access_always_allowed_slugs()` (header API is not company-gated).
...
5. **Emitters (minimum):** ticket assign (`modules/tickets/create.php`); onboarding approval email (`modules/employee_onboarding_requests/`); warranty expiry (`scripts/run_email_alert_rules.php` → equipment assignee); todo `assigned_to_employee_id`; events `assigned_to_employee_id`; **alerts `assigned_to_employee_id`** (`modules/alerts/index.php` create/edit); **appointments `assigned_to_employee_id`** (`modules/appointments/index.php` `list_all` inline assign); emails `to_email` / `cc_email` on successful send log (`itm_email_log_send()`); notes `shared_with_json` (new share targets on edit); ticket comment `@username` mentions (`modules/ticket_comments/` — **F2** user picker in create/edit + server notify on save, new mentions only on edit); **`live_chat_conversations.assigned_to_employee_id`** (`modules/live_chat_conversations/index.php` create/edit). Live Chat message/waiting flows continue using `itm_employee_notification_create()` in `modules/live_chat/api.php`.
...
5. **Live Agent launch:** `includes/itm_live_chat_launch_options.php` includes an **Appointment** card (`open_mode: browser_tab`) linking to `modules/appointments/` for self-service IT visit scheduling.
...
The Appointment module (`modules/appointments/`) provides employee self-service IT visit booking (visit reason, weekly slot modal, in-person or remote when allowed).
...
2. **Fresh install:** canonical DDL/DML/triggers in `db/01_schema.sql`, `db/02_data.sql`, `db/03_triggers.sql` (import via `bash scripts/import_database_split.sh`). **`db/migrations/appointments.sql`** remains for existing databases that predate the module; **`db/migrations/appointments_booking_lock.sql`** adds `booking_lock` on live DBs that still use the enum column; **`db/migrations/appointments_type.sql`** replaces the enum with **`appointments_type`** + **`appointments_type_id`** (destructive to `appointments` rows).
...
4. **API:** `modules/appointments/api.php` — `week_slots` (GET), `schedule` (POST + CSRF); rate limit enforced.
5. **Live Chat:** Live Agent launch menu includes **Appointments** (see **Chatbot & Knowledge Base** above).
6. **Sidebar:** Planning → Appointments (`includes/ui_config.php`); tenant configuration → **`modules/appointment_settings/`** (slug `appointment_settings`, Planning → Appointment Settings). Bespoke booking UI (not flattened scaffold CRUD); `list_all.php` / `view.php` for booking review.
...
2. **Settings UI:** `modules/settings/index.php` — **UI Configuration** card (money symbol select `EUR`/`GBP`/`USD` with suffix/prefix checkboxes; portal date format; 12h/24h time; combined datetime style toggles + default). POST normalized by `itm_ui_locale_normalize_post_values()`; persisted by `itm_save_ui_configuration()` (`mysqli_stmt_bind_param` type string `iissssiiiiisssssiissiiiis` — 25 INSERT placeholders).
...
8. **Equipment tab:** [index.php?tab=equipment](http://localhost/it-management/modules/license_management/index.php?tab=equipment) lists tenant equipment that has catalog software (`equipment_software`). **Software** select (`software_id`) filters the list; empty = all installed catalog software. Columns show equipment/software/license **labels** (not raw IDs) with links to view pages. Row actions: 🔎 [equipment view](http://localhost/it-management/modules/equipment/view.php) and ✏️ [equipment edit](http://localhost/it-management/modules/equipment/edit.php). Helper: `itm_software_license_list_equipment()` in `includes/itm_software_license_link.php`.
9. **Audit logging:** `db/` defines `trg_license_management_audit_insert|update|delete` and `trg_license_types_audit_*` (when Type rows are quick-added).
10. **Regression scripts** (`scripts/SCRIPTS.md`, catalog `scripts/scripts.php`): `php scripts/module_browser_qa_runner.php --module=license_management --company=1`; `php scripts/verify_software_license_links.php`.
...
* **Testing/reporting guardrail (mandatory):**
  ...
  * **PHPUnit suite / HTML coverage:** `php scripts/run_tests.php` or browser `scripts/run_tests.php?run=1&mode=coverage` — report at `phpunit/coverage/html/coverage.html`. **CLI PHP must load:** `dom`, `json`, `libxml`, `mbstring`, `tokenizer`, `xml`, `xmlwriter` (PHPUnit hard requirement). **HTML coverage** additionally needs **Xdebug** or **PCOV** with coverage mode. On **Dunebox** (`D:\dunebox-v1.0.6`), run once: `powershell -ExecutionPolicy Bypass -File scripts/setup_dunebox_php_from_laragon.ps1` (copies **Xdebug** from Laragon portable, writes `php.ini` for **mbstring** + coverage). Full list and verify commands: **`scripts/SCRIPTS.md` → PHPUnit test runner**.
...
* **LDAP SSO login:** [sso-ldap.php](http://localhost/it-management/sso-ldap.php) requires the PHP **`ldap`** extension on the **Apache** SAPI (`ldap_connect()` in `includes/itm_ldap_auth.php`). **Dunebox / Laragon:** enable `extension=ldap` in the same PHP 7.4.33 `php.ini` as above (template includes `ldap` after `setup_dunebox_php_from_laragon.ps1`); copy `php_ldap.dll` into `ext\` if missing; **restart Apache**. Verify: `php -r "echo function_exists('ldap_connect') ? 'ok' : 'missing';"` or [verify_sso_ldap.php?run=1](http://localhost/it-management/scripts/verify_sso_ldap.php?run=1). Canonical doc: `docs/SSO_LDAP.md` → PHP `ldap` extension.
