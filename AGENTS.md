...
3. **Header UI:** `includes/header.php` renders 🔔 + unread badge + dropdown; `js/notifications.js` uses **SSE** (`modules/notifications/api.php?stream=1`) for unread-count updates with **60s polling fallback**; click marks read (POST `action=mark_read`) then navigates to `action_url`. **`modules/notifications/api.php` must call `itm_release_session_lock()`** after auth so SSE does not hold the PHP session lock and block other pages. **`notifications`** and **`employee_notifications`** slugs are **always allowed** in `itm_module_access_always_allowed_slugs()` (header API is not company-gated).
