...
**Checklist per standard module (Tier A, including Protection Zone folders):** **`error_log`** (delete `error_log.txt` when writable; else record byte offset and only fail on *new* lines this module) → **`list`** (index HTTP 200, no fatal) → FK-aware **`clear`** → **`sample_data`** (HTTP; FK parents seeded first — e.g. `expenses` → `departments`, `budget_categories`, `cost_centers`, `gl_accounts`; DB fallback via `itm_seed_table_from_database_sql()` / `itm_seed_resolve_fk_from_database_sql()` when anchor ids differ) → **`add`** (insert ~30 random tenant rows when count &lt; `records_per_page` + 1; grow unique-scope parents first) → **`bulk_cancel`** (runner HTTP contract: `js/bulk-delete-selection.js` + index bulk form, no inline `selectionMode`; browser still clicks **Cancel**) → **`bulk_delete`** (when row count after `add` ≥ `records_per_page` and bulk UI + `delete.php` + CSRF: POST `delete.php` with up to 3 `ids[]`; explicit N/A note when skipped) → search → sort → create → view → edit → list_all → **export_pdf** → **export_xls** (parse list table; row count is not import count) → **import_db** (one insertable row smoke test from export headers + `database.sql` FK values when needed; **`inserted=1` is pass**, not full export row count) → **single_delete** (FK retry) → **clear_table** (same row gate as `bulk_delete`) → **`sample_data`** (end restore on empty table) → **`error_log`** (0 new errors since module scope).

**Tier A bulk steps (runner vs browser):**

| Step | What it does |
|------|----------------|
| **`add`** | Insert ~30 random tenant rows if count &lt; `records_per_page` + 1 (`mbqa_ensure_bulk_sample_rows`; grow unique-scope parents first). |
| **`bulk_cancel`** | **Browser:** after **Select to Delete**, `cancelButton.textContent = 'Cancel'` (`js/bulk-delete-selection.js`) hides checkboxes and clears selection without POST. **Runner:** HTTP contract on index after `add` — shared `js/bulk-delete-selection.js` (Cancel + `exitSelectionMode`), `bulk-delete-form` + `bulk_delete` toggle, no inline `selectionMode`; static `data-itm-bulk-cancel` must be `type="button"`. |
| **`bulk_delete`** | If rows ≥ perPage and bulk UI + `delete.php` + CSRF: POST `delete.php` with up to 3 `ids[]`. **Browser:** **Delete Selected** after checkboxes. **Runner:** direct POST (skips Select to Delete / Cancel). |
