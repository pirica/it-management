...
**Tier A step exceptions:** edit the per-module map in `mbqa_runner_module_step_exceptions()` inside `scripts/module_browser_qa_runner.php` (module slug → step → N/A note). Example: `user_companies` skips `create`, `sample_data`, `add`, `import_db` with module-specific notes in that map only.
