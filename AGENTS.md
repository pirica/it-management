...
3. **Header UI:** `includes/header.php` renders 🔔 + unread badge + dropdown; `js/notifications.js` uses **SSE** (`modules/notifications/api.php?stream=1`) for unread-count updates with **60s polling fallback**; click marks read (POST `action=mark_read`) then navigates to `action_url`.
