...
- **FieldsMissingBespokeGateTest.php** — bespoke gate contract checks; reviewed registry (`scripts/data/fields_missing_reviewed.json`, `[SKIP][fail][reviewed]` labels); strict gate exit (`--strict-gate` / unreviewed `[SKIP][fail]`); bookmarks in-memory Search/Sort via `bkm_query_bookmarks_for_list()` (`testBookmarksBespokeGatePassesSearchAndSort`); index list audit meta hidden via `itm_crud_is_list_hidden_audit_field()` (`testCableColorsBespokeGateHidesAuditMetaOnIndexList`); view audit meta required on readable view paths (`testCompaniesBespokeGateFlagsMissingViewAuditMeta`, `testBespokeViewAuditMetaFailsWhenFieldAbsent`, `testBespokeViewAuditMetaPassesWithViewColumnsLoop`).
