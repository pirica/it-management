...
- **FieldsMissingBespokeGateTest.php** — `itm_fields_missing_report.php` dynamic form heuristics (`id` / `company_id` false-positive guards; audit meta hidden on bespoke module forms including `vlans`); reviewed bespoke gate registry (`scripts/data/fields_missing_reviewed.json`, `[SKIP][fail][reviewed]` labels).
