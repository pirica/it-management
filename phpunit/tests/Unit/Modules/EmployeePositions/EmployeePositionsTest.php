...
namespace Tests\Unit\Modules\EmployeePositions;

use PHPUnit\Framework\TestCase;

class EmployeePositionsTest extends TestCase
{
    private $conn;
    private $companyId = 1;
    ...
    public function testCRUD()
    {
        ...
        $sql = "INSERT INTO `employee_positions` (company_id, `name`, `active`) VALUES (?, ?, ?)";
        ...
        // 2. Read
        $res = mysqli_query($this->conn, "SELECT * FROM `employee_positions` WHERE id = $id");
        ...
        $updateSql = "UPDATE `employee_positions` SET `name` = ? WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $updateSql);
        mysqli_stmt_bind_param($stmt, 'si', $updatedValue, $id);
        $this->assertTrue(mysqli_stmt_execute($stmt));
        mysqli_stmt_close($stmt);

        $res = mysqli_query($this->conn, "SELECT `name` FROM `employee_positions` WHERE id = $id");
        $row = mysqli_fetch_assoc($res);
        $this->assertEquals($updatedValue, $row['name']);

        // 4. Delete
        $deleteSql = "DELETE FROM `employee_positions` WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $deleteSql);
        mysqli_stmt_bind_param($stmt, 'i', $id);
        $this->assertTrue(mysqli_stmt_execute($stmt));
        mysqli_stmt_close($stmt);

        $res = mysqli_query($this->conn, "SELECT COUNT(*) as count FROM `employee_positions` WHERE id = $id");
        $row = mysqli_fetch_assoc($res);
