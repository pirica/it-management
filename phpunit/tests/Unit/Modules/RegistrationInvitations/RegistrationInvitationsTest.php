...
namespace Tests\Unit\Modules\RegistrationInvitations;

use PHPUnit\Framework\TestCase;

class RegistrationInvitationsTest extends TestCase
{
    private $conn;
    private $companyId = 1;
    ...
    public function testCRUD()
    {
        ...
        $sql = "INSERT INTO `registration_invitations` (company_id, `email`, `invitation_code`, `active`) VALUES (?, ?, ?, ?)";
        ...
        $bindValues[] = $data['invitation_code'];
        ...
        // 2. Read
        $res = mysqli_query($this->conn, "SELECT * FROM `registration_invitations` WHERE id = $id");
        ...
        $updateSql = "UPDATE `registration_invitations` SET `email` = ? WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $updateSql);
        mysqli_stmt_bind_param($stmt, 'si', $updatedValue, $id);
        $this->assertTrue(mysqli_stmt_execute($stmt));
        mysqli_stmt_close($stmt);

        $res = mysqli_query($this->conn, "SELECT `email` FROM `registration_invitations` WHERE id = $id");
        $row = mysqli_fetch_assoc($res);
        $this->assertEquals($updatedValue, $row['email']);

        // 4. Delete
        $deleteSql = "DELETE FROM `registration_invitations` WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $deleteSql);
        mysqli_stmt_bind_param($stmt, 'i', $id);
        $this->assertTrue(mysqli_stmt_execute($stmt));
        mysqli_stmt_close($stmt);

        $res = mysqli_query($this->conn, "SELECT COUNT(*) as count FROM `registration_invitations` WHERE id = $id");
        $row = mysqli_fetch_assoc($res);
