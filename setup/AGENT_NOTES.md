...
- Completion writes `setup/.installed` under **`itm_setup_wizard_project_root()`** and deletes `setup/index.php` + `setup/includes/itm_setup_wizard.php` there (step 8). Finish redirects to **`ITM_APP_URL` login** from wizard session when set (cross-folder installs).
