...
Step 1 exposes an editable **project root** field. **Auto-detect** row updates live (vanilla JS) to show the repaired/normalized path that will be saved — including Windows paste repair when backslashes are stripped after `C:`. **Public application URL (ITM_APP_URL)** and the **Detected BASE_URL** preview row update live when the project root folder name changes (for example `it-management` → `it-management3` rewrites `http://host/it-management/` → `http://host/it-management3/`). **New path:** when the folder does not exist, the wizard creates it (`0755`), downloads `pirica/it-management` from GitHub (`git clone` with ZIP fallback), and verifies `db/01_schema.sql`. **Existing path:** fails unless it matches the current runtime install folder (in-place setup). Optional `.env`: `ITM_SETUP_GITHUB_REPO`, `ITM_SETUP_GITHUB_BRANCH`. Session path drives step 2 checks (`db/`, upload trees, `.env`); paths display with Windows backslashes when applicable. **Run verification** creates upload hardening under the step 1 project root (warning when it differs from the runtime PHP request path).
