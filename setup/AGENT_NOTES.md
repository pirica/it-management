...
Step 7 lists active seed companies from `companies` with per-company checkboxes, **Select All** / **Unselect All**, and POST `sample_company_ids[]` to `itm_setup_wizard_install_sample_data_for_companies()` (wraps `itm_seed_all_tables_from_database_sql()` per tenant).
