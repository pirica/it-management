...
- Depends on the entire database schema as defined in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
...
- **verify_database_sql_import.sh** — full `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` import + table-count assertion (derived from `CREATE TABLE` lines, currently 130); CI job **database-import** in `.github/workflows/smoke.yml` (also runs `verify_crud_fk_label_search.php`).
...
- **All `scripts/apply*.php` maintenance tools** — shared contract via **`lib/itm_apply_script_bootstrap.php`**: Browser + CLI, dry-run default, `--apply` / `?apply=1` (Admin) to write, named target lists after the count summary. Catalog: `scripts/scripts.php`. **`apply_module_sample_data_seed.php`** — propagates sample `INSERT`s into `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` for every seeded `company_id` (single-row and multi-row `VALUES` blocks); **mirror mode** detects `INSERT … SELECT N, … FROM table WHERE company_id = 1` and appends only to the source tenant VALUES (e.g. `knowledge_base`); browser errors via `itm_seed_fwrite_stderr()`; built-in help lists dry-run/apply URLs.
- **verify_audit_columns.php** — schema gate for mandatory audit columns including soft-delete fields.
- **verify_audit_logs_disclosure.php** — three-step employees audit disclosure regression (static `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` trigger scan, live disposable UPDATE probe, retro scan); prints each step with `[PASS]`/`[FAIL]`.
...
- **check_database_sql_company_name_uniques.php** — tenant UNIQUE audit over `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (via `includes/database_sql_unique_audit.php`). Skips intentional duplicate-name / junction tables including `bookmark_folders` and `floor_plan_item_tags`, private `events`, and ephemeral `*_share_sessions` tables (see `includes/itm_qr_share.php`, `modules/events/AGENT_NOTES.md`).
...
- **list_boolean_integer_fields.php** — parses split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql) and live database to list Boolean/tinyint/int/integer columns, matched to modules by name.
- **list_enum_fields.php** — parses split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql) and live database to list ENUM columns, matched to modules by name.
- **extract_by_fields.php** — parses split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql) to extract column definitions matching keywords (by, to, employee_id, employee), matching tables to modules by name.
...
- **check_phones.php** / **list_phone_columns.php** — PII auditing for phone-related columns in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`. Browser or CLI via `lib/script_cli_output.php`.
...
- **check_delimiters.php** / **check_duplicates.php** / **verify_sql.php** / **check_sql_errors.php** — SQL audit tools for `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`. Browser or CLI via `lib/script_cli_output.php`.
- **fix_sql.php** / **fix_sql_broad.php** / **fix_sql_departments.php** — Maintenance for `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (active columns, triggers, column counts). CLI only.
