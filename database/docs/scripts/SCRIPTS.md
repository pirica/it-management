...
| Script | Purpose |
|--------|---------|
| `php scripts/list_boolean_integer_fields.php` | Parses both `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` and the live database to list fields of Boolean, int, tinyint, and other numeric types, matching tables to modules by name. |
| `php scripts/list_enum_fields.php` | Parses both `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` and the live database to list ENUM fields, matching tables to modules by name. |
| `php scripts/extract_by_fields.php` | Parses `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` to extract column definitions containing keywords like `by`, `to`, `employee_id`, `employee`. Output is formatted and saved to `scripts/fields_by.txt`. |
...
| `php scripts/verify_audit_logs_disclosure.php` | Three-step employees audit disclosure regression: static `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` trigger scan, live disposable employee UPDATE probe, retro scan of recent `employees` audit rows. Prints each step; optional `ITM_TEST_COMPANY_ID`. |
...
| **database-import** | `bash scripts/verify_database_sql_import.sh` then `php scripts/verify_crud_fk_label_search.php` | Full `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` import on MySQL 8.0 service; asserts table count matches **130** `CREATE TABLE` entries; runtime FK label search regression |
...
* `scripts/tickets_related_equipment_delete_test.php` (seeds sample ticket rows from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`).

**Script that dumps seed SQL (no DB writes):**

* `scripts/export_floor_plan_folders_seed.php` prints `INSERT` statements to stdout for pasting into `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
...
| # | Step | What it checks |
|---|------|----------------|
| 1 | **`mysql`** | Whether `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` defines sample `INSERT` rows for the module table. Parsed from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` via `itm_parse_database_sql_inserts()` (same tuples as UI sample seed). Manual equivalent: `SELECT * FROM \`{table}\`` in phpMyAdmin on a fresh import — **0 row(s) (empty)** e.g. `ip_addresses`, or **N row(s)** e.g. `departments`. Informational **Pass**; note records the count. Fails only if `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` is missing/unreadable. Tier C/D report `N/A`. |
...
**`mysql` verification (file or SQL):** prefer reading `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (runner step 1). To spot-check in MySQL: `SELECT COUNT(*) FROM \`{table}\`` on a database loaded from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` — expect the same N as the runner note (global insert count, not per-tenant). Empty modules (`patches_updates`, `ip_addresses`, …) drive **`sample_data`** N/A notes such as `No sample rows found in split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql) for this module.`
...
Run `verify_roles_permissions.php` when changing `modules/roles_permissions/`, `js/roles-permissions-matrix.js`, `includes/itm_role_module_permissions.php`, or `employee_roles` / `role_module_permissions` / `role_hierarchy` schema in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
...
Run `verify_ops_report.php` when changing `modules/ops_report/` or `ops_report*` tables in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.

### Reports Hub scripts

| Script | Purpose |
|--------|---------|
| `php scripts/verify_reports_hub.php` | Regression for `modules/reports/`: all `api/helpers.php` chart payloads, Hotel Operations MTD metrics (`ops_report` / `ops_report_fb_outlet`), budget vs actual / YoY totals, `modules_registry` slug `reports`, and core Chart.js canvas ids in `index.php`. Optional `ITM_TEST_COMPANY_ID` (default 1). Requires `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` Reports Hub sample seeds (ops_report daily trend, F&B covers, expanded budgets/expenses). Browser + CLI. |

Run `verify_reports_hub.php` when changing `modules/reports/`, `modules/reports/api/helpers.php`, or Reports Hub-related seeds in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.

### Email Management scripts

| Script | Purpose |
|--------|---------|
| `php scripts/verify_emails_module.php` | Regression: `emails`, `email_smtp_configurations`, `email_alert_rules` tables, `modules_registry` row, default SMTP seed, alert rule seeds, `itm_send_email()` helper; company 1 warranty/license **30-day alert window is a hard fail** (script inserts disposable license sample when empty, then deletes it). `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` uses relative `DATE_ADD(CURDATE(), …)` expiry seeds so fresh imports stay in-window |
...
Run `verify_emails_module.php` when changing `modules/emails/`, `includes/itm_email.php`, or `email*` tables in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
...
Run `verify_system_status.php` when changing `modules/system_status/`, `scripts/system_status_api.php`, `includes/itm_system_status_native.php`, `includes/itm_system_status_powershell.php`, `includes/itm_system_status_storage.php`, `includes/itm_system_status_cache.php`, `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` `system_status`, or any `includes/*.ps1` metrics script. On large tenants the storage tree scan and `information_schema` queries can be slow — run Refresh from CLI or raise PHP `max_execution_time` in browser if needed. API dispatcher: `scripts/system_status_api.php?action=…` (Admin only; invalid action → HTTP 400). Module UI: `modules/system_status/index.php` — tabs read cached JSON from `system_status`; **Refresh** POST runs `itm_system_status_refresh_all()`. Sub Storage parent nodes sum child totals plus direct files in each folder. For README screenshots see **Roles & Permissions scripts** above (`take_screenshots_modules.py`).
...
- `employee_fields_missing.php` or `fields_missing.php --module=employees` — after `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` `employees` columns or profile/list UI changes.
...
**Destroy -> document -> fresh clone:** if a script wrecks `itmanagement` or critical trees, record the culprit in `scripts/data/scripts-matrix-destroy-log.md` (status `DESTROYED_ENV`), re-import `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (or `bash scripts/verify_database_sql_import.sh`), sanity-check, then resume. Full protocol lives in `SCRIPTS_TEST_MATRIX.md`.
...
**Codacy Cloud (`.codacy.yml` + `.semgrep.yaml`):** repo root `.codacy.yml` sets global `exclude_paths` for generated/QA trees. Opengrep rule `tainted-sql-string` (message: “User data flows into this manually-constructed SQL string”) is forked in `.semgrep.yaml` at **WARNING** severity with extra taint sanitizers (`rawurlencode`, `htmlspecialchars`, `http_build_query`) for URL href false positives. After merge, enable **Opengrep → Configuration file** on the repository **Code patterns** page so Codacy loads `.semgrep.yaml`. **Note:** configuration-file mode controls which Opengrep patterns run — this file currently ships only the forked `tainted-sql-string` rule; keep other Opengrep patterns enabled in the Codacy UI or extend `.semgrep.yaml` before switching. Prefer repo gates (`check_codacy_xss_echo.php --strict`, `check_sql_injection_coverage.php`) over Codacy for PHP echo/SQL policy. Adjust Codacy **quality gates** (Settings → Gates) so new issues at Warning severity do not fail PRs if the forked rule still reports URL builders.
