...
| `bash scripts/verify_database_sql_import.sh` | Full `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` import + table count; calls schema verify | Module HTTP behaviour |
...
3. **Fresh clone** the database from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (do not continue on partial state).
4. **Sanity-check**, then resume at the **next** script (or re-run only if the failure was a false alarm).

### Detect destruction

- Table count far below expected (see `scripts/number_db_tables.txt` / `CREATE TABLE` count in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`)
...
| 1 | `verify_database_sql_import.sh` | MySQL | destroys-DB | CI database-import | Full split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql) re-import — baseline clone |
...
| 2 | `check_not_operator.php` | PHP | none | static-manual | Warning-only Codacy `!` review (always exit 0; not in smoke) |
...
| 5 | `apply_module_sample_data_seed.php` | isolated-env | writes-repo-or-DB | excluded-blanket | Writes split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql) seed blocks |
