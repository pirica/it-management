...
| --- | --- |
| **Tables** | 130 |
| **Sample rows** | ~3,085 (from <code>split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql)</code>) |
...
<p align="center">UI entry points (<code>sidebar.php</code>, <code>dashboard.php</code>, <code>modules/company_module_access/</code>) call shared helpers in <code>includes/itm_company_module_access.php</code>. Enforcement gates every <code>modules/*</code> request after login. Data lives in <code>modules_registry</code> (global catalog) and <code>company_module_access</code> (per-company <code>enabled</code> flags). Fresh installs seed all company × module rows from <code>split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql)</code>; upgrades can run <code>php scripts/seed_company_module_access.php</code>.</p>
...
2. Import `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` into MySQL.
...
For an existing database, apply the Floor Plans tables from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (`floor_plan_folders`, `floor_plan_tags`, `floor_plans`, `floor_plan_item_tags`) if they are not already present.
...
**Setup:** import the Floor Plans section from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` on existing databases. If tables are missing, the gallery shows an explicit migration message instead of a generic company error.
...
If a table reports `doesn't exist in engine`, rebuild only that table from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`:
