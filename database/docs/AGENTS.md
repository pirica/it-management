...
### Database (`database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`)

* Database and tables use **`utf8mb4`** with **`utf8mb4_unicode_ci`** (see top of `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`: `CREATE DATABASE … utf8mb4`, `SET NAMES utf8mb4`).
...
2. **Create** — when adding a new in-scope folder, or when audit finds a gap, add `AGENT_NOTES.md` in the **same PR** using section headings from `templates/AGENT_NOTES.md` and facts from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`, module PHP, and `scripts/scripts.php`. Do **not** bulk-generate module notes without reading the code — the template forbids blind automation for `modules/`.
...
* **Schema Updates:** If a field/table is deleted or a header renamed, update `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
* **No live `ALTER TABLE` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`:** put keys, indexes, and foreign keys inside each table’s `CREATE TABLE`. Import already runs with `FOREIGN_KEY_CHECKS=0`, so FKs may reference tables created later in the file. Commented `-- ALTER TABLE …` lines are historical notes for existing databases only — do not add new executable `ALTER` statements.
* **Company Scoping:**
...
* **Standard Fields:** Every new table in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` must include:
...
* **Multi-company seed admins (mandatory):** Fresh import seeds five companies and one Admin employee per company. Role / access / status FKs and related grants must be **tenant-correct** (subqueries or `INSERT … SELECT` by `company_id` + name — never hardcode `role_id = 1` / `access_level_id = 1` for every company, and never hardcode `employee_roles.id` values that fight auto-increment).
    * Usernames: company 1 → `Admin`; companies 2–5 → `Admin2` … `Admin5` (login is global by username/`LIMIT 1`, so duplicate `admin` usernames are unsafe). Password for all seed admins: `Admin` (bcrypt in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`).
...
Tables that store **private user content** must **not** be copied into `audit_logs` and must **not** define `trg_{table}_audit_*` triggers in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`. Do not add PHP `itm_log_audit()` / `itm_run_query()` audit hooks for mutations on these tables — keep the data private.
...
8. **Audit logging:** `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` defines `trg_{table}_audit_insert|update|delete` on every `ops_report*` table (parent + all child tables). Child triggers include `ops_report_id` in JSON payloads.
...
8. **Audit logging:** `email_smtp_configurations` and `email_alert_rules` use `trg_*_audit_*` triggers in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`. The **`emails`** send log is **private-data exempt** (no audit triggers — see **Private data — no audit trail**).
...
7. **Standard CRUD:** Flattened departments scaffold — bulk delete, search, pagination, Excel import/export (`import_excel_rows` on `index.php`), empty-state sample data from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
8. **Audit logging:** `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` defines `trg_license_management_audit_insert|update|delete` and `trg_license_types_audit_*` (when Type rows are quick-added).
...
* **Source:** Seed rows must match `INSERT INTO` entries in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` for that module table.
...
* **Mandatory column + SQL relation audit (hard fail):**
  ...
  * Validate visible-column rendering in `index.php`, `view.php`, and `list_all.php` against `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`/`information_schema` relationships.
...
* **Risk of Regression (login.php):** Any changes to the login flow (e.g., joining with `employee_roles`) must be carefully verified against the schema in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` to avoid breaking authentication for all users.
...
* **Import `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (full file — do not strip the first lines):**
    ```cmd
    cd /d C:\Users\NelsonSalvador\Downloads\laragon-portable\www\it-management
    "C:\Users\NelsonSalvador\Downloads\laragon-portable\bin\mysql\mysql-8.4.3-winx64\bin\mysql.exe" -u root -pitmanagement --default-character-set=utf8mb4 < split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql)
    ```
    Verify: `SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='itmanagement';` → **130**, or `php scripts/verify_database_schema.php` (lists any missing tables). A partial import often stops at table **73** (`employee_companies`) — missing block includes `role_hierarchy` … `workstation_ram`, `rack_planner`. Common deploy bugs: stripping the first lines of `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (removes `DROP DATABASE`), wrong MySQL password, or `re-download-replace_DB.ps1` piping without `-pitmanagement`. Use the updated `laragon-portable\www\re-download.ps1` (full file + table count). Capture stderr (`2> mysql-import.err`) — MySQL may exit 0 while statements failed.
  * **PowerShell piping:** `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` in git is **LF**; `-split "`r`n"` can yield a single “line” and skip the strip branch — still import the **complete** file. Prefer `cmd /c "\"C:\Users\NelsonSalvador\Downloads\laragon-portable\bin\mysql\mysql-8.4.3-winx64\bin\mysql.exe\" -u root -pitmanagement --default-character-set=utf8mb4 < split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql)"` from the repo directory over stdin `Process` piping when imports truncate.
...
* **Pre-merge review pass (required before merge):** on every PR, run a targeted review of the changed files (last N files in the diff when large) against this `AGENTS.md`, including at minimum:
  ...
  * `php scripts/check_database_sql_company_name_uniques.php` when `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` or tenant unique keys changed.
...
2. **MySQL** — PHP connects on `127.0.0.1:3306` after the localhost socket attempt in `config/config.php`. If `service mysql start` or `/var/lib/mysql` fails (common on Cloud Agent: InnoDB `undo_*` / OS error 71 on the repo workspace mount), use a **writable datadir outside the workspace**:
    ...
    mysql -u root -pitmanagement --socket=/tmp/itm-mysql.sock --default-character-set=utf8mb4 < split database files (database/01_schema.sql, database/02_triggers.sql, database/03_data.sql)
