...
- Database triggers `trg_employee_sidebar_preferences_audit_insert`, `trg_employee_sidebar_preferences_audit_update`, `trg_employee_sidebar_preferences_audit_delete` on `employee_sidebar_preferences` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
