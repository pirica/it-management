...
- Database triggers `trg_workstation_os_versions_audit_insert`, `trg_workstation_os_versions_audit_update`, `trg_workstation_os_versions_audit_delete` on `workstation_os_versions` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
