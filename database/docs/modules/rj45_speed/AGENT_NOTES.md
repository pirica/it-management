...
- Database triggers `trg_rj45_speed_audit_insert`, `trg_rj45_speed_audit_update`, `trg_rj45_speed_audit_delete` on `rj45_speed` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **equipment**, **switch_ports**, **idf_ports**, **idf_links**. [Cursor-Valid]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM rj45_speed WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO rj45_speed (company_id, name, active) VALUES (?, ?, ?)");
