...
- Database triggers `trg_workstation_modes_audit_insert`, `trg_workstation_modes_audit_update`, `trg_workstation_modes_audit_delete` on `workstation_modes` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **employees** (`workstation_mode_id`). [Cursor-Valid]
