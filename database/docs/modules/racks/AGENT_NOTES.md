...
- Database triggers `trg_racks_audit_insert`, `trg_racks_audit_update`, `trg_racks_audit_delete` on `racks` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
