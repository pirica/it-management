...
- Database triggers `trg_role_assignment_rights_audit_insert`, `trg_role_assignment_rights_audit_update`, `trg_role_assignment_rights_audit_delete` on `role_assignment_rights` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Defines which roles may assign other roles. [Cursor-Valid]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM role_assignment_rights WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO role_assignment_rights (company_id, name) VALUES (?, ?)");
