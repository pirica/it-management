...
- **Type** values come from tenant-scoped `license_types` rows (seeded in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` + cross-company `INSERT IGNORE` replication). Quick-add on the Type select inserts via `select_options_api.php` (`license_types` whitelist); full CRUD is under **`modules/license_types/`** with **company_id** hidden.
...
- **`license_management` seeds in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`** are declared **after** `suppliers` so FK parents exist before sample INSERTs (one sample row per company, companies 1–5). [Cursor-Valid]
- **`license_types` seeds** — five lookup rows per company (companies 1–5) plus cross-company `INSERT IGNORE` replication in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`. [Cursor-Valid]
