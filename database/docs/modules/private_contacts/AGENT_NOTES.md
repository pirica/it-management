...
## 2. Key Tables
- **private_contacts** — contact records scoped by `employee_id` and `company_id`. PII text columns are `TEXT` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` so vault ciphertext fits (re-import or widen legacy `varchar` columns before enabling encryption on existing DBs).
