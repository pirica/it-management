...
- Database triggers `trg_role_hierarchy_audit_insert`, `trg_role_hierarchy_audit_update`, `trg_role_hierarchy_audit_delete` on `role_hierarchy` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Defines role ordering — tied to **employee_roles** (`role_id`). [Cursor-Valid]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM role_hierarchy WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO role_hierarchy (company_id, name) VALUES (?, ?)");
