# AGENT_NOTES.md - Is Server

## 1. Module Purpose
A filtered view of the Equipment module specifically for server devices. Provides a type-scoped sidebar entry for infrastructure and operations teams without the full equipment catalogue.

## 2. Key Tables
- **equipment** — main asset records (read/write via `modules/equipment/`).
- **equipment_types** — joined for the `Server` type filter (`et.name`).

## 3. Required Relationships
- **equipment** → depends on **companies** (`company_id`).
- **equipment** → depends on **equipment_types** (`equipment_type_id`; filter matches `Server`).
...
- **Filter:** List and view enforce `LOWER(TRIM(et.name)) = 'server'` via `$equipmentTypeNameFilter = 'Server'` in `index.php` / `view.php`.
...
- **Standard Equipment list** (search, sort, pagination, export) scoped to servers.
...
## 7. File Structure
- **index.php** — sets `$equipmentTypeNameFilter = 'Server'` and wrapper flags; `require '../equipment/index.php'`.
...
## 10. Common Pitfalls
- **Do not delete canonical wrappers:** Keep `modules/is_server/` and sibling `is_*` folders. [Cursor-Valid]
- **Type filter:** `$equipmentTypeNameFilter` must match `equipment_types.name` (`Server`). [Cursor-Valid]
...
Provides a specialised view for server asset management alongside the general Equipment module.
