...
- Legacy installs: drop `uq_bookmark_folders_company_scope` if still present (`database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` comment under `CREATE TABLE bookmark_folders`). [Cursor-Valid]
