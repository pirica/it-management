...
- Sidebar folder tree rows show **✏️** (rename modal) beside **🗑️** when the user may edit the folder (`bkm_can_edit_folder()` and vault unlocked for private folders). Rename posts `action=rename_folder` on `index.php` (name only; parent/shared/active unchanged).
...
- **QR / code share (`join.php`):** `bkm_can_edit_bookmark()` users may create 30-minute temporary read links; private bookmarks require unlocked vault. `bookmark_share_sessions` stores plaintext `payload_json`. UI: 📱, `images/whatsapp.svg`, and 📨 on dual-pane and `list_all` actions; modal via shared `includes/itm_qr_share_modal.php`. Public page: `join.php` (`ITM_QR_SHARE_PUBLIC`). Regression: `php scripts/verify_qr_share_modules.php`, `php scripts/verify_whatsapp_share.php`, `php scripts/verify_outlook_share.php`.
...
- **rename_folder** (POST on `index.php`) — `folder_id`, `name`, optional `redirect_view`; renames via `bkm_update_folder_row()` without changing parent/shared/active.
...
- Existing DBs that still have `uq_bookmark_folders_company_scope` must `DROP INDEX` that key (see comment under `bookmark_folders` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`). [Cursor-Valid]
