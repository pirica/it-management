...
- Database triggers `trg_rack_statuses_audit_insert`, `trg_rack_statuses_audit_update`, `trg_rack_statuses_audit_delete` on `rack_statuses` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
