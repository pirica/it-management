...
- **Schema column name**: `floor_plan_folders.parent_folder_id` is the parent FK (not `parent_folder_name`). Gallery helpers and `index.php` folder create/move handlers use `parent_folder_id`; `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` triggers and unique key must match. [Cursor-Valid]
