...
- Database triggers `trg_ticket_categories_audit_insert`, `trg_ticket_categories_audit_update`, `trg_ticket_categories_audit_delete` on `ticket_categories` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
