# AGENT_NOTES.md - Is Workstation

## 1. Module Purpose
A filtered view of the Equipment module specifically for workstation devices (desktops, laptops, and related endpoints). Provides a type-scoped sidebar entry for desktop support teams without the full equipment catalogue.

## 2. Key Tables
- **equipment** — main asset records (read/write via `modules/equipment/`).
- **equipment_types** — joined for the `Workstation` type filter (`et.name`).

## 3. Required Relationships
- **equipment** → depends on **companies** (`company_id`).
- **equipment** → depends on **equipment_types** (`equipment_type_id`; filter matches `Workstation`).
- **equipment** → may link to **manufacturers**, **equipment_statuses**, **it_locations**, **racks**, **idfs**, and **employees**.
- Workstation-specific columns on **equipment** (`workstation_os_type_id`, `workstation_ram_id`, `workstation_office_id`, etc.) are edited via equipment forms.

## 4. Business Rules (Critical for Agents)
- **Filter:** List and view enforce `LOWER(TRIM(et.name)) = 'workstation'` via `$equipmentTypeNameFilter = 'Workstation'` in `index.php` / `view.php`.
...
- INSERT, UPDATE, and DELETE on `equipment` are logged by MySQL triggers `trg_equipment_audit_insert`, `trg_equipment_audit_update`, and `trg_equipment_audit_delete` (see `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`).
...
## 10. Common Pitfalls
- **Do not delete canonical wrappers:** Keep `modules/is_workstation/` and sibling `is_*` folders. [Cursor-Valid]
- **Type filter:** `$equipmentTypeNameFilter` must match `equipment_types.name` (`Workstation`). [Cursor-Valid]
...
$stmt = $conn->prepare(
    "SELECT e.id, e.name, e.hostname, e.workstation_processor
     FROM equipment e
...
Provides a specialised view for workstation asset management alongside the general Equipment module.
