...
`database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` defines AFTER INSERT/UPDATE/DELETE triggers that write JSON payloads to `audit_logs`:
...
- **Editing `idf_ports` only:** leaves `switch_ports` and the equipment switch-port manager out of date — always use sync helpers or API endpoints. [Cursor-Valid]
- **IDF sync test:** run `php scripts/idfs_sync_human_test.php` after any port workflow change; treat `[FAIL]` as a hard stop. [Cursor-Valid]
- **Deleting linked ports:** remove `idf_links` referencing the port first; otherwise deletes fail or leave orphan link rows. [Cursor-Valid]
- **Partial fibre/RJ45 sets:** `idf_ports_sync.php` treats `switch_ports` as authoritative for SFP numbering — do not synthesise duplicate fibre rows in `idf_ports`. [Cursor-Valid]
- **List/search bug:** define `$displayFieldColumns = $uiColumns` before search loops that reference `$displayFieldColumns`. [Cursor-Fixed]
