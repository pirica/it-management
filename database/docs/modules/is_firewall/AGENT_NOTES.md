# AGENT_NOTES.md - Is Firewall

## 1. Module Purpose
A filtered view of the Equipment module specifically for firewall devices. Provides a type-scoped sidebar entry for security and network teams to manage firewall assets without the full equipment catalogue.

## 2. Key Tables
- **equipment** — main asset records (read/write via `modules/equipment/`).
- **equipment_types** — joined for the `Firewall` type filter (`et.name`).

## 3. Required Relationships
- **equipment** → depends on **companies** (`company_id`).
- **equipment** → depends on **equipment_types** (`equipment_type_id`; filter matches `Firewall`).
...
- **Filter:** List and view enforce `LOWER(TRIM(et.name)) = 'firewall'` via `$equipmentTypeNameFilter = 'Firewall'` in `index.php` / `view.php`.
...
- **Standard Equipment list** (search, sort, pagination, export) scoped to firewalls.
...
## 7. File Structure
- **index.php** — sets `$equipmentTypeNameFilter = 'Firewall'` and wrapper flags; `require '../equipment/index.php'`.
...
Provides a specialised view for firewall asset management alongside the general Equipment module.
