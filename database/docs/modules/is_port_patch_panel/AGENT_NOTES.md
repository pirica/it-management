# AGENT_NOTES.md - Is Port Patch Panel

## 1. Module Purpose
A filtered view of the Equipment module specifically for port patch panel devices. Provides a type-scoped sidebar entry for cabling and infrastructure teams without the full equipment catalogue.

## 2. Key Tables
- **equipment** — main asset records (read/write via `modules/equipment/`).
- **equipment_types** — joined for the `Port Patch Panel` type filter (`et.name`).

## 3. Required Relationships
- **equipment** → depends on **companies** (`company_id`).
- **equipment** → depends on **equipment_types** (`equipment_type_id`; filter matches `Port Patch Panel`).
...
## 10. Common Pitfalls
- **Do not delete canonical wrappers:** Keep `modules/is_port_patch_panel/` and sibling `is_*` folders. [Cursor-Valid]
- **Type filter:** `$equipmentTypeNameFilter` must match `equipment_types.name` exactly (`Port Patch Panel` — multi-word string). [Cursor-Valid]
...
```php
$typeName = 'Port Patch Panel';
$stmt = $conn->prepare(
    "SELECT e.id, e.name, e.patch_port
     FROM equipment e
...
Provides a specialised view for patch panel asset management alongside the general Equipment module.
