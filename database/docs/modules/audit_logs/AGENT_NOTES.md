...
- **Automatic triggering:** Most **audited** tables use MySQL triggers (`trg_{table}_audit_insert|update|delete`) in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`. PHP modules may also call `itm_log_audit()` when `ui_configuration.enable_audit_logs` is on.
...
- **Employees trigger redaction:** `trg_employees_audit_*` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` must not log `password`, `vault_key_hash`, `reset_token`, `reset_token_hash`, or `reset_token_expires_at`. Regression: `php scripts/verify_audit_logs_disclosure.php` (static trigger scan + live disposable employee probe + retro scan).
...
- **Admin toolbar**: Download ALL Logs, Backup ALL Logs, and Clear ALL Logs buttons (admin role only). List header uses `data-itm-new-button-managed` with centered `$moduleListHeading` (`itm_sidebar_label_for_module()` fallback `🧾 Audit Logs`) and Settings-positioned Refresh controls (no create action — `create.php` redirects to index). Reviewed UI config gaps (`+ New Button`, new-button position/style, bulk delete, edit/delete entry, back/save) are listed in `scripts/data/ui_configuration_reviewed.json`.
- **Favicon:** `index.php` `<head>` calls `itm_render_head_favicon_link()` so the tab icon matches Settings (not header.php JS alone).
