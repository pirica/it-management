...
- Database triggers `trg_ticket_statuses_audit_insert`, `trg_ticket_statuses_audit_update`, `trg_ticket_statuses_audit_delete` on `ticket_statuses` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **tickets** (`status_id`). [Cursor-Valid]
