...
- Database triggers `trg_supplier_statuses_audit_insert`, `trg_supplier_statuses_audit_update`, `trg_supplier_statuses_audit_delete` on `supplier_statuses` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
