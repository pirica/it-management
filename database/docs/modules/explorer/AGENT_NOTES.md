...
## 2. Key Tables
- **explorer** — file/folder metadata (optional tracking). Supports standard audit columns: `active`, `deleted_by`, `deleted_at`, `created_by`, `created_at`, `updated_by`, `updated_at`. Uniques are declared on `CREATE TABLE`: `uq_explorer_company_path_name` (`company_id`, `folder_path`(191), `file_name`(191)) and `uq_explorer_user_path_name` (adds `employee_id`) — no live `ALTER TABLE` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
