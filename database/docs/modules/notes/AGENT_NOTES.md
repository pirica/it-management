...
- **Private data (no audit):** `notes` and `note_labels` must not write to `audit_logs` and have no `trg_*_audit_*` triggers in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` (see `AGENTS.md` → **Private data — no audit trail**).
