...
- Seed data in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` duplicates types per company; never copy rows across tenants without setting `company_id`.
...
- **Deleting in-use types:** `ON DELETE RESTRICT` from `idf_positions` blocks deletion — reassign positions in `modules/idfs/` or `idf_positions` first. [Cursor-Valid]
