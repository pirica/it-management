# AGENT_NOTES.md - Is Router

## 1. Module Purpose
A filtered view of the Equipment module specifically for router devices. Provides a type-scoped sidebar entry for network teams to manage routing hardware without the full equipment catalogue.

## 2. Key Tables
- **equipment** — main asset records (read/write via `modules/equipment/`).
- **equipment_types** — joined for the `Router` type filter (`et.name`).

## 3. Required Relationships
- **equipment** → depends on **companies** (`company_id`).
- **equipment** → depends on **equipment_types** (`equipment_type_id`; filter matches `Router`).
...
- **Filter:** List and view enforce `LOWER(TRIM(et.name)) = 'router'` via `$equipmentTypeNameFilter = 'Router'` in `index.php` / `view.php`.
...
- **Standard Equipment list** (search, sort, pagination, export) scoped to routers.
...
## 7. File Structure
- **index.php** — sets `$equipmentTypeNameFilter = 'Router'` and wrapper flags; `require '../equipment/index.php'`.
...
## 10. Common Pitfalls
- **Do not delete canonical wrappers:** Keep `modules/is_router/` and sibling `is_*` folders. [Cursor-Valid]
- **Type filter:** `$equipmentTypeNameFilter` must match `equipment_types.name` (`Router`). [Cursor-Valid]
...
$typeName = 'Router';
$stmt = $conn->prepare(
    "SELECT e.id, e.name, e.hostname, e.ip_address
     FROM equipment e
...
Provides a specialised view for router asset management alongside the general Equipment module.
