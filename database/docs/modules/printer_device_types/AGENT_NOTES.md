...
- Database triggers `trg_printer_device_types_audit_insert`, `trg_printer_device_types_audit_update`, `trg_printer_device_types_audit_delete` on `printer_device_types` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
