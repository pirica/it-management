...
- **email_smtp_configurations** — SMTP host/port/credentials plus IMAP port (default 143) and POP3 port (default 110), TLS mode (default `None`), and require-secure toggle (default off); `is_default = 1` selects the tenant transport. `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` seeds one default **IT Manager** profile per company (`companies` id 1–5).
...
- Alert runner: `php scripts/run_email_alert_rules.php` (schedule via cron). Company 1 `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` seeds use relative warranty/license expiry (`DATE_ADD(CURDATE(), …)`) so rows stay inside the default 30-day window after import. `verify_emails_module.php` **fails** (does not skip) when the window is empty; it inserts a disposable company-1 license sample, re-asserts, then deletes it. Other tenants need enabled rules plus `notify_emails`. Use `--verbose` when dispatched count is 0.
...
- **SMTP / alert rules:** `email_smtp_configurations` and `email_alert_rules` remain auditable via `trg_*_audit_*` triggers in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`.
