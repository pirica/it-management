...
- Database triggers `trg_employee_roles_audit_insert`, `trg_employee_roles_audit_update`, `trg_employee_roles_audit_delete` on `employee_roles` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **users**, **role_hierarchy**, **role_module_permissions**, **role_assignment_rights**. [Cursor-Valid]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM employee_roles WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO employee_roles (company_id, name, active) VALUES (?, ?, ?)");
