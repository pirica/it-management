# AGENT_NOTES.md - Is POS

## 1. Module Purpose
A filtered view of the Equipment module specifically for point-of-sale (POS) devices. Provides a type-scoped sidebar entry for retail and hospitality IT teams without the full equipment catalogue.

## 2. Key Tables
- **equipment** — main asset records (read/write via `modules/equipment/`).
- **equipment_types** — joined for the `POS` type filter (`et.name`).

## 3. Required Relationships
- **equipment** → depends on **companies** (`company_id`).
- **equipment** → depends on **equipment_types** (`equipment_type_id`; filter matches `POS`).
...
- **Filter:** List and view enforce `LOWER(TRIM(et.name)) = 'pos'` via `$equipmentTypeNameFilter = 'POS'` in `index.php` / `view.php`.
...
- **Standard Equipment list** (search, sort, pagination, export) scoped to POS terminals.
...
## 7. File Structure
- **index.php** — sets `$equipmentTypeNameFilter = 'POS'` and wrapper flags; `require '../equipment/index.php'`.
...
## 10. Common Pitfalls
- **Do not delete canonical wrappers:** Keep `modules/is_pos/` and sibling `is_*` folders. [Cursor-Valid]
- **Type filter:** Use `POS` (uppercase) — must match `equipment_types.name` case-insensitively via `LOWER(TRIM(et.name))`. [Cursor-Valid]
...
Provides a specialised view for POS asset management alongside the general Equipment module.
