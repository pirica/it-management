...
- Email Alert Rules is a sub-feature of **Email Management**. Server-side CRUD permission checks must use the parent module slug **`emails`**, not `email_alert_rules`, because `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` registers Email Management in `modules_registry` as `emails`.
