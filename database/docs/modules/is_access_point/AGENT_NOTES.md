...
- Entries are written to **audit_logs** with `table_name = 'equipment'` and JSON `old_values` / `new_values`; session context comes from `@app_employee_id` / `@app_company_id` set in `config/config.php`.
