...
- Database triggers `trg_vlans_audit_insert`, `trg_vlans_audit_update`, `trg_vlans_audit_delete` on `vlans` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
