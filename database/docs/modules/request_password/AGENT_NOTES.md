...
- Audit triggers are defined in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` for INSERT, UPDATE, and DELETE actions on `request_password`, which capture both old and new states in the `audit_logs` table.
