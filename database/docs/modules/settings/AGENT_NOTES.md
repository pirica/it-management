...
- **UI Persistence**: Changes to button positions or pagination must call `collectAndSetHiddenFields()` in the UI. Fresh-import / column defaults for `table_actions_position`, `new_button_position`, `export_buttons_position`, and `back_save_position` are **`left`** (`database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql`, `itm_ui_config_defaults()`); Settings dropdown labels mark **Left (default)**.
