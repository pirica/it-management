...
- **add_sample_data** (POST index) — seeds empty tenant from `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` when table empty.
...
- `trg_rack_planner_audit_insert|update|delete` in `database/01_schema.sql`, `database/02_triggers.sql`, and `database/03_data.sql` logging all fields.
