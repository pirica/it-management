...
- **JSON import validation**: `itm_handle_json_table_import()` rejects invalid numeric, date/datetime, and enum column values (increments `failed`, sets `ok:false`, HTTP 400 when no rows inserted/updated). Regression: `php scripts/verify_json_import_validation.php`. On update, only columns provided in the import (or auto-derived) are modified in the database to prevent data loss (`providedFields` tracking).
