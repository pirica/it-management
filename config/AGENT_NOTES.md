...
- **JSON import validation**: `itm_handle_json_table_import()` rejects non-numeric values for numeric columns (increments `failed`, sets `ok:false`, HTTP 400 when no rows inserted/updated). Regression: `php scripts/verify_json_import_validation.php`.
