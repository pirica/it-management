...
- **Session cookies (web only):** before `session_start()`, `session_set_cookie_params()` sets `httponly=true`, `samesite=Lax`, `path=ITM_SESSION_COOKIE_PATH` (app subdirectory when deployed under e.g. `/it-management`, otherwise `/`), and `secure` when the request is HTTPS (including `X-Forwarded-Proto=https` / port 443). CLI skips cookie params. Scoped path avoids sharing `PHPSESSID` with other PHP apps on the same host (prevents login CSRF token mismatches).
