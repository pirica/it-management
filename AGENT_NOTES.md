...
- **Personalized Sidebar:** lists every catalog module (except `dashboard_link`). Checkbox **checked** state uses `itm_sidebar_item_effective_visible()` — same rules as `includes/sidebar.php` (visibility pref + `has_module_access` + audit/equipment gates). Unchecking saves `is_visible=0` via `itm_user_config_save_personalized_sidebar_items()` (full layout persist, not partial INSERT). Modules without company access appear in the grid unchecked and cannot be enabled. Labels link to `modules/{slug}/` (or catalog `href`) in a new tab (`itm-user-config-sidebar-link`).
