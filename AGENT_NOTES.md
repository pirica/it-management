...
- **Personalized Sidebar:** each catalog label (except `dashboard_link`) is a link to `modules/{slug}/` (or the item `href`) with `target="_blank"`, `rel="noopener noreferrer"`, and `style="text-decoration:none"` so prefs stay clickable without underline chrome (e.g. 📊 Reports Hub → `modules/reports/`).
