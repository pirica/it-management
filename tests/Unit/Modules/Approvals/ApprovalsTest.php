...
class ApprovalsTest extends TestCase
{
    ...
    public function testCRUD()
    {
        ...
        $resforecast_revision_id = mysqli_query($this->conn, "SELECT id FROM `forecast_revisions` WHERE " . (strpos('forecast_revisions', 'companies') === false && strpos('forecast_revisions', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
        ...
        $resstage = mysqli_query($this->conn, "SELECT id FROM `approvals_stage` WHERE " . (strpos('approvals_stage', 'companies') === false && strpos('approvals_stage', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
        ...
        $resapproved_by = mysqli_query($this->conn, "SELECT id FROM `users` WHERE " . (strpos('users', 'companies') === false && strpos('users', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
