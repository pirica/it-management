...
class UserRolesTest extends TestCase
{
    ...
    public function testCRUD()
    {
        ...
        $sql = "INSERT INTO `user_roles` (company_id, `name`, `active`) VALUES (?, ?, ?)";
        ...
        $updateSql = "UPDATE `user_roles` SET `name` = ? WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $updateSql);
        mysqli_stmt_bind_param($stmt, 'si', $updatedValue, $id);
        $this->assertTrue(mysqli_stmt_execute($stmt));
        mysqli_stmt_close($stmt);

        $res = mysqli_query($this->conn, "SELECT `name` FROM `user_roles` WHERE id = $id");
        $row = mysqli_fetch_assoc($res);
