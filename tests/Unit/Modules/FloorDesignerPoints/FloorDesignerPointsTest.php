...
class FloorDesignerPointsTest extends TestCase
{
    ...
    public function testCRUD()
    {
        ...
        $resfloor_designer_id = mysqli_query($this->conn, "SELECT id FROM `floor_designer` WHERE " . (strpos('floor_designer', 'companies') === false && strpos('floor_designer', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
        ...
        $respoint_type_id = mysqli_query($this->conn, "SELECT id FROM `switch_port_types` WHERE " . (strpos('switch_port_types', 'companies') === false && strpos('switch_port_types', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
        ...
        $resswitch_id = mysqli_query($this->conn, "SELECT id FROM `equipment` WHERE " . (strpos('equipment', 'companies') === false && strpos('equipment', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
        ...
        $resswitch_port_id = mysqli_query($this->conn, "SELECT id FROM `switch_ports` WHERE " . (strpos('switch_ports', 'companies') === false && strpos('switch_ports', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
