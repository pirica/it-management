...
class FloorPlansTest extends TestCase
{
    ...
    public function testCRUD()
    {
        ...
        $resfolder_id = mysqli_query($this->conn, "SELECT id FROM `floor_plan_folders` WHERE " . (strpos('floor_plan_folders', 'companies') === false && strpos('floor_plan_folders', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
        ...
        // Find or fallback for created_by_user_id (users)
        $rescreated_by_user_id = mysqli_query($this->conn, "SELECT id FROM `users` WHERE " . (strpos('users', 'companies') === false && strpos('users', 'users') === false ? "company_id = {$this->companyId}" : "1=1") . " LIMIT 1");
        if ($rowcreated_by_user_id = mysqli_fetch_assoc($rescreated_by_user_id)) {
            $data['created_by_user_id'] = $rowcreated_by_user_id['id'];
        } else {
            // If no existing record, we might need to seed it, but for now we skip this test if mandatory
