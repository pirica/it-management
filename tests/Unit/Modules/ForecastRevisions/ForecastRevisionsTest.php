...
namespace Tests\Unit\Modules\ForecastRevisions;

use PHPUnit\Framework\TestCase;

class ForecastRevisionsTest extends TestCase
{
    private $conn;
    private $companyId = 1;
    ...
    public function testCRUD()
    {
        ...
        if ($rowgm_approved_by = mysqli_fetch_assoc($resgm_approved_by)) {
            ...
            // If no existing record, we might need to seed it, but for now we skip this test if mandatory
            $data['gm_approved_by'] = null;
        }

        $sql = "INSERT INTO `forecast_revisions` (company_id, `cost_center_id`, `gl_account_id`, `year`, `month`, `forecast_amount`, `status`, `active`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $bindValues[] = $data['forecast_amount'];
        ...
        // 2. Read
        $res = mysqli_query($this->conn, "SELECT * FROM `forecast_revisions` WHERE id = $id");
        ...
        $updateSql = "UPDATE `forecast_revisions` SET `notes` = ? WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $updateSql);
        mysqli_stmt_bind_param($stmt, 'si', $updatedValue, $id);
        $this->assertTrue(mysqli_stmt_execute($stmt));
        mysqli_stmt_close($stmt);

        $res = mysqli_query($this->conn, "SELECT `notes` FROM `forecast_revisions` WHERE id = $id");
        $row = mysqli_fetch_assoc($res);
        $this->assertEquals($updatedValue, $row['notes']);

        // 4. Delete
        $deleteSql = "DELETE FROM `forecast_revisions` WHERE id = ?";
        $stmt = mysqli_prepare($this->conn, $deleteSql);
        mysqli_stmt_bind_param($stmt, 'i', $id);
        $this->assertTrue(mysqli_stmt_execute($stmt));
        mysqli_stmt_close($stmt);

        $res = mysqli_query($this->conn, "SELECT COUNT(*) as count FROM `forecast_revisions` WHERE id = $id");
        $row = mysqli_fetch_assoc($res);
