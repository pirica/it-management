...
if ($crud_table === 'vlans_per_port' && in_array($crud_action, ['index', 'list_all'], true)) {
    $desiredOrder = ['module_id', 'port_number', 'active', 'type_id', 'vlan_id', 'allowed_vlans', 'access_policy_id'];
    $byField = [];
...
if ($crud_action === 'create' && $company_id > 0) {
    itm_vlans_per_port_apply_create_form_defaults($conn, (int)$company_id, $data);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    if (empty($errors)) {
        if ($crud_action === 'create' && $company_id > 0) {
            itm_vlans_per_port_apply_create_sql_defaults($conn, (int)$company_id, $data, $sqlValues);
        }
