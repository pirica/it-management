<?php
/**
 * Public join page for temporary password share sessions (QR / 6-digit code).
 */
...
if ($accessToken !== '') {
    $session = itm_qr_share_fetch_session_by_token($conn, passwords_share_table_name(), $accessToken);
    if (!$session) {
        $error = 'This share link has expired or is invalid.';
    }
} elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && $submittedCode !== '') {
    $session = itm_qr_share_fetch_session_by_code($conn, passwords_share_table_name(), $submittedCode);
    if (!$session) {
...
itm_qr_share_render_join_page(
    'password',
    passwords_share_join_script_path(),
    $accessToken,
    $submittedCode,
    $error,
    $session,
