...
function loadEntries() {
    apiCall('list_entries', { folder_id: currentFolderId, search: searchQuery }).then(data => {
        ...
        data.forEach(e => {
            ...
            row.innerHTML = `
                <td class="itm-actions-cell" data-itm-actions-origin="1" style="text-align: center;"><div class="itm-actions-wrap"><button class="btn btn-sm" type="button" onclick="itmOpenQrShareModal('ajax_handler.php', ${e.id}, { action: 'create_share_session' })" title="Share to device"><img src="../../images/QR.svg" alt="" width="16" height="16" style="display:block;"></button> <button class="btn btn-sm btn-outline-primary" onclick="openEntryModal(${e.id})" title="Edit">✏️</button> <button class="btn btn-sm btn-outline-danger" onclick="deleteEntry(${e.id})" title="Delete">🗑️</button></div></td>
