<?php
if (!isset($crud_table)) { $crud_table = 'role_hierarchy'; }
if (!isset($crud_title)) { $crud_title = 'Role Hierarchy'; }
...
function cr_fk_label_for_value($conn, $fk, $company_id, $rawValue) {
    $value = (int)$rawValue;
    if ($value <= 0) {
        return '';
    }
    ...
    $idCol = (string)$fk['REFERENCED_COLUMN_NAME'];
    $meta = cr_fk_metadata($conn, $table);
    $labelCol = (string)$meta['label_col'];
    $cacheKey = $table . '|' . $idCol . '|' . $labelCol . '|' . (int)$company_id . '|' . $value;
    if (array_key_exists($cacheKey, $cache)) {
        return (string)$cache[$cacheKey];
    }

    $sql = 'SELECT ' . cr_escape_identifier($labelCol) . ' AS label FROM ' . cr_escape_identifier($table)
        . ' WHERE ' . cr_escape_identifier($idCol) . '=' . $value;
    if (in_array('company_id', $meta['available'], true) && $company_id > 0) {
        $sql .= ' AND company_id=' . (int)$company_id;
    }
    $sql .= ' LIMIT 1';

    $res = mysqli_query($conn, $sql);
    $row = ($res && ($found = mysqli_fetch_assoc($res))) ? $found : null;
    if (!$row && in_array('company_id', $meta['available'], true)) {
        $fallbackSql = 'SELECT ' . cr_escape_identifier($labelCol) . ' AS label FROM ' . cr_escape_identifier($table)
            . ' WHERE ' . cr_escape_identifier($idCol) . '=' . $value . ' LIMIT 1';
        $fallbackRes = mysqli_query($conn, $fallbackSql);
        $row = ($fallbackRes && ($found = mysqli_fetch_assoc($fallbackRes))) ? $found : null;
    }
...
function cr_fk_options_with_current($conn, $fk, $company_id, $currentValue) {
    $options = cr_fk_options($conn, $fk, $company_id);
    $currentId = (int)$currentValue;
    if ($currentId <= 0) {
        return $options;
    }

    foreach ($options as $option) {
        if ((int)($option['id'] ?? 0) === $currentId) {
            return $options;
        }
    }

    $label = cr_fk_label_for_value($conn, $fk, $company_id, $currentId);
    if ($label === '') {
        return $options;
    }

    $options[] = ['id' => $currentId, 'label' => $label];
    return $options;
}
...
function cr_render_cell_value($table, $field, $value, $displayValue = null) {
    if ($field === 'color') {
        ...
        if (preg_match('/^#[A-Fa-f0-9]{6}$/', $color)) {
            return '<span title="' . sanitize(strtoupper($color)) . '" style="display:inline-block;width:10px;height:10px;border:1px solid #999;border-radius:2px;vertical-align:middle;margin-right:6px;background:' . sanitize($color) . ';"></span>' . sanitize(strtoupper($color));
        }
    }
