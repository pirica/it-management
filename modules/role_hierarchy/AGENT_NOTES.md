...
- Database triggers `trg_role_hierarchy_audit_insert`, `trg_role_hierarchy_audit_update`, `trg_role_hierarchy_audit_delete` on `role_hierarchy` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
