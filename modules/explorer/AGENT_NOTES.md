...
- **Department folders:** `index.php` and `api.php` call `explorer_ensure_tenant_storage_scaffold()` on load — creates `files/{company_id}/Departments/{departments.code}/` when missing for every active tenant department (and standard Common/Private/Trash segments). `file.php` ACL uses the same `departments.code` segment as `get_full_path()`, not numeric `department_id`.
...
- **Employee profile photos (`file.php`):** paths matching `Private/*/profile/` require tenant authorization before serve: active session `company_id` must match the photo owner’s home `company_id`, **or** the session **`employee_id`** requester must pass **`itm_employee_has_company_access()`** (home company or active **`employee_companies`** grant). Cross-tenant reads without a grant return **403**. Grant checks use the requester passed from `file.php`, not `login_employee_id`, so an unrelated signed-in session cannot widen access. Storage root is resolved from the **photo owner’s home `company_id`** (parsed from `Private/{username}_{employee_id}/…`), not the tenant-switcher session company — otherwise multi-company admins get 404 after upload. Other `Private/` paths remain owner-scoped under the active session company.
...
```php
$dir = get_full_path($storage_root, $path, $user_id, $dept_code, $username);
if (!$dir) {
