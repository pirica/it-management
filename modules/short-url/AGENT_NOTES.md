...
- **short_url_settings** — per-company defaults (expiry days, code min length, HTTPS requirement, analytics, password allow)
...
- Configuration tab: admin edit only (`itm_is_admin()`); all users read.
- No Add sample data.

## 5. UI Behavior Requirements

Bespoke UI with `?tab=links|configuration` (emails-style tabs). Hero shorten form on Links tab with feature cards (custom code, password, expiry, QR). Gate-excluded in `scripts/data/ui_configuration_excluded_modules.txt`.

- **index.php** — router; tabs in `includes/partials/render.php`
