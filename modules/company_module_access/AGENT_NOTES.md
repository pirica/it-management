...
- Matrix and registry list order: `module_slug ASC` from `itm_list_all_modules_registry()` (not column-header sort on the matrix).
- **fields_missing reviewed gate:** matrix `index.php` intentionally omits flattened CRUD **Sort**, **Pagination**, and **Import Excel** (`import_excel_rows` on matrix table). Documented in `scripts/data/fields_missing_reviewed.json` — matrix shows all modules; registry import/export uses `table-tools.js` on the matrix card and `data-itm-db-import-endpoint` on `list_all` only.
- `index.php` — company × module matrix with AJAX checkboxes and ✅/❌ indicators (`1` = ✅, `0` = ❌ only; never ✓/✗), compact editable emoji inputs (pre-filled with effective icon), Select All / Cancel Select / Unselect All, optional client-side matrix filter (`#cma-matrix-filter`), and server-side search (`$_GET['search']`, `$searchConditions`, slug/name filter on `$registryRows`) with Search submit + emoji-only 🔙 reset on the matrix card. Matrix table uses `table-tools.js` Export Excel, Export PDF, and Import Excel (same toolbar as before the export opt-out was removed). List header uses `data-itm-new-button-managed` with centered `$moduleListHeading` from `itm_sidebar_label_for_module()` and Settings `new_button_position` create slots (Matrix / Registry List links stay in the left toolbar group).
