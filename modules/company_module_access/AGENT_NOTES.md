...
- Opt-in policy: `has_module_access()` requires `company_module_access.enabled = 1` plus active registry row; missing row denies access. Fresh installs seed all company × module rows in `db/03_triggers.sql`. `itm_ensure_company_module_access_for_company()` (called from `has_module_access()` prefetch) auto-inserts missing pairs as `enabled=1` after partial imports.
...
- **fields_missing reviewed gate:** matrix `index.php` intentionally omits flattened CRUD **Sort** and **Pagination** (documented in `scripts/data/fields_missing_reviewed.json`). Matrix shows all registry modules ordered by slug; **Import Excel** uses `import_excel_rows` → `itm_handle_json_table_import($conn, 'modules_registry', 0)` on matrix and registry list tables (`data-itm-db-import-endpoint="index.php"`).
- **ui_configuration reviewed gate:** matrix intentionally omits flattened CRUD **Sort**, **Pagination**, and scaffold **bulk delete** toolbar (documented in `scripts/data/ui_configuration_reviewed.json`). Rows stay `ORDER BY module_slug ASC`; bulk access changes use `ajax_action=bulk_toggle_access`, not `bulk-delete-selection.js`.
- `index.php` — company × module matrix with AJAX checkboxes and ✅/❌ indicators (`1` = ✅, `0` = ❌ only; never ✓/✗), compact editable emoji inputs (pre-filled with effective icon), Select All / Cancel Select / Unselect All, optional client-side matrix filter (`#cma-matrix-filter`), and server-side search (`$_GET['search']`, `$searchConditions`, slug/name filter on `$registryRows`) with Search submit + emoji-only 🔙 reset on the matrix card. Matrix and registry list tables use `data-itm-db-import-endpoint="index.php"`; JSON `import_excel_rows` on POST imports into `modules_registry` via `itm_handle_json_table_import()`. Matrix table uses `table-tools.js` Export Excel, Export PDF, and Import Excel. List header uses `data-itm-new-button-managed` with centered `$moduleListHeading` from `itm_sidebar_label_for_module()` and Settings `new_button_position` create slots (Matrix / Registry List links stay in the left toolbar group).
- `list_all.php` — flat registry list with the same server-side search (`$searchConditions` on slug/name) and Search + 🔙 reset row.
