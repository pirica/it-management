<?php
$crud_table = 'hotel_booking_amenities';
$crud_title = 'Booking Amenities';
...
if ($field === 'active') {
    ...
    if ($field === 'icon_slug' && ($GLOBALS['crud_table'] ?? '') === 'hotel_booking_amenities') {
        return itm_hotel_booking_amenity_icon_markup((string) $value, 24) . ' <span class="hb-amenity-slug-label">' . sanitize((string) $value) . '</span>';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    if ($crud_table === 'hotel_booking_amenities') {
        $slug = itm_hotel_booking_amenity_sanitize_slug((string) ($_POST['icon_slug'] ?? ($data['icon_slug'] ?? 'default')));
        $data['icon_slug'] = $slug;
        $sqlValues['icon_slug'] = "'" . mysqli_real_escape_string($conn, $slug) . "'";
    }
            ...
            <?php if (in_array($crud_action, ['index', 'list_all'], true)): ?>
                <div data-itm-new-button-managed="server" style="position:relative;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;min-height:40px;">
                    <?php if (in_array($newButtonPosition, ['left', 'left_right'], true)): ?>
                        <?php itm_hospitality_render_list_create_and_hub('btn btn-primary'); ?>
                    <?php else: ?>
                        <span></span>
                    <?php endif; ?>
                ...
                <p style="margin-bottom:12px;"><a href="icons.php" class="btn btn-sm" title="Manage amenity SVG icons">Manage icons</a></p>
                                ...
                                <?php foreach ($uiColumns as $col): $f = $col['Field']; ?>
                                    <td>
                                        <?php if ($f === 'comments' && trim((string)($row[$f] ?? '')) !== ''): ?>
                                            <a class="btn btn-sm" href="edit.php?id=<?php echo (int)$row['id']; ?>">??</a>
                                        <?php else: ?>
...
<button type="submit" name="add_sample_data" value="1" class="btn btn-primary">Add sample data</button>
                ...
                <?php if ($totalRows > $perPage): ?>
                    <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-top:12px;">
                        ...
                        <div style="display:flex;gap:6px;flex-wrap:wrap;">
                            <?php if ($page > 1): ?>
                                <a class="btn btn-sm" href="?search=<?php echo urlencode($searchRaw); ?>&sort=<?php echo urlencode($sort); ?>&dir=<?php echo urlencode($dir); ?>&page=1" title="First page">??</a>
                                <a class="btn btn-sm" href="?search=<?php echo urlencode($searchRaw); ?>&sort=<?php echo urlencode($sort); ?>&dir=<?php echo urlencode($dir); ?>&page=<?php echo $page - 1; ?>" title="Previous page">??</a>
                            <?php endif; ?>
                            <span class="btn btn-sm" style="pointer-events:none;opacity:.8;">Page <?php echo $page; ?> of <?php echo $totalPages; ?></span>
                            <?php if ($page < $totalPages): ?>
                                <a class="btn btn-sm" href="?search=<?php echo urlencode($searchRaw); ?>&sort=<?php echo urlencode($sort); ?>&dir=<?php echo urlencode($dir); ?>&page=<?php echo $page + 1; ?>" title="Next page">??</a>
                                <a class="btn btn-sm" href="?search=<?php echo urlencode($searchRaw); ?>&sort=<?php echo urlencode($sort); ?>&dir=<?php echo urlencode($dir); ?>&page=<?php echo $totalPages; ?>" title="Last page">??</a>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endif; ?>
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    ?>
                        ...
                        <div class="form-group">
                            ...
                            <?php elseif ($name === 'icon_slug' && $crud_table === 'hotel_booking_amenities'): ?>
                                <?php
                                $iconSlugs = itm_hotel_booking_amenity_icon_slugs();
                                $selectedSlug = itm_hotel_booking_amenity_sanitize_slug($displayVal !== '' ? $displayVal : itm_hotel_booking_amenity_guess_slug($data['name'] ?? ''));
                                ?>
                                <input type="hidden" name="icon_slug" id="hb-amenity-icon-slug-input" value="<?php echo sanitize($selectedSlug); ?>">
                                <div class="hb-amenity-icon-picker" role="radiogroup" aria-label="Icon">
                                <?php foreach ($iconSlugs as $slug): ?>
                                <label class="hb-amenity-icon-option<?php echo $slug === $selectedSlug ? ' is-selected' : ''; ?>">
                                <input type="radio" name="hb_amenity_icon_pick" value="<?php echo sanitize($slug); ?>" <?php echo $slug === $selectedSlug ? 'checked' : ''; ?>>
                                <?php echo itm_hotel_booking_amenity_icon_markup($slug, 32); ?>
                                <span><?php echo sanitize($slug); ?></span>
                                </label>
                                <?php endforeach; ?>
                                </div>
            ...
            <?php elseif ($crud_action === 'view'): ?>
                ...
                <div class="card">
                    ...
                    <p style="margin-top:16px;"><a href="index.php" class="btn">??</a> <a class="btn btn-primary" href="edit.php?id=<?php echo (int)($data['id'] ?? 0); ?>">??</a></p>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
<style>
.hb-amenity-icon-picker { display:flex; flex-wrap:wrap; gap:12px; margin-top:8px; }
.hb-amenity-icon-option { display:flex; flex-direction:column; align-items:center; gap:4px; padding:8px; border:2px solid var(--border-color,#dde1e6); border-radius:6px; cursor:pointer; font-size:.75rem; min-width:72px; }
.hb-amenity-icon-option.is-selected { border-color: var(--primary-color,#0b3d6d); background:#f0f6fc; }
.hb-amenity-icon-option input { position:absolute; opacity:0; pointer-events:none; }
.hb-amenity-slug-label { font-size:.85rem; color:#666; margin-left:6px; }
</style>
<script>
document.querySelectorAll('.hb-amenity-icon-option input[type="radio"]').forEach(function (radio) {
  radio.addEventListener('change', function () {
    var hidden = document.getElementById('hb-amenity-icon-slug-input');
    if (hidden) hidden.value = radio.value;
    document.querySelectorAll('.hb-amenity-icon-option').forEach(function (lab) {
      lab.classList.toggle('is-selected', lab.contains(radio) && radio.checked);
    });
  });
});
</script>
