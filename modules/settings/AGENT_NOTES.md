...
- **API Access card**: **Free** tier — no API key UI; copy states **signed-in session** is required for programmatic access and for `scripts/api.php?rate_limit=1` without `api_key` (probe stamps **API Key Last Used** without consuming quota). **Paid** tiers — `api_key` editable (save or generate); **API v2 scope** checkboxes (`api_key_scopes`: `tickets.read`, `tickets.write`, `equipment.read`, `equipment.write`); separate **Save API v2 scopes** submit; OpenAPI link to [scripts/openapi.php?format=json](http://localhost/it-management/scripts/openapi.php?format=json). New keys seed read-only scopes. `tier` is a **blocked** `<select>`; integration API rate-limit counters are read-only. **Explorer API hourly limit** — `explorer_api_rate_limit_per_hour` on `ui_configuration` (default **1200**; **0** = unlimited); POST `save_explorer_api_rate_limit` via `itm_explorer_api_save_rate_limit_per_hour()` (`includes/itm_explorer_api_rate_limit.php`); read-only rolling-hour used/remaining for the signed-in employee. Optional platform override: `.env` `ITM_EXPLORER_API_RATE_LIMIT_PER_HOUR`. Shown on all tiers (separate from paid integration API keys). **API Key Last Used** is read-only (`#api_key_last_used_at_display`) — label from `itm_api_format_key_last_used_display_label()` (`includes/itm_api_rate_limit.php`, UK `dd/mmm/yyyy HH:mm`); `NULL`/empty → `Never`; value comes from `ui_configuration.api_key_last_used_at` updated by `itm_api_touch_key_last_used_at()` on successful rate-limit probe and by `itm_api_consume_rate_limit()` on allowed API requests. Cross-tenant Admin switches (e.g. TechCorp **Admin** on **CloudTech Services**, company `4`) resolve API fields via `itm_api_resolve_configuration_employee_id()` → tenant seed admin (`Admin4`) so the card reads that tenant's `ui_configuration` row, not a missing `(company_id, login_employee_id)` pair. POST `save_api_key` / `generate_api_key` / `save_api_v2_scopes` rejected on Free tier.
...
- Rate-limit probe: `GET scripts/api.php?rate_limit=1` — **Free** omits `api_key` when `PHPSESSID` is signed in; **paid** sends `api_key`. Successful probe stamps `api_key_last_used_at` (no quota consumed). See `includes/itm_api_rate_limit.php` and [scripts/api.php?rate_limit=1](http://localhost/it-management/scripts/api.php?rate_limit=1).
