...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['ics_file'])) {
    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        $_SESSION['crud_success'] = "Successfully imported $importedCount alerts.";
    } else {
...
if (isset($_GET['export']) && $_GET['export'] === 'ics') {
    ...
    $filename = "alerts_export_" . ($export_id > 0 ? "id_$export_id" : date('Ymd')) . ".ics";
...
if ($crud_action === 'delete') {
    ...
    itm_require_crud_role_module_permission($conn, 'delete', 'alerts');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    itm_require_crud_role_module_permission($conn, $crud_action, 'alerts');
    cr_require_valid_csrf_token();
            ...
            <?php if (in_array($crud_action, ['index', 'list_all'], true)): ?>
                ...
                <div class="card" style="margin-bottom:16px;">
                    ...
                    <div style="margin-top: 16px; border-top: 1px solid var(--border); padding-top: 16px;">
                        <form method="POST" enctype="multipart/form-data" id="alertsIcsImportForm" style="width:100%;">
                            <input type="hidden" name="csrf_token" value="<?php echo itm_get_csrf_token(); ?>">
                            <div class="form-group" style="margin:0;width:100%;">
                                <label>Import ICS (.ics)</label>
                                <div id="alertsIcsUploadTarget" class="itm-photo-upload-target" role="button" tabindex="0" aria-label="Upload ICS file">
                                    <p class="itm-dropzone-hint">Drag and drop .ics file here, or click to browse.</p>
                                    <input type="file" name="ics_file" id="alertsIcsFileInput" accept=".ics" required style="font-size: 0.8rem; border: 1px solid var(--border); padding: 5px; border-radius: 4px; background: var(--bg-primary); color: var(--text-primary); width: 100%;">
                                </div>
                                <button type="submit" class="btn btn-sm btn-primary" style="margin-top: 10px;">Import Alerts</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <?php if ($totalRows > $perPage): ?>
                    <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-top:12px;">
                        ...
                        <div style="display:flex;gap:6px;flex-wrap:wrap;">
                            ...
                            <?php endif; ?>
                            <span class="btn btn-sm" style="pointer-alerts:none;opacity:.8;">Page <?php echo $page; ?> of <?php echo $totalPages; ?></span>
