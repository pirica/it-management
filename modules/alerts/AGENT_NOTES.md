...
- **Sample data visibility:** `itm_seed_insert_alerts_sample_row()` in `includes/itm_sample_data_seed.php` inserts one **global** alert (`assigned_to_employee_id IS NULL`) with a unique title even when other users' private alerts exist for the company — the list gate uses `itm_alerts_build_scoped_where_sql()` so only **visible** rows block the button. Legacy template helpers (`itm_seed_apply_alerts_sample_row_defaults()`) remain for fallback paths. Rows with a non-null assignee and `created_by = NULL` are hidden from other users — fix with `UPDATE alerts SET assigned_to_employee_id = NULL, created_by = <employee_id> WHERE …`. [Cursor-Fixed]
