...
- Database triggers `trg_warranty_types_audit_insert`, `trg_warranty_types_audit_update`, `trg_warranty_types_audit_delete` on `warranty_types` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
