...
- Database triggers `trg_warranty_types_audit_insert`, `trg_warranty_types_audit_update`, `trg_warranty_types_audit_delete` on `warranty_types` in `database.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **equipment** (`warranty_type_id`). [Valid]-[2026-07-15]
