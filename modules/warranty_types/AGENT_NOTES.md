...
- Database triggers `trg_warranty_types_audit_insert`, `trg_warranty_types_audit_update`, `trg_warranty_types_audit_delete` on `warranty_types` in `db/03_triggers.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **equipment** (`warranty_type_id`). [Cursor-Valid]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM warranty_types WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO warranty_types (company_id, name) VALUES (?, ?)");
