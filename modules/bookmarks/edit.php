...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($conn, "UPDATE bookmarks SET folder_name = ?, title = ?, url = ?, notes = ?, shared = ?, active = ? WHERE id = ?");
        mysqli_stmt_bind_param($stmt, 'isssiii', $folder_name, $title, $url, $notes, $shared, $active, $id);
        ...
        <form method="POST" class="form-grid">
            ...
            <div class="form-group">
                ...
                <select name="folder_name">
                    ...
                    <?php echo bkm_render_folder_options($folder_tree, $data['folder_name']); ?>
                </select>
            </div>
            <div class="form-group">
                <label>Notes</label>
