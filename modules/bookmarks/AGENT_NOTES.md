...
- Dual-pane `index.php` bulk toolbar (`Select to Delete`, Cancel, Clear Table, **Move to** folder select) and row `ids[]` checkboxes appear only when `$showBulkActions = ($totalRows >= $perPage)`; uses shared `bulk-delete-selection.js` with `#select-all-rows`. **Move to** form (`#bulk-move-form`) stays hidden until at least one row checkbox is checked.
