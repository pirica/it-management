...
- **QR / code share (`join.php`):** `bkm_can_edit_bookmark()` users may create 30-minute temporary read links; private bookmarks require unlocked vault. `bookmark_share_sessions` stores plaintext `payload_json`. UI: 📱 and `images/whatsapp.svg` on dual-pane and `list_all` actions; modal via shared `includes/itm_qr_share_modal.php`. Public page: `join.php` (`ITM_QR_SHARE_PUBLIC`). Regression: `php scripts/verify_qr_share_modules.php`, `php scripts/verify_whatsapp_share.php`.
