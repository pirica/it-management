...
- **Add sample data:** `index.php` POST `add_sample_data` calls `itm_seed_insert_backup_tape_log_today_row()` after `itm_seed_ensure_server_equipment()` only (Server equipment — not Switch), inserting a direct **today** row — not the stale template date from `02_data_sample.sql`. Button shows when no servers exist or the selected server has no row for today.
