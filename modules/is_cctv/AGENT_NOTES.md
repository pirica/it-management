...
- **Do not delete canonical wrappers:** Keep `modules/is_cctv/` and sibling `is_*` folders.
- **Type filter:** Use `CCTV` (not `Cctv`) — must match `equipment_types.name` case-insensitively via `LOWER(TRIM(et.name))`.
