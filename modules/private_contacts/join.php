<?php
/**
 * Public join page for temporary private contact share sessions (QR / 6-digit code).
 */
...
if ($accessToken !== '') {
    $session = itm_qr_share_fetch_session_by_token($conn, pc_share_table_name(), $accessToken);
    if (!$session) {
        $error = 'This share link has expired or is invalid.';
    }
} elseif ($_SERVER['REQUEST_METHOD'] === 'POST' && $submittedCode !== '') {
    $session = itm_qr_share_fetch_session_by_code($conn, pc_share_table_name(), $submittedCode);
    if (!$session) {
