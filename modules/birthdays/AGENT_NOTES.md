...
- Filter card: month `<select>` and **Search (all fields)** across visible columns (Name, Day, Department); marked `data-itm-no-export-pdf="1"` and `data-itm-no-export-excel="1"` so PDF/Excel exports omit controls.
