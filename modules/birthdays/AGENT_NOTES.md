...
- PDF export must not include the month/search filter card; keep `data-itm-no-export-pdf` on the controls card. [Cursor-Valid]
- Do not use `LEFT JOIN` on `employee_statuses` — the `INNER JOIN` is intentional so employees without a status never appear. [Cursor-Valid]
- Month filter must use `MONTH(e.birthday)`, not `DAY()` or string formatting on the full date. [Cursor-Valid]
