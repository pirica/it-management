...
- **Do not delete canonical wrappers:** Keep `modules/is_port_patch_panel/` and sibling `is_*` folders.
- **Type filter:** `$equipmentTypeNameFilter` must match `equipment_types.name` exactly (`Port Patch Panel` — multi-word string).
