...
- Database triggers `trg_rack_statuses_audit_insert`, `trg_rack_statuses_audit_update`, `trg_rack_statuses_audit_delete` on `rack_statuses` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
