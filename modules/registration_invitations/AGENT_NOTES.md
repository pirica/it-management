...
- **Soft-delete + audit meta:** list hides `created_*`/`updated_*`/`deleted_*` and filters `deleted_at IS NULL`; view shows those six meta fields (`*_by` as employee name, `*_at` as `d-m-Y - H:i:s`); create/edit stamp `created_*`/`updated_*` via hidden inputs; delete soft-sets `deleted_by`/`deleted_at`. Delete handlers treat `itm_run_query()` as failed only when it returns `=== false`. **Add sample data** gate uses `itm_seed_tenant_row_count()` (live rows with `deleted_at IS NULL`), not raw `COUNT(*)`. Duplicate-key re-seed restores soft-deleted rows by **`email`** (`itm_detect_fk_business_key_columns()` → `includes/detect_fk_dropdown_ui_risk_lib.php`). Regression: `php scripts/verify_registration_invitations_sample_cycle.php --cycles=4` (companies 1–5) or `--company=4` for one tenant. Browser: [verify_registration_invitations_sample_cycle.php?run=1&cycles=4](http://localhost/it-management/scripts/verify_registration_invitations_sample_cycle.php?run=1&cycles=4). Helpers: `includes/itm_crud_audit_fields.php`, `includes/itm_sample_data_seed.php`. Inventory: `docs/list_soft-delete.txt`. [Cursor-Fixed]
...
- Exposing invitation codes in audit log payloads to non-admin users. [Cursor-Valid]
