...
$crud_table = 'integration_webhooks';
$crud_title = 'Integration Webhooks';
...
require_once __DIR__ . '/iw_module_bootstrap.php';
$pk = 'id';
...
$fieldColumns = iw_module_filter_columns($fieldColumns);
$secretColumnMeta = null;
foreach ($columns as $col) {
    if (($col['Field'] ?? '') === 'secret_encrypted') {
        $secretColumnMeta = $col;
        break;
    }
}
$hasCompany = false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    if ($crud_table === 'integration_webhooks') {
        iw_module_apply_post_rules($crud_action, $data, $errors);
    }
    ...
    if (empty($errors)) {
        ...
        if ($crud_table === 'integration_webhooks' && is_array($secretColumnMeta)) {
            $fields[] = cr_escape_identifier('secret_encrypted');
            $placeholders[] = '?';
            $params[] = $data['secret_encrypted'] ?? null;
            $types .= 's';
        }
        ...
        if ($crud_action === 'create') {
            ...
            if ($crud_table === 'integration_webhooks' && !empty($_POST['rotate_secret']) && is_array($secretColumnMeta)) {
                $fields[] = cr_escape_identifier('secret_encrypted');
                $params[] = $data['secret_encrypted'] ?? null;
                $types .= 's';
            }
...
<button type="submit" name="add_sample_data" value="1" class="btn btn-primary">Add sample data</button>
            ...
            <?php elseif (in_array($crud_action, ['create', 'edit'], true)): ?>
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    <?php endforeach; ?>
                    <?php if ($crud_table === 'integration_webhooks'): ?>
                        <div class="form-group">
                            <label>Event types hint</label>
                            <p style="margin:0;color:#57606a;">Comma-separated: <?php echo sanitize(implode(', ', itm_webhook_queue_event_types())); ?> or *</p>
                        </div>
