...
- **Signatures UI:** list on `signatures.php`; create/edit in modal (Name + Quill); delete in confirm modal; compose dropdown with empty option, signature names, ➕, **✏️** (edit selected), and **🗑️** (delete). After edit from compose (`return_to=compose`), redirect keeps the same `signature_id` selected in the dropdown. List table uses `data-itm-no-table-actions-layout="1"` so Settings table-actions layout does not clone action cells; `js/webmail-signatures.js` uses document-level click delegation for edit/delete/create.
