...
- **Compose preview:** **🔎** beside **📤** opens a modal (`includes/webmail_compose_preview_modal.php`) with labeled **Subject**, From, To, CC, and body; fed by `POST compose.php?ajax_action=preview_message` (CSRF); body uses `webmail_compose_resolve_html_body_for_send()` (same merge/sanitize as send).
