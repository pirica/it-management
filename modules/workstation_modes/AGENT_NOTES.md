...
- Database triggers `trg_workstation_modes_audit_insert`, `trg_workstation_modes_audit_update`, `trg_workstation_modes_audit_delete` on `workstation_modes` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.

## 10. Common Pitfalls
- Do not delete rows still referenced by inbound FKs — reassign or detach dependents for the active `company_id` first.
- Referenced by **employees** (`workstation_mode_id`).
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM workstation_modes WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO workstation_modes (company_id, name, active) VALUES (?, ?, ?)");
