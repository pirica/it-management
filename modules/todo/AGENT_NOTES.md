...
- **Assignee dropdown:** load active users scoped to `company_id` only (`users.company_id` or `user_companies` row). After tasks load, `todo_merge_assignee_users()` augments the map with inactive assignees on existing rows so list/view labels stay visible. Do not include global `Admin` via username bypass.
