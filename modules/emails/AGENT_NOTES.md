...
## 2. Key Tables
- **emails** — send/receive log (`from_email`, `to_email`, `cc_email`, `subject`, `status` = `sent` | `failed` | `received`, `sent_at`, `details`, `is_archived` default `0` hides rows from Send Logs list/stats while keeping `view.php?id=` reachable).
...
- `itm_send_email($to, $subject, $html, $companyId)` logs every attempt to **emails** when `company_id` resolves (new rows use `is_archived = 0`).
