...
- **ticket_inbound_email_routing_rules** — per-tenant keyword rules (`keyword`, optional `priority_id`, `category_id`, `assigned_to_employee_id`, `sort_order`) applied on new inbound tickets.
...
- **Inbound tickets:** default SMTP profile may set `imap_host` + **Create tickets from inbound mail** (`inbound_ticket_enabled`). Cron: [run_inbound_email_tickets.php?run=1](http://localhost/it-management/scripts/run_inbound_email_tickets.php?run=1). Local Laragon/Dunebox: `imap_host` = **`mailpit`** polls the Mailpit HTTP API (`http://localhost/mailpit/api/v1`); production mailboxes use PHP `imap`. Apply on live DB: [apply_mailpit_inbound_email_config.php?run=1&apply=1](http://localhost/it-management/scripts/apply_mailpit_inbound_email_config.php?run=1&apply=1). Route mail to `companies.email`. Inbound events log to **emails** with JSON `details` (`inbound_event`, optional `raw_payload` on failures). Subject-only `Re:`/`Fwd:` threading matches open tickets only when the sender already participated (inbound history, assignee, comment author, or a single unambiguous `created_by` match). Regression: [verify_inbound_email_tickets.php?run=1](http://localhost/it-management/scripts/verify_inbound_email_tickets.php?run=1). Manual test send: `php scripts/send_mailpit_inbound_test_email.php --company=1` (optional `--process`).
