...
- **Inbound tickets:** default SMTP profile may set `imap_host` + **Create tickets from inbound mail** (`inbound_ticket_enabled`). Cron: `php scripts/run_inbound_email_tickets.php` (requires PHP `imap` extension). Route mail to `companies.email`; regression: `php scripts/verify_inbound_email_tickets.php`.
...
- SMTP form: toggle **Set as default SMTP**; password field with reveal button; **IMAP host** + port; toggle **Create tickets from inbound mail**; **POP3** port, TLS mode, and require-secure toggle; help text references `companies.email`; test send on edit.
