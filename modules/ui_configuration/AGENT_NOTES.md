...
- **API columns:** `api_key`, `api_key_is_active`, `api_key_last_used_at`, `rate_limit_window_start`, `rate_limit_request_count`, `rate_limit_enabled`, `tier` (`Free`/`Basic`/`Pro`/`Enterprise`). **Free** — unlimited, **no API key**, identity from authenticated session (`company_id` + `user_id` in `PHPSESSID` via `itm_api_resolve_rate_limit_row()`). **Paid** — hourly caps; `api_key` required for keyless HTTP/programmatic calls.
