...
- **Fresh-import seeds:** one default row per company bound to **that company’s** seed Admin (`INSERT … SELECT` from `employees` where `work_email LIKE 'admin@techcorp.example%.com'`). Position fields seed as **`left`** for `table_actions_position`, `new_button_position`, `export_buttons_position`, and `back_save_position` (matches `CREATE TABLE` column defaults and `itm_ui_config_defaults()`). Do not set `employee_id = 1` for companies 2–5.
- **Add sample data (empty table):** `itm_seed_insert_ui_configuration_sample_row()` mirrors fresh-import defaults (`records_per_page` **25**, `app_name` **⚙️ IT Controls**, tenant favicon path) and resolves `employee_id` via **`itm_seed_resolve_tenant_seed_admin_employee_id()`** (seed Admin username / work_email, then **signed-in employee with `itm_employee_has_company_access()`** for empty or cross-company tenants, then first live tenant employee). Empty gate and list **Add sample data** button use **`itm_seed_tenant_row_count()`** (live rows only — `deleted_at IS NULL`); soft-deleted rows for the target employee are restored instead of re-inserted.
...
- **Boolean toggles on list/view:** `enable_all_error_reporting`, `enable_audit_logs`, `enable_chatbot`, `enable_sidebar_section_collapse`, `enable_auto_scaffolding`, `api_key_is_active`, `rate_limit_enabled`, and locale prefs (`ui_money_symbol_suffix`, `ui_money_symbol_prefix`, `ui_datetime_*_enabled`) render ✅/❌ via `cr_render_cell_value()` (including duplicated renderers in `list_all.php` and `view.php`); create/edit use checkbox double-label where visible. Employee-facing locale editing lives on **Settings → UI Configuration** (`modules/settings/index.php`); this scaffold module mirrors stored columns only.
