...
- Database triggers `trg_equipment_rj45_audit_insert`, `trg_equipment_rj45_audit_update`, `trg_equipment_rj45_audit_delete` on `equipment_rj45` in `database.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **equipment** (`switch_rj45_id`). [Valid]-[2026-07-15]
