...
- **Photo storage:** files under `booking/images/{hotel_id}/hotel_photos/` via `itm_hotel_booking_photo_storage_dir($hotelId, 'hotel_photos')`. **Seed backfill:** `php scripts/seed_hotel_booking_sample_photos.php --apply` copies `hb_seed_*.jpg` into each seed company’s primary hotel folder (companies 1–5) and upserts `hotel_booking_hotel_photos` rows.
...
- **Contact emails:** `contact_email` (admin label **Info**, general enquiries) and `reservations_email` (admin label **Email**, bookings desk) are optional `varchar(255)` fields on create/edit/view/list; POST validates with `filter_var(..., FILTER_VALIDATE_EMAIL)` when non-empty. Portal/public UI uses the same **Info** / **Email** labels. Seeds: company 1 `info@techcorp-retreat.example` / `reservations@techcorp-retreat.example`; companies 2–5 use `info@retreat.{companyslug}.example` and `reservations@retreat.{companyslug}.example` (slug = lowercased company name without spaces).
