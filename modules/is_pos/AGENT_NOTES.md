...
- **Do not delete canonical wrappers:** Keep `modules/is_pos/` and sibling `is_*` folders.
- **Type filter:** Use `POS` (uppercase) — must match `equipment_types.name` case-insensitively via `LOWER(TRIM(et.name))`.
