...
if ($searchRaw !== '') {
    ...
    $where .= " AND (COALESCE(NULLIF(e.display_name, ''), CONCAT(e.first_name, ' ', e.last_name)) LIKE '{$searchValue}' OR COALESCE(e.work_email, '') LIKE '{$searchValue}' OR COALESCE(e.personal_email, '') LIKE '{$searchValue}')";
}

// Build the order by clause
$sortSql = $sort === 'employee_name'
    ? "COALESCE(NULLIF(e.display_name, ''), CONCAT(e.first_name, ' ', e.last_name)) {$dir}"
    : ($sort === 'email' ? "COALESCE(NULLIF(e.work_email, ''), e.personal_email, '') {$dir}" : "COALESCE(NULLIF(e.display_name, ''), CONCAT(e.first_name, ' ', e.last_name)) ASC");
...
if (($_GET['export'] ?? '') === 'csv') {
    ...
    foreach ($employees as $row) {
        ...
        $line = [(string)($row['employee_name'] ?? ''), $effectiveEmail];
        ...
        <div class="content">
            <div data-itm-new-button-managed="server" style="position:relative;display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;min-height:40px;">
                ...
                <?php if (in_array($newButtonPosition, ['right', 'left_right'], true)): ?>
                    <a href="?<?php echo sanitize(esa_module_build_query(['search' => $searchRaw, 'sort' => $sort, 'dir' => $dir, 'export' => 'csv'])); ?>" class="btn btn-primary" title="Export CSV">⬇</a>
                <?php else: ?>
                    <span></span>
                <?php endif; ?>
            </div>
            ...
            <?php if ($totalRows > $perPage): ?>
                <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-top:12px;">
                    ...
                    <div style="display:flex;gap:6px;flex-wrap:wrap;">
                        <?php if ($page > 1): ?>
                            <a class="btn btn-sm" href="?<?php echo sanitize(esa_module_build_query(['search' => $searchRaw, 'sort' => $sort, 'dir' => $dir, 'page' => $page - 1])); ?>" title="◀️ Previous">Previous</a>
                        <?php endif; ?>
                        <span class="btn btn-sm" style="pointer-events:none;opacity:.8;">Page <?php echo $page; ?> of <?php echo $totalPages; ?></span>
                        <?php if ($page < $totalPages): ?>
                            <a class="btn btn-sm" href="?<?php echo sanitize(esa_module_build_query(['search' => $searchRaw, 'sort' => $sort, 'dir' => $dir, 'page' => $page + 1])); ?>" title="▶️ Next">Next</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
