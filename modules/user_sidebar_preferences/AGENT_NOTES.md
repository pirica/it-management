...
- Database triggers `trg_user_sidebar_preferences_audit_insert`, `trg_user_sidebar_preferences_audit_update`, `trg_user_sidebar_preferences_audit_delete` on `user_sidebar_preferences` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
