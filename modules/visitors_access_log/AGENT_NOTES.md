...
- **Date/time inputs:** create/edit and quick-add use `itm_render_uk_datetime_input()` for `date_time_in` / `date_time_out` (UK `dd/mmm/yyyy HH:mm` + hidden `datetime-local`); POST handlers parse via `itm_parse_datetime_input()`.
