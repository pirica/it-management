...
- Database triggers `trg_forecast_revisions_status_audit_insert`, `trg_forecast_revisions_status_audit_update`, `trg_forecast_revisions_status_audit_delete` on `forecast_revisions_status` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
