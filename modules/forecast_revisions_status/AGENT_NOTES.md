...
- Database triggers `trg_forecast_revisions_status_audit_insert`, `trg_forecast_revisions_status_audit_update`, `trg_forecast_revisions_status_audit_delete` on `forecast_revisions_status` in `database.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **forecast_revisions** and **approvals** (`status`). [Valid]-[2026-07-15]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM forecast_revisions_status WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO forecast_revisions_status (company_id, name, active) VALUES (?, ?, ?)");
