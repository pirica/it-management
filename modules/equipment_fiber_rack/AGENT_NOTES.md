...
- Database triggers `trg_equipment_fiber_rack_audit_insert`, `trg_equipment_fiber_rack_audit_update`, `trg_equipment_fiber_rack_audit_delete` on `equipment_fiber_rack` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
