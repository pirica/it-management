...
itm_crud_record_share_handle_ajax_request($conn, 'forecast_revisions');
    ...
    if (empty($errors)) {
        ...
        if (itm_run_query($conn, $sql, $dbErrorCode, $dbErrorMessage)) {
            if (function_exists('itm_approval_inbox_sync_module_record')) {
                ...
                if ($savedRecordId > 0) {
                    itm_approval_inbox_sync_module_record($conn, (int)$company_id, 'forecast_revisions', $savedRecordId);
                }
            }
            header('Location: ' . $listUrl);
            exit;
            ...
            <?php elseif ($crud_action === 'view'): ?>
                ...
                <div class="card">
                    ...
                    <p style="margin-top:16px;">
                        <?php echo itm_crud_record_share_render_action_buttons('forecast_revisions', (int)($data['id'] ?? 0), 'forecast revision'); ?>
