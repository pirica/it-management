...
function cr_delete_inventory_categories_scoped(
    ...
    $deleteSql = function_exists('itm_crud_build_soft_delete_sql')
        ? itm_crud_build_soft_delete_sql(
            $table,
            $deleteAllTenantCategories
                ? (' WHERE company_id=' . (int)$companyId)
                : (' WHERE id IN (' . implode(',', array_values($idList)) . ') AND company_id=' . (int)$companyId),
            (int)($_SESSION['employee_id'] ?? 0)
        )
        : ('DELETE FROM ' . cr_escape_identifier($table) . (
            $deleteAllTenantCategories
                ? (' WHERE company_id=' . (int)$companyId)
                : (' WHERE id IN (' . implode(',', array_values($idList)) . ') AND company_id=' . (int)$companyId)
        ));
    // Keep legacy hard-delete build unused when helper available.
