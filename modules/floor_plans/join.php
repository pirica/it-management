<?php
/**
 * Public join page for temporary Floor Plan share sessions (QR / 6-digit code).
 */
...
if ($accessToken !== '') {
    ...
    $session = itm_qr_share_fetch_session_by_code($conn, floor_plans_share_table_name(), $submittedCode);
    if (!$session) {
...
itm_qr_share_render_join_page(
    'floor plan',
    floor_plans_share_join_script_path(),
    $accessToken,
    $submittedCode,
    $error,
    $session,
