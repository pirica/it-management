...
- **Assignee notifications**: When `assigned_to_employee_id` is set on create, or changed on edit, `itm_notify_event_assigned()` notifies the assignee (including self-assign — matches tickets/alerts). Edit form posts hidden `id` when the query string is omitted.
