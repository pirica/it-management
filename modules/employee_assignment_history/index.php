...
/**
 * Employee Assignment History Module - Index
 * 
 * Main list view for assignment history records. Users can view, create, edit, and delete
 * records defined for the company.
 */

$crud_table = $crud_table ?? 'employee_assignment_history';
$crud_title = $crud_title ?? 'Employee Assignment History';
...
/**
 * Appends a persisted FK value to the option set when company-scoped options miss it.
 */
function cr_fk_options_with_selected($conn, $fk, $company_id, $selectedId) {
    if (function_exists('itm_fk_append_selected_option')) {
        return itm_fk_append_selected_option($conn, $fk, (int)$company_id, cr_fk_options($conn, $fk, $company_id), $selectedId);
    }
    ...
    $fallbackLabel = cr_fk_label_for_id($conn, $fk, $selectedId, $company_id);
...
/**
 * Resolves display labels for FK values with company-first and legacy fallback-by-id behavior.
 */
...
if ($crud_action === 'delete') {
    ...
    itm_require_crud_role_module_permission($conn, 'delete', 'employee_assignment_history');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    itm_require_crud_role_module_permission($conn, $crud_action, 'employee_assignment_history');
    cr_require_valid_csrf_token();
                        ...
                        <div class="form-group">
                            ...
                            <?php elseif (isset($fkMap[$name])): ?>
                                <?php
                                    ...
                                    $opts = cr_fk_options_with_selected($conn, $fkRow, (int)$company_id, $fkSelectedId);
