...
- Database triggers `trg_equipment_fiber_audit_insert`, `trg_equipment_fiber_audit_update`, `trg_equipment_fiber_audit_delete` on `equipment_fiber` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
