...
- Database triggers `trg_equipment_types_audit_insert`, `trg_equipment_types_audit_update`, `trg_equipment_types_audit_delete` on `equipment_types` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.

## 10. Common Pitfalls
- Do not delete rows still referenced by inbound FKs — reassign or detach dependents for the active `company_id` first.
- Referenced by **equipment** and **catalogs**.
