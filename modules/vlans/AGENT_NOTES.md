...
- Database triggers `trg_vlans_audit_insert`, `trg_vlans_audit_update`, `trg_vlans_audit_delete` on `vlans` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.

## 10. Common Pitfalls
- Do not delete rows still referenced by inbound FKs — reassign or detach dependents for the active `company_id` first.
- Referenced by **switch_ports**, **idf_ports**, **ip_subnets** (`vlan_id`).
