...
- Database triggers `trg_role_assignment_rights_audit_insert`, `trg_role_assignment_rights_audit_update`, `trg_role_assignment_rights_audit_delete` on `role_assignment_rights` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
