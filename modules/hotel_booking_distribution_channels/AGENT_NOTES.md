...
- `edit.php` — update channel, rotate API key, partner credentials (Booking.com), generate webhook signing secret, outbound webhook API key, rate-plan mapping CRUD, add mapping rows, **Map all room types** / **Map all hotels** (auto-suggest OTA `external_code` values for unmapped rows). **`webhook_url`** validated via `itm_hotel_booking_distribution_validate_webhook_url()` (blocks private/link-local/metadata targets before save).
