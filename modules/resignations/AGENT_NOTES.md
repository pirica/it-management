...
- Filter card: Week, Month, Year, Employment Status multi-select, Employee Type multi-select, Search (all fields). Search matches visible columns including **dd/mmm/yyyy** admission/last-work-day text and **Official Resignation Week** (`{week}/{yy}` via `DATE_FORMAT` / `CONCAT(WEEK…)`). Search reset uses emoji-only 🔙 (`title="Clear"`) when a query is active. Control card uses `data-itm-no-export-pdf` and `data-itm-no-export-excel`.
...
- Date columns display as **dd/mmm/yyyy** via `itm_format_date_display()`.
