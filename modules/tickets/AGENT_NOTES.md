...
- **Create/edit audit scrape:** business **Created By** (`created_by_employee_id`) and **ticket logged-at** (`created_at`) are editable on `create.php` — not scaffold audit columns; reviewed in `scripts/data/fields_missing_reviewed.json`. New-ticket default **Created By** uses the tenant **context** employee via `modules/tickets/tickets_form_employee_defaults.php` (`tickets_resolve_context_employee_id()` → `itm_resolve_company_context_employee_id()` + `itm_company_session_login_employee_id()`), with per-request session sync from `itm_ensure_company_context_employee_session()` in `config/config.php` so cross-tenant **Admin** at CloudTech maps to **Admin4** (id `4`), not TechCorp **Admin** (id `1`). **Assigned To** stays unselected (`-- Select --`) on create unless the user picks an assignee; edit loads persisted FKs from the row (re-import `db/` for corrected `TCK-0001` seeds on companies 2–5). Regression: `php scripts/verify_tickets_create_defaults.php`.
...
- **`db/` seeds:** one `TCK-0001` row per seeded company (ids 1–5), each with tenant `Open` `status_id`. `created_by_employee_id` and `assigned_to_employee_id` use that tenant's seed Admin employee (`Admin` / `Admin2`…`Admin5` — same numeric id as `company_id` on fresh import), not company-1 Admin for every tenant. The late `@replicate_source_company_id` block does **not** copy `tickets` — explicit per-company inserts are the only seed rows (avoids duplicate open tickets on import).
