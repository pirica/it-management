...
- **Add sample data:** `sample_seed_helpers.php` + `itm_seed_table_from_database_sql('tickets')`; empty gate uses **`tickets_tenant_active_row_count()`** (`deleted_at IS NULL` and `is_archived = 0`). Sample inserts force **`is_archived = 0`** (`itm_seed_apply_tickets_sample_row_defaults()`); legacy archived sample/fallback rows are repaired on list load via **`tickets_repair_invisible_sample_rows()`**.
