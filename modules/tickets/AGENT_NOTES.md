...
- **Activity feed:** [view.php](http://localhost/it-management/modules/tickets/view.php) **Activity** card merges `ticket_comments` + `ticket_activity` via `itm_ticket_unified_activity_feed()`; add comment POST `add_ticket_comment` (CSRF). Edit/archive log `status_changed`, `priority_changed`, `assigned`, `archived` / `unarchived` through `itm_ticket_log_edit_field_changes()` and `itm_ticket_activity_log()`. Helpers: `includes/itm_ticket_activity.php`, `includes/itm_ticket_comments.php`. Regression: `php scripts/verify_ticket_activity.php`.
