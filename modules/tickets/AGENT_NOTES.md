...
- **Create/edit audit scrape:** business **Created By** (`created_by_employee_id`) and **ticket logged-at** (`created_at`) are editable on `create.php` — not scaffold audit columns; reviewed in `scripts/data/fields_missing_reviewed.json`. New-ticket default **Created By** uses the tenant **context** employee (`tickets_resolve_context_employee_id()` → `itm_resolve_company_context_employee_id()` + `itm_company_session_login_employee_id()`), so cross-tenant **Admin** at CloudTech maps to **Admin4** (id `4`), not TechCorp **Admin** (id `1`). **Assigned To** stays unselected (`-- Select --`) on create unless the user picks an assignee; edit loads persisted FKs from the row (re-import `db/` for corrected `TCK-0001` seeds on companies 2–5).
