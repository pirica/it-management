...
- **Inbound email:** `companies.email` is the tenant routing To address. Cron runner `php scripts/run_inbound_email_tickets.php` polls the default SMTP profile IMAP mailbox when `email_smtp_configurations.inbound_ticket_enabled = 1`. New mail creates tickets; replies with `TCK-####` or `[#id]` in the subject append `ticket_comments`. Dedupe: `ticket_inbound_email_messages`. Core: `includes/itm_inbound_email_tickets.php`. Regression: `php scripts/verify_inbound_email_tickets.php`.
