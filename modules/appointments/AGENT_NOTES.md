...
- **`appointment_settings.booking_enabled`:** when `booking_enabled = 0` (or legacy rows without the column and `active = 0`), `itm_appointment_build_week_slots()` returns `booking_disabled` and schedule/reschedule API actions return HTTP 403 with a clear message. Booking UI shows a banner and disables scheduling. Toggle in [Appointment Settings](http://localhost/it-management/modules/appointment_settings/index.php).
