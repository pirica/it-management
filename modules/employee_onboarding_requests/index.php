...
function cr_current_user_display_name($conn, $company_id) {
    ...
    $sql = 'SELECT username, first_name, last_name FROM `employees`' . $companyFilter . ' LIMIT 1';
    ...
    if ((!$result || mysqli_num_rows($result) === 0) && $company_id > 0) {
        $result = mysqli_query($conn, 'SELECT username, first_name, last_name FROM `employees` WHERE id=' . $currentUserId . ' LIMIT 1');
    }
...
function cr_onboarding_employee_email($conn, $company_id, $employeeId) {
    ...
    $sql = 'SELECT work_email, personal_email FROM `employees` WHERE id=?';
    if ($company_id > 0) {
        $sql .= ' AND company_id=?';
    }
    $sql .= ' LIMIT 1';
    ...
    if ((!$row || (trim((string)($row['work_email'] ?? '')) === '' && trim((string)($row['personal_email'] ?? '')) === '')) && $company_id > 0) {
        $fallbackStmt = mysqli_prepare($conn, 'SELECT work_email, personal_email FROM `employees` WHERE id=? LIMIT 1');
        if ($fallbackStmt) {
    ...
    return trim((string)($row['work_email'] ?? '')) ?: (string)($row['personal_email'] ?? '');
}
...
function cr_is_truthy_checkbox_value($value) {
    ...
    return in_array($text, ['1', 'yes', 'true', 'on', 'active', 'y', '�
'], true);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $crud_action === 'view' && isset($_POST['send_approval_email'])) {
    ...
    $htmlBody = '<p>Please review and approve onboarding request <strong>#' . (int)$recordId . '</strong> for <strong>' . sanitize($employeeFullName) . '</strong>.</p>'
        . '<p><a class="itm-plain-link" href="' . sanitize($approvalApproveUrl) . '">Approve request</a> · '
        . '<a class="itm-plain-link" href="' . sanitize($approvalDeclineUrl) . '">Decline request</a></p>';
...
' : '❌'; ?></span>
                                            ...
                                            <?php elseif (isset($onboardingSystemAccessFields[$name])): ?>
                                                ...
                                                <label class="itm-checkbox-control">
                                                    ...
                                                    <span class="itm-check-indicator" aria-hidden="true"><?php echo $isChecked ? '�
' : '❌'; ?></span>
                                                </label>
