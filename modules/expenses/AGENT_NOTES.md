...
- **Create/edit field order:** `itm_expenses_reorder_form_field_columns()` — supplier → dates → PO → PO accepted → quotation order → quotation accepted → amounts → tax → tax snapshot → currency / exchange rate → payment mode → status → cost center / GL / description / invoice / bill / invoice links (`includes/itm_expenses_ap.php`). List grid hides `tax_rate_snapshot`, `exchange_rate`, `bill_id`, and `invoice_id`; create/edit merge them back from `$fieldColumns` for view parity.
...
- **`purchase_order_accepted`**, **`quotation_order_accepted`**, **`is_recursive`**: list/view render ✅/❌ via `cr_render_cell_value()`; create/edit keep checkbox double-label pattern.
...
- **Saved report views:** 💾 save control on search row (`includes/itm_saved_reports_ui.php`); date range + paid status + supplier filters whitelisted. Shared list filters: `includes/itm_expenses_list_query.php`. Doc: `docs/SAVED_REPORT_VIEWS.md`.
