...
/**
 * Equipment Types Module - Index
 * 
 * Main list view for equipment types. Users can view, create, edit, and delete
 * records defined for the company.
 */

$crud_table = $crud_table ?? 'idf_device_type';
$crud_title = $crud_title ?? 'IDF Device Type';
...
function cr_humanize_field($field) {
    ...
    $map = [
        'idfdevicetype_name' => 'IDF Device Type Name',
...
if ($crud_action === 'delete') {
    ...
    itm_require_crud_role_module_permission($conn, 'delete', 'idf_device_type');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    itm_require_crud_role_module_permission($conn, $crud_action, 'idf_device_type');
    cr_require_valid_csrf_token();
