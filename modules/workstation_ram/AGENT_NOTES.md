...
- Database triggers `trg_workstation_ram_audit_insert`, `trg_workstation_ram_audit_update`, `trg_workstation_ram_audit_delete` on `workstation_ram` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
