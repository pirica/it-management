...
- Database triggers `trg_workstation_ram_audit_insert`, `trg_workstation_ram_audit_update`, `trg_workstation_ram_audit_delete` on `workstation_ram` in `database.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).

## 10. Common Pitfalls

- **Soft-delete + audit meta:** list hides `created_*`/`updated_*`/`deleted_*` and filters `deleted_at IS NULL`; view shows those six meta fields (`*_by` as employee name, `*_at` as `d-m-Y - H:i:s`); create/edit use `$uiColumns` (audit meta not shown as widgets) and stamp `created_*`/`updated_*` via hidden inputs; delete soft-sets `deleted_by`/`deleted_at`. List/view render `active` as Active/Inactive badges (no raw `1`/`0`). Helpers: `includes/itm_crud_audit_fields.php`. Inventory: `docs/list_soft-delete.txt`. [Cursor-Fixed]
...
- Referenced by **equipment** (`workstation_ram_id`). [Cursor-Valid]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM workstation_ram WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO workstation_ram (company_id, name) VALUES (?, ?)");
