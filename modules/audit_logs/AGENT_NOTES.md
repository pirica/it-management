...
- **Admin-only UI:** `index.php` and `view.php` require `itm_is_admin()` **or** `role_module_permissions` `can_view` on **Audit Logs** (demo2 / Demo Audit role); non-authorized users redirect to the dashboard. Backup/clear/download remain admin-only (`$isAuditLogsAdmin`). Missing `company_id` redirects to `index.php`; disabled audit logs use `itm_exit_forbidden()` (not plain `exit()` before layout).
