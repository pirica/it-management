...
## 2. Key Tables
- **employees** — main employee data (`photo`, `full_name`, `insurance_n`, `birthday`, `hide_year`, `start_date`, `employee_type_id`, `termination_date`, `role_id`, `access_level_id`, `is_hidden`, `vault_key_hash`, `totp_secret`, `totp_enabled` among profile/auth fields).
...
- **Login eligibility (mandatory):** use `employment_status_id` → `employee_statuses.name` = **Active** (case-insensitive) via `includes/itm_employee_employment_status.php`. Do **not** use row `employees.active` for login — that column is the soft-delete mirror only (`active=1` live / `active=0` when soft-deleted).
- **Soft-delete + hidden active:** list filters `deleted_at IS NULL` and hides the six audit meta fields and row `active`; create/edit stamp hidden `active=1` (not a checkbox); delete/bulk/clear soft-delete via `itm_crud_build_soft_delete_sql()` (`deleted_by` / `deleted_at` / `active=0`). **List/view show Employment Status as badges** (not row `active`). View lists `deleted_by`, `deleted_at`, `created_by`, `created_at`, `updated_by`, `updated_at` (employee names / `d-m-Y - H:i:s`). Soft-deleted rows remain reachable by `view.php?id=`. Helpers: `includes/itm_crud_audit_fields.php`. Inventory: `docs/list_soft-delete.txt`.
- **List scope:** administrator-only module — all entry files call `itm_require_admin()`; non-admins are redirected to the dashboard (same as companies / employee_companies). Index lists every employee row for the active session `company_id` only (`WHERE e.company_id = ?`) **excluding** `is_hidden = 1` rows (`includes/itm_employees_hidden_accounts.php`) and soft-deleted rows. `company_id` and `is_hidden` never render on the index table. Hidden-account delete guards protect `is_hidden = 1` rows where present. **Empty list:** index tbody shows centered **No records found.** when `$totalRows === 0` (uses `$employeeListResult`, not import `$importRows`). **Add sample data:** shown when `$companyVisibleRowCount === 0` (visible live rows for tenant, no search/duplicate filter); POST seeds via `itm_seed_table_from_database_sql()`. Re-seed after soft-delete restores the template row by business keys `username` + `work_email` (`includes/detect_fk_dropdown_ui_risk_lib.php`); scalar subqueries in the sample template are resolved on restore (`itm_seed_resolve_template_value_for_restore()`). Regression: `php scripts/verify_employees_sample_cycle.php`.
- **Bulk toolbar gate:** index sets `$showBulkActions = ($totalRows >= $perPage)` after the tenant count (`itm_resolve_records_per_page()`). Bulk card, row `ids[]` checkboxes, and header select-all render only when `$showBulkActions`; includes `js/bulk-delete-selection.js` in `index.php` HTML (plus header global load) with Cancel via `data-itm-bulk-cancel="1"`.
- **Fresh-import seed admins:** five rows (`Admin` / `Admin2` … `Admin5` for companies 1–5), password `Admin`. `access_level_id` and `employment_status_id` use tenant subqueries at INSERT; `role_id` is set after `employee_roles` seeds via `UPDATE … JOIN` on Admin by `company_id`. Do not seed all companies with `role_id = 1` / `access_level_id = 1`.
...
- **Import (mandatory):**
  ...
  - **Contact email:** neither `work_email` nor `personal_email` is individually required in UI; at least one must be non-empty on create, edit, and import insert/update (helper: `includes/itm_employee_contact_email.php`). Regression: `php scripts/verify_employee_contact_email.php`.
...
- **Auth-sensitive columns:** list and view hide `password`, `vault_key_hash`, `totp_secret`, `totp_enabled` (flag only — never show ciphertext), and reset-token fields via `includes/itm_employees_auth_sensitive_fields.php` (`itm_employees_auth_sensitive_field_names()` merged into index list `$hiddenColumns`; `view.php` uses an explicit field whitelist). TOTP is configured per employee in `user-config.php#vault-security`, not in this admin module.
...
- **Admin delete dependencies:** `employees_delete_record()` (admin-only entry via `delete.php`) runs `itm_employees_detach_delete_dependencies()` inside a transaction, then soft-deletes via `itm_crud_build_soft_delete_sql()` (`active=0`, `deleted_by`, `deleted_at`). Detach clears vaults, access grants, attempts, approvers, and other safe children. Remaining inbound FKs that are not detached (for example `forecast_revisions.submitted_by`) do **not** block soft-delete — the employee row stays for audit `view.php?id=`. Clear-table loops the same helper for live rows (`deleted_at IS NULL`). Helper: `includes/itm_employees_delete_dependencies.php`; shared delete logic: `delete_functions.php`.
- **Role / access level:** `role_id` → `employee_roles.name`, `access_level_id` → `access_levels.name` on create/edit/view/index via `includes/profile_role_access_fields.php` and list FK label joins. `role_id` assignment is restricted by `role_assignment_rights` for non-admin users.
...
## 10. Common Pitfalls
- **Circular Reporting**: Avoid setting an employee to report to themselves or creating a loop. [Cursor-Valid]
- **Profile photo upload on edit:** `emp_profile_photo_store_upload()` needs `username` and `id` on the employee array; omitting `id` in `edit.php` shows a misleading username error. [Cursor-Valid]
- **Resignations report:** after setting `termination_date`, confirm the active company matches `employees.company_id`. If the row is missing from the weekly report, run `php scripts/debug_resignations_termination_date.php --date=18/06/2026 --company_id=4 --employee_id=432 --week=25 --month=6 --year=2026`. [Cursor-Valid]
