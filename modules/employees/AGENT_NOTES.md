...
- **Bulk toolbar gate:** index sets `$showBulkActions = ($totalRows >= $perPage)` after the tenant count (`itm_resolve_records_per_page()`). Bulk card, row `ids[]` checkboxes, and header select-all render only when `$showBulkActions`; uses shared `js/bulk-delete-selection.js` (Cancel via `data-itm-bulk-cancel="1"`).
