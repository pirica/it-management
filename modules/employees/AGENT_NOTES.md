...
- **Soft-delete + hidden active:** list filters `deleted_at IS NULL` and hides the six audit meta fields; create/edit stamp hidden `active=1` (not a checkbox); delete/bulk/clear soft-delete via `itm_crud_build_soft_delete_sql()` (`deleted_by` / `deleted_at` / `active=0`). View lists `active` badges plus `deleted_by`, `deleted_at`, `created_by`, `created_at`, `updated_by`, `updated_at` (employee names / `d-m-Y - H:i:s`). Soft-deleted rows remain reachable by `view.php?id=`. Helpers: `includes/itm_crud_audit_fields.php`. Inventory: `docs/list_soft-delete.txt`.
