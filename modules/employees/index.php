 ...
 *
 * Why: Status lookup and creation must be scoped by company_id to prevent cross-tenant leaks.
 */
if (!function_exists("emp_status_id_from_raw")) {
function emp_status_id_from_raw($conn, $rawStatus, $company_id) {
        ...
        if (empty($errors)) {
            ...
            foreach (array_slice($rows, 1) as $rowOffset => $row) {
                ...
                if (!empty($mapped['raw_status_code'])) {
                    $mapped['employment_status_id'] = emp_status_id_from_raw($conn, $mapped['raw_status_code'], (int)$company_id);
                ...
                } elseif (empty($mapped['department_id']) || !is_numeric($mapped['department_id'])) {
        ...
        <div class="content">
            ...
            <div data-itm-new-button-managed="server" style="position:relative;display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:20px;min-height:40px;flex-wrap:wrap;">
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    <?php if (in_array($newButtonPosition, ['right', 'left_right'], true)): ?><a href="create.php" class="btn btn-primary itm-list-new-button" title="Create">➕</a><?php endif; ?>
