    ...
    if (!$rows || count($rows) < 2) {
        ...
        if (empty($errors)) {
            ...
            foreach (array_slice($rows, 1) as $rowOffset => $row) {
                ...
                $columns = ['company_id','duplicate','first_name','last_name','work_email','personal_email','employee_code','external_id','username','display_name','job_code','comments','raw_status_code','termination_date','request_date','requested_by','termination_requested_by','department_id','employment_status_id'];
...
$preferredOrder = ['id','duplicate','external_id','username','display_name','work_email','personal_email','raw_status_code','first_name','last_name','job_code','employee_position_id','reports_to','department_id','request_date','requested_by','termination_requested_by','termination_date','employment_status_id','workstation_mode_id','assignment_type_id','comments'];
