...
- **Do not delete canonical wrappers:** Keep `modules/is_other/` and sibling `is_*` folders.
- **Type filter:** `$equipmentTypeNameFilter` must match `equipment_types.name` (`Other`).
