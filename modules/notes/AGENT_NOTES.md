...
- **Share notifications:** `itm_notify_note_shared()` on create for all share targets; on edit only **newly added** employee IDs (owner excluded). Edit form posts hidden `id` when the query string is omitted.
