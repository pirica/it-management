...
function notes_handle_vault_requests($conn, $user_id)
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['master_key'])) {
        ...
        if (password_verify((string)$_POST['master_key'], (string)$userRow['vault_key_hash'])) {
            ...
            $redirect = (string)($_POST['notes_vault_redirect'] ?? 'index.php');
...
function notes_render_vault_lock_screen($csrfToken, array $vaultState, $redirectTarget = 'index.php')
{
    ...
    <div style="max-width: 400px; margin: 80px auto; text-align: center;" class="card">
        ...
        <h2>Vault Locked</h2>
        <p>Enter your master key to access private note titles, content, checklists, and labels.</p>
        <?php if ($error !== ''): ?>
            <div class="alert alert-danger"><?php echo sanitize($error); ?></div>
        <?php endif; ?>
        <form method="POST">
            <input type="hidden" name="csrf_token" value="<?php echo sanitize($csrfToken); ?>">
            <input type="hidden" name="notes_vault_redirect" value="<?php echo sanitize($redirectTarget); ?>">
