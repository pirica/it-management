...
/**
 * Bookmark Folders Module - Index
 *
 * Main list view for bookmark folders. Users can view, create, edit, and delete
 * bookmark folder records defined for the company.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['index', 'list_all'], true) && isset($_POST['add_sample_data'])) {
    ...
    if ($insertedRows <= 0 && $seedError !== '') {
        // Fallback for bookmark_folders if seed fails or is empty
...
// Visibility rules for bookmark_folders if employee_id and shared exist
$cols_names = array_column($columns, 'Field');
if (in_array('employee_id', $cols_names) && in_array('shared', $cols_names)) {
    $user_id = (int)($_SESSION['employee_id'] ?? 0);
    $where .= ($where === '' ? ' WHERE ' : ' AND ') . "(employee_id = $user_id OR shared = 1)";
}
