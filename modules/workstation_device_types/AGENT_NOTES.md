...
- Database triggers `trg_workstation_device_types_audit_insert`, `trg_workstation_device_types_audit_update`, `trg_workstation_device_types_audit_delete` on `workstation_device_types` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.

## 10. Common Pitfalls
- Do not delete rows still referenced by inbound FKs — reassign or detach dependents for the active `company_id` first.
- Referenced by **equipment** (`workstation_device_type_id`).
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM workstation_device_types WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO workstation_device_types (company_id, name, active) VALUES (?, ?, ?)");
