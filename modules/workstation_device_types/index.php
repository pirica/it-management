...
/**
 * Workstation Device Types - Main Entry Point
 * 
 * Provides a CRUD interface for managing workstation hardware types 
 * (e.g., Desktop, Laptop, Tablet, All-in-One).
 */
...
if ($crud_action === 'delete') {
    ...
    itm_require_crud_role_module_permission($conn, 'delete', 'workstation_device_types');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    itm_require_crud_role_module_permission($conn, $crud_action, 'workstation_device_types');
    cr_require_valid_csrf_token();
    $sqlValues = [];
