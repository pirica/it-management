<?php
$crud_table = $crud_table ?? 'employee_companies';
$crud_title = $crud_title ?? 'Employee Companies';
...
// Why: Cross-tenant user-company mappings are administrator-only.
...
function cr_resolve_employee_companies_import_employee_id($conn, int $companyId, int $preferredUserId): int
{
    ...
    if ($preferredUserId > 0) {
        if (cr_employee_companies_scope_link_exists($conn, $companyId, $preferredUserId)) {
            return 0;
        }
...
function cr_import_user_display_label($conn, int $employeeId, int $companyId): string
{
    if ($employeeId <= 0) {
        return 'Unknown user';
    }

    // Why: Ensure user details are only visible if they belong to or are already linked to the active company.
...
function cr_employee_companies_scope_link_exists($conn, int $companyId, int $employeeId): bool
{
...
function cr_format_employee_companies_import_row_error($conn, int $spreadsheetRow, int $employeeId, int $dbCode, string $dbMsg, int $companyId): string
{
    $prefix = 'Row ' . $spreadsheetRow . ': ';

    if ($employeeId > 0 && (
        (int)$dbCode === 1062
        || stripos($dbMsg, 'uq_employee_companies_company_scope') !== false
        || stripos($dbMsg, 'Duplicate entry') !== false
    )) {
        return $prefix . '"' . cr_import_user_display_label($conn, $employeeId, $companyId)
...
function cr_render_cell_value($table, $field, $value, $row = []) {
    ...
    if ($table === 'employee_companies' && in_array($field, ['employee_id', 'company_id', 'granted_by_employee_id'], true)) {
        $id = (int)$value;
        if ($id <= 0) {
            return '';
        }

        if ($field === 'employee_id' || $field === 'granted_by_employee_id') {
            $companyId = (int)($GLOBALS['company_id'] ?? 0);
            // Why: Ensure user details are only visible if they belong to or are already linked to the active company.
            $sql = 'SELECT u.username
                    FROM employees u
                    WHERE u.id=' . $id . '
                    AND (u.company_id=' . $companyId . ' OR EXISTS (SELECT 1 FROM employee_companies uc WHERE uc.employee_id = u.id AND uc.company_id=' . $companyId . '))
                    LIMIT 1';
            $res = mysqli_query($conn, $sql);
            $userRow = $res ? mysqli_fetch_assoc($res) : null;
            return sanitize((string)($userRow['username'] ?? $id));
        }

        if ($field === 'company_id') {
            // Why: Only show the company name if it matches the active company being managed to prevent cross-tenant leaks.
            $activeCompanyId = (int)($GLOBALS['company_id'] ?? 0);
...
function cr_is_qa_import_user_company_row($conn, $row) {
    ...
    $sql = 'SELECT username FROM employees WHERE id=' . $employeeId . ' LIMIT 1';
    $res = mysqli_query($conn, $sql);
    $userRow = $res ? mysqli_fetch_assoc($res) : null;
...
function cr_is_admin_user_company_row($conn, $row) {
    ...
    $sql = 'SELECT ur.name FROM employees u LEFT JOIN employee_roles ur ON ur.id = u.role_id WHERE u.id=' . $employeeId . ' LIMIT 1';
    $res = mysqli_query($conn, $sql);
...
// Why: Employee Companies assignments are managed via edit (multi-company select), not a standalone create screen.
if (($crud_table ?? '') === 'employee_companies' && $crud_action === 'create') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['index', 'list_all'], true) && strpos((string)($_SERVER['CONTENT_TYPE'] ?? ''), 'application/json') !== false) {
    ...
    if (is_array($jsonBody) && isset($jsonBody['import_excel_rows'])) {
        ...
        for ($rowIndex = 1; $rowIndex < count($importRows); $rowIndex++) {
            ...
            $effectiveUserId = (int)($rowData['employee_id'] ?? 0);
            ...
            if (($GLOBALS['crud_table'] ?? '') === 'employee_companies') {
                ...
                if ($resolvedUserId <= 0) {
                    ...
                    if (count($importErrors) < $maxImportErrors) {
                        if ($preferredUserId > 0) {
                            $importErrors[] = cr_format_employee_companies_import_row_error(
                                $conn,
                                $spreadsheetRow,
                                $preferredUserId,
                                1062,
                                'uq_employee_companies_company_scope',
                                $scopeCompanyId
                            );
                        } else {
                ...
                $rowData['employee_id'] = (string)$resolvedUserId;
...
if ($crud_action === 'delete') {
    ...
    if ($bulkAction === 'clear_table') {
        ...
        $candidateIds = [];
        $skippedAdminCount = 0;
        $candidateRes = mysqli_query($conn, 'SELECT id, employee_id FROM ' . cr_escape_identifier($crud_table) . $scopeWhere);
    ...
    if ($bulkAction === 'bulk_delete') {
        ...
        foreach ($ids as $rawId) {
            ...
            $selectedCount++;
            $rowSql = 'SELECT employee_id FROM employee_companies WHERE id=' . $id;
            if ($hasCompany && $company_id > 0) {
