...
function cr_import_user_display_label($conn, int $employeeId): string
{
    if ($employeeId <= 0) {
        return 'Unknown user';
    }
...
function cr_format_employee_companies_import_row_error($conn, int $spreadsheetRow, int $employeeId, int $dbCode, string $dbMsg): string
{
    ...
    if ($employeeId > 0 && (
        ...
        return $prefix . '"' . cr_import_user_display_label($conn, $employeeId)
...
function cr_render_cell_value($table, $field, $value, $row = []) {
    ...
    if ($table === 'employee_companies' && in_array($field, ['employee_id', 'company_id', 'granted_by_employee_id'], true)) {
        ...
        if ($field === 'employee_id' || $field === 'granted_by_employee_id') {
            $sql = 'SELECT username FROM employees WHERE id=' . $id . ' LIMIT 1';
            $res = mysqli_query($conn, $sql);
            $row = $res ? mysqli_fetch_assoc($res) : null;
