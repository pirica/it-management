...
if (in_array($crud_action, ['edit', 'view'], true) && $editId > 0) {
    ...
    } elseif (($crud_table ?? '') === 'employee_companies' && isset($data['employee_id'])) {
        $targetUserId = (int)$data['employee_id'];
        $resUserCompanies = mysqli_query($conn, 'SELECT company_id FROM employee_companies WHERE employee_id=' . $targetUserId . ' ORDER BY company_id');
        while ($resUserCompanies && ($row = mysqli_fetch_assoc($resUserCompanies))) {
            $selectedCompanyIds[] = (int)$row['company_id'];
        }
    }
}

if (($crud_table ?? '') === 'employee_companies') {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    if (empty($errors) && ($crud_table ?? '') === 'employee_companies' && $crud_action === 'edit') {
        $targetUserId = (int)($data['employee_id'] ?? 0);
        $postedCompanyIds = $_POST['company_ids'] ?? [];
        if (!is_array($postedCompanyIds)) {
            $postedCompanyIds = [];
        }
        $postedCompanyIds = array_values(array_unique(array_filter(array_map('intval', $postedCompanyIds))));
        $selectedCompanyIds = $postedCompanyIds;

        if ($targetUserId <= 0) {
            $errors[] = 'User is required.';
        } elseif (empty($postedCompanyIds)) {
            $errors[] = 'At least one company is required.';
        } else {
            $grantedByValue = $data['granted_by_employee_id'] ?? 'NULL';
            ...
            $deleteSql = 'DELETE FROM employee_companies WHERE employee_id=' . $targetUserId;
            if (itm_run_query($conn, $deleteSql, $dbErrorCode, $dbErrorMessage)) {
                $insertOk = true;
                foreach ($postedCompanyIds as $companyId) {
                    $insertSql = 'INSERT INTO employee_companies (employee_id, company_id, granted_by_employee_id, active) VALUES ('
        ...
        <div class="content">
            ...
            <?php elseif (in_array($crud_action, ['create', 'edit'], true)): ?>
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    ?>
                        <?php if ($name === 'company_id' && ($crud_table ?? '') === 'employee_companies' && $crud_action === 'edit'): ?>
                        ...
                        <div class="form-group">
                            ...
                            <?php elseif (($crud_table ?? '') === 'employee_companies' && $crud_action === 'edit' && $name === 'employee_id'): ?>
                                <?php
                                    $userLabel = '';
                                    $employeeId = (int)$displayVal;
                                    if ($employeeId > 0) {
                                        $userRes = mysqli_query($conn, 'SELECT username FROM employees WHERE id=' . $employeeId . ' LIMIT 1');
