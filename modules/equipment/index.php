...
<script>
    (function () {
        ...
        function findOptionNameByAliases(options, aliases) {
            const wanted = (Array.isArray(aliases) ? aliases : []).map(function (alias) {
                return String(alias || '').trim().toLowerCase();
            }).filter(function (alias) { return alias !== ''; });
            const match = (Array.isArray(options) ? options : []).find(function (item) {
                return wanted.indexOf(String(item.name || '').trim().toLowerCase()) !== -1;
            });
            return match ? String(match.name) : '';
        }

        function findOptionIdByAliases(options, aliases) {
            const wanted = (Array.isArray(aliases) ? aliases : []).map(function (alias) {
                return String(alias || '').trim().toLowerCase();
            }).filter(function (alias) { return alias !== ''; });
            const match = (Array.isArray(options) ? options : []).find(function (item) {
        ...
        function isDefaultSeedPortColor(color) {
            const normalized = String(color || '').trim().toLowerCase();
            return normalized === '' || normalized === 'black' || normalized === 'grey' || normalized === 'gray';
        }

        function isAutoGeneratedPortLabel(label, portNumber, portType) {
            const normalized = normalizePortLabelValue(label);
            if (normalized === '') {
                return true;
            }
            const typeToken = normalizePortType(portType) === 'sfp' ? 'SFP' : 'RJ45';
            const escapedToken = typeToken.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            const autoPattern = new RegExp('^' + escapedToken + '\\s*' + String(portNumber || '').trim() + '$', 'i');
            return autoPattern.test(normalized);
        }

        function resolveFkSelectValue(portValue, switchMetaValue, options) {
            const saved = String(portValue || '').trim();
            if (saved !== '' && saved !== '0') {
                return saved;
            }
            const meta = String(switchMetaValue || '').trim();
            if (meta !== '' && meta !== '0') {
                return meta;
            }
        ...
        function selectPort(el) {
            ...
            togglePortControlSections(portType);
            document.getElementById('selectedPort').textContent = el.dataset.portNumber;

            let colorValue = String(el.dataset.color || '');
            if (isDefaultSeedPortColor(colorValue)) {
                colorValue = findOptionNameByAliases(colorOptions, ['gray', 'grey']) || colorValue;
            }

            let statusValue = String(el.dataset.status || '');
            if (isUnknownPortStatus(statusValue)) {
                statusValue = findOptionNameByAliases(statusOptions, ['up']) || statusValue;
            }

            let rj45CableValue = String(el.dataset.rj45SpeedId || '');
            if (normalizePortType(portType) === 'rj45' && rj45CableValue.trim() === '') {
                rj45CableValue = findOptionIdByAliases(rj45SpeedOptions, ['cat6a']) || rj45CableValue;
            }

            let labelValue = normalizePortLabelValue(el.dataset.label);
            if (isAutoGeneratedPortLabel(labelValue, el.dataset.portNumber, portType)) {
                labelValue = '';
            }
            ...
            const vlanValue = resolveFkSelectValue(el.dataset.vlanId, '', vlanOptions);
            const rackValue = resolveFkSelectValue(el.dataset.toRackId, readSwitchMeta('rack_id'), rackOptions);
            const idfValue = resolveFkSelectValue(el.dataset.idfId, readSwitchMeta('idf_id') || switchMetaIdfValue, idfOptions);
            const locationValue = resolveFkSelectValue(el.dataset.toLocationId, readSwitchMeta('location_id'), locationOptions);

            document.getElementById('colorSelect').value = colorValue;
            document.getElementById('statusSelect').value = statusValue;
            document.getElementById('rj45CableSelect').value = rj45CableValue;
            document.getElementById('labelInput').value = labelValue;
            document.getElementById('vlanSelect').value = vlanValue;
            ...
            document.getElementById('idfSelect').value = idfValue;
            document.getElementById('toRackSelect').value = rackValue;
            document.getElementById('toLocationSelect').value = locationValue;
