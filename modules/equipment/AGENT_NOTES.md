...
- **Active / soft-delete:** Row `active` is a soft-delete mirror (create/edit hidden `active=1`; soft-delete sets `active=0` with `deleted_by` / `deleted_at`). Business Active/Inactive stays on `status_id` → `equipment_statuses`. List filters `deleted_at IS NULL` and hides the six audit meta fields; view shows `active` badges plus `deleted_by`, `deleted_at`, `created_by`, `created_at`, `updated_by`, `updated_at`. Helpers: `includes/itm_crud_audit_fields.php`. Inventory: `docs/list_soft-delete.txt`.
