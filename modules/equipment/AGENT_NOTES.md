...
- **Lifecycle & depreciation:** Columns `lifecycle_stage`, `depreciation_start_date`, `useful_life_months`, `salvage_value`, `disposal_date`, `disposal_reason` on `equipment`. Create/edit section in `create.php`; view card + timeline + disposal form via `includes/itm_asset_depreciation.php` (`itm_asset_lifecycle_record_disposal()`). Monthly cron: `php scripts/run_asset_depreciation.php`. Verify: `php scripts/verify_asset_depreciation.php`.
