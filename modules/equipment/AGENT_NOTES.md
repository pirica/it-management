...
- **Create validation:** `create.php` requires **Name**, **Type**, and **Status** before save. Switch equipment also requires **RJ45 Ports**. Missing values return `Please fill required fields: …` listing each label. Invalid tenant FK selections (for example `status_id` from another company) return field-specific messages. `switch_port_numbering_layout_id` resolves to the first tenant layout (or errors when none exist). DB foreign-key failures map to `Please select a value for …` via `itm_format_db_constraint_error()` in `config/config.php`.
