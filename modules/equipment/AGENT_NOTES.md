...
- **Create validation:** `create.php` requires **Name** and **Type** before save. Switch equipment also requires **RJ45 Ports**. **Status** is optional on the form — empty selection defaults to the tenant **Active** `equipment_statuses` row (or the first status for that company). Audit meta on create uses `CURRENT_TIMESTAMP` for `created_at` / `updated_at` and SQL `NULL` for `deleted_by` / `deleted_at` (never empty-string timestamps). DB failures capture `mysqli_errno()` / `mysqli_error()` **before** `mysqli_rollback()`. See `scripts/debug_equipment_create_rollback_errno.php`.
- **Create/edit POST:** `equipment_persist_row_prepared()` in `create.php` performs equipment INSERT/UPDATE via prepared statements; switch/idf port sync UPDATEs on save are prepared too (no `escape_sql()` on POST paths).
...
- **Switch Port Manager (`index.php?switch_id=&spm=1`):** clicking a port hydrates the control panel with tenant FK defaults when the row is still at seed values — **Color** Gray/Grey, **Status** Up (replaces seeded Unknown), **RJ45 Cable** Cat6a, **Label** blank when the stored label is the auto-generated `RJ45 N` / `SFP N` token. **VLAN**, **To Rack**, **To IDF**, and **To Location** use the saved port FK when set; otherwise the switch equipment `rack_id` / `idf_id` / `location_id` meta; otherwise auto-select when the dropdown has exactly one option; otherwise the last lookup row id (highest id in the tenant list).
...
- **Hardware EOL dates:** `eol_date`, `extended_date`, `esu_date` on `equipment` (independent of OS/Office/software). Create/edit next to warranty. View shows those fields plus **inherited** Office/OS/software catalog dates (read-only).
- **Software catalog links:** `software_ids[]` multi-select on create/edit; `itm_equipment_software_sync()` inside the existing save transaction. Junction `equipment_software` stores no dates. Doc: `docs/SOFTWARE_EOL.md`.
- **Lifecycle & depreciation:** Columns `lifecycle_stage` (includes `written_off`), `depreciation_start_date`, `useful_life_months`, `salvage_value`, `disposal_date`, `disposal_reason`, pending disposal columns (`disposal_pending_*`) on `equipment`. Create/edit section in `create.php`; view card + timeline + disposal form via `includes/itm_asset_depreciation.php` (`itm_asset_lifecycle_submit_disposal()` routes to immediate disposal or admin approval when `companies.asset_disposal_approval_required = 1`). Monthly cron: `php scripts/run_asset_depreciation.php`. Verify: `php scripts/verify_asset_depreciation.php`.
...
- **Search & Filter**: Index list search uses `includes/itm_equipment_search.php` (`itm_equipment_search_join_sql()`, `itm_equipment_build_search_where_sql()`) to match scalar fields (including dates, purchase cost, workstation/switch/printer columns, `photo_filename`) plus all create/edit FK labels (department code, supplier code, location code, rack code, assignee username, type/manufacturer/status/warranty/workstation/switch lookup names, etc.). Supplier and assignee columns are not shown on the index table. Search reset uses emoji-only 🔙 on `<a>` (`title="Clear"`). Shared list filters: `includes/itm_equipment_list_query.php` (`itm_equipment_list_parse_filters()`). Regression: `php scripts/verify_employees_equipment_search_coverage.php`.
- **Saved report views:** 💾 save control on search row (`includes/itm_saved_reports_ui.php`); optional type filter `equipment_type_name`. Doc: `docs/SAVED_REPORT_VIEWS.md`.
- **View related modules:** `view.php` renders **Patches & Updates** (linked `patches_updates` row by `equipment_id`, one per asset) and **Related tickets** (tickets with matching `equipment_id`) via `includes/itm_patches_updates_integrations.php` and `includes/itm_equipment_view_related.php`; gated by company module access. Create shortcuts: [modules/patches_updates/create.php?equipment_id=](http://localhost/it-management/modules/patches_updates/create.php?equipment_id=) and [modules/tickets/create.php?equipment_id=](http://localhost/it-management/modules/tickets/create.php?equipment_id=). Regression: [verify_equipment_view_related.php?run=1](http://localhost/it-management/scripts/verify_equipment_view_related.php?run=1).
...
- **UI:** Share on view.php; Switch Port Manager share on index.php?switch_id=&spm=1 (share_kind=switch_ports).
