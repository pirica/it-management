...
- **Create validation:** `create.php` requires **Name** and **Type** before save. Switch equipment also requires **RJ45 Ports**. **Status** is optional on the form — empty selection defaults to the tenant **Active** `equipment_statuses` row (or the first status for that company). Missing values return `Please fill required fields: …` listing each label. DB failures capture `mysqli_errno()` / `mysqli_error()` **before** `mysqli_rollback()` so users see column-specific messages (see `scripts/debug_equipment_create_rollback_errno.php`). `switch_port_numbering_layout_id` resolves per tenant; layout is required only for switch equipment.
