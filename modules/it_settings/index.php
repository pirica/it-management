...
/**
 * IT Settings Module - Index
 *
 * Main list view for IT department settings. Users can view, create, edit, and delete
 * IT department settings records defined for the company.
 */

$crud_table = $crud_table ?? 'it_settings';
$crud_title = $crud_title ?? 'IT Department Settings';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['index', 'list_all'], true) && strpos((string)($_SERVER['CONTENT_TYPE'] ?? ''), 'application/json') !== false) {
    ...
    if (is_array($jsonBody) && isset($jsonBody['import_excel_rows'])) {
        ...
        for ($rowIndex = 1; $rowIndex < count($importRows); $rowIndex++) {
            ...
            if (itm_run_query($conn, $sql, $dbErrorCode, $dbErrorMessage) !== false) {
                $insertedRows++;
            }
        }
...
foreach ($fieldColumns as $col) {
    $data[$col['Field']] = '';
    if ($col['Field'] === 'created_by' && empty($data[$col['Field']]) && isset($_SESSION['employee_id'])) {
        $data[$col['Field']] = (string)$_SESSION['employee_id'];
    }
}
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    ?>
                        <?php if ($name === 'created_by'): ?>
                            <input type="hidden" name="created_by" value="<?php echo sanitize((string)$displayVal); ?>">
                            <?php continue; ?>
                        <?php endif; ?>
