<?php
require_once (defined('ROOT_PATH') ? ROOT_PATH . 'config/config.php' : __DIR__ . '/../../../config/config.php');
...
if (!defined('IDF_INCLUDE_HELPERS_ONLY')) {
    ...
    if (function_exists('itm_require_crud_role_module_permission')) {
        $apiScript = basename($_SERVER['SCRIPT_FILENAME'] ?? '');
        $rbacAction = 'view';
        if (preg_match('/(save|update|regen|sync|move|reorder|add|copy)/i', $apiScript)) {
            $rbacAction = 'edit';
        } elseif (preg_match('/(delete|remove)/i', $apiScript)) {
            $rbacAction = 'delete';
        }
        itm_require_crud_role_module_permission($conn, $rbacAction, 'idfs');
    }
if (!isset($company_id)) {
    $company_id = (int)($_SESSION['company_id'] ?? 0);
}

if (!function_exists('idf_read_json')) { function idf_read_json(): array {
...
require_once (defined('ROOT_PATH') ? ROOT_PATH . 'modules/idfs/idf_positions_schema.php' : __DIR__ . '/../idf_positions_schema.php');
