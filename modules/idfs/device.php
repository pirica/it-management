    ...
    <div class="idf-modal" onclick="event.stopPropagation()">
        ...
        <div>
            ...
            <div style="display:flex; gap:8px; align-items:center;">
                <input class="input" id="cableColorModalPicker" type="color" value="#ffff00" style="height:40px; max-width:70px; padding:4px;">
                <input class="input" id="cableColorModalInput" type="text" placeholder="Type new cable color or use picker">
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cableColorModalPicker = document.getElementById('cableColorModalPicker');
    const cableColorModalInput = document.getElementById('cableColorModalInput');
    if (cableColorModalPicker && cableColorModalInput) {
        cableColorModalPicker.addEventListener('input', (event) => {
            cableColorModalInput.value = event.target.value || '#ffff00';
        });
        cableColorModalInput.addEventListener('input', (event) => {
            cableColorModalPicker.value = normalizeColorToHex(event.target.value || 'yellow');
        });
    }
...
function openCableColorModal(cableColorSelect) {
    ...
    const cableColorPicker = document.getElementById('cableColorModalPicker');
    if (cableColorInput) {
        cableColorInput.value = activeCableColorSelect?.dataset.previousValue || 'yellow';
    }
    if (cableColorPicker) {
        cableColorPicker.value = normalizeColorToHex(cableColorInput?.value || 'yellow');
    }
