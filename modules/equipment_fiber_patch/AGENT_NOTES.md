...
- Database triggers `trg_equipment_fiber_patch_audit_insert`, `trg_equipment_fiber_patch_audit_update`, `trg_equipment_fiber_patch_audit_delete` on `equipment_fiber_patch` in `db/03_triggers.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
