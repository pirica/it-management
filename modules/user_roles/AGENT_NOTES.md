...
- Database triggers `trg_user_roles_audit_insert`, `trg_user_roles_audit_update`, `trg_user_roles_audit_delete` on `user_roles` in `database.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).

## 10. Common Pitfalls
- Do not delete rows still referenced by inbound FKs — reassign or detach dependents for the active `company_id` first.
- Referenced by **users**, **role_hierarchy**, **role_module_permissions**, **role_assignment_rights**.
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM user_roles WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO user_roles (company_id, name, active) VALUES (?, ?, ?)");
