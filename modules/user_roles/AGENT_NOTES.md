...
- Database triggers `trg_user_roles_audit_insert`, `trg_user_roles_audit_update`, `trg_user_roles_audit_delete` on `user_roles` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
