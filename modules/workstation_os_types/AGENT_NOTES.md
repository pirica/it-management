...
- Database triggers `trg_workstation_os_types_audit_insert`, `trg_workstation_os_types_audit_update`, `trg_workstation_os_types_audit_delete` on `workstation_os_types` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.

## 10. Common Pitfalls
- Do not delete rows still referenced by inbound FKs — reassign or detach dependents for the active `company_id` first.
- Referenced by **equipment** (`workstation_os_type_id`).
