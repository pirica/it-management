...
/**
 * Appointment Business Hours Module - Create
 *
 * Provides the interface and logic for adding new departments to the system.
 */
...
function cr_render_cell_value($table, $field, $value) {
    ...
    if (($GLOBALS['crud_table'] ?? '') === 'appointment_business_hours') {
        if (in_array($field, ['is_closed', 'allows_in_person', 'allows_remote'], true)) {
            return ((int)$value === 1) ? '✅' : '❌';
        }
    }
