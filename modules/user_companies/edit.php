...
if (in_array($crud_action, ['edit', 'view'], true) && $editId > 0) {
    ...
    } elseif (($crud_table ?? '') === 'user_companies' && isset($data['user_id'])) {
        $targetUserId = (int)$data['user_id'];
        $resUserCompanies = mysqli_query($conn, 'SELECT company_id FROM user_companies WHERE user_id=' . $targetUserId . ' ORDER BY company_id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    if (empty($errors) && ($crud_table ?? '') === 'user_companies' && $crud_action === 'edit') {
        ...
        if ($targetUserId <= 0) {
            ...
            $deleteSql = 'DELETE FROM user_companies WHERE user_id=' . $targetUserId;
            if (itm_run_query($conn, $deleteSql, $dbErrorCode, $dbErrorMessage)) {
                $insertOk = true;
        ...
        <div class="content">
            ...
            <?php elseif (in_array($crud_action, ['create', 'edit'], true)): ?>
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    ?>
                        <?php if ($name === 'company_id' && ($crud_table ?? '') === 'user_companies' && $crud_action === 'edit'): ?>
