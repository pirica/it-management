...
- Database triggers `trg_switch_port_types_audit_insert`, `trg_switch_port_types_audit_update`, `trg_switch_port_types_audit_delete` on `switch_port_types` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
