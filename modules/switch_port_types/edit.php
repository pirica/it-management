    ...
    if ($id > 0) {
        ...
        if ($stmt) {
            ...
            if (!mysqli_stmt_execute($stmt)) {
                $_SESSION['itm_flash_error'] = cr_humanize_db_error(mysqli_stmt_error($stmt));
            }
            mysqli_stmt_close($stmt);
        }
    }
                ...
                if ($existingId !== null) {
                    ...
                    if ($stmtIns) {
                        ...
                        if (mysqli_stmt_execute($stmtIns)) {
                            ...
                            $errors[] = 'Could not add related value for ' . cr_humanize_field($name) . ': ' . mysqli_stmt_error($stmtIns);
                            $data[$name] = null;
                        }
    ...
    if (empty($errors)) {
        ...
        if ($crud_action === 'create') {
            ...
            if ($stmt) {
                ...
                $errors[] = 'Database error: ' . cr_humanize_db_error(mysqli_stmt_error($stmt));
                mysqli_stmt_close($stmt);
            }
        } else {
            $sets = [];
