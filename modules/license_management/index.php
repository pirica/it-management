 ...
 *
 * Main list view for software licenses. Users can view, create, edit, and delete
 * license records scoped to the active company.
 */
...
function cr_is_hidden_employee_field($field) {
    ...
    $hidden = ['company_id', 'user_id', 'location_id', 'location', 'employee_code'];
...
/**
 * Converts user price input: comma decimal separator becomes dot.
 */
function cr_normalize_price_input($rawValue): string
{
    return str_replace(',', '.', trim((string)$rawValue));
}
...
$preferredOrder = ['name', 'license_key', 'license_type_id', 'quantity', 'supplier_id', 'purchase_date', 'expiry_date', 'price', 'active', 'notes'];
...
usort($fieldColumns, static function ($a, $b) use ($fieldOrderMap) {
    ...
    $bPos = $fieldOrderMap[$b['Field']] ?? 999;
    if ($aPos === $bPos) {
        ...
        for ($rowIndex = 1; $rowIndex < count($importRows); $rowIndex++) {
            ...
            foreach ($importColumns as $idx => $columnMeta) {
                ...
                if (preg_match('/int|decimal|float|double/', (string)$columnMeta['Type'])) {
                    if ($fieldName === 'price') {
                        $rawValue = cr_normalize_price_input($rawValue);
                    }
                    if ($fieldName === 'quantity' && $rawValue === '') {
                        $rawValue = '1';
                    }
                ...
                if (in_array($fieldName, ['purchase_date', 'expiry_date'], true) && function_exists('itm_parse_date_input')) {
                    $parsedImportDate = itm_parse_date_input($rawValue);
                    if ($parsedImportDate !== '') {
                        $rowData[$fieldName] = "'" . mysqli_real_escape_string($conn, $parsedImportDate) . "'";
                    }
                    continue;
                }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    itm_require_crud_role_module_permission($conn, $crud_action, 'license_management');
    cr_require_valid_csrf_token();

    if ($crud_table === 'license_management') {
        $licenseName = trim((string)($_POST['name'] ?? ''));
        if ($licenseName === '') {
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    ?>
                        <div class="form-group">
                            ...
                            <?php else: ?>
                                <input type="text" name="<?php echo sanitize($name); ?>" value="<?php echo sanitize($displayVal); ?>"<?php echo ($name === 'name') ? ' required' : ''; ?>>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
