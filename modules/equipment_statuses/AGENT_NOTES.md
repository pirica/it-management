...
- Database triggers `trg_equipment_statuses_audit_insert`, `trg_equipment_statuses_audit_update`, `trg_equipment_statuses_audit_delete` on `equipment_statuses` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
