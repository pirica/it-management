...
- `.hb-plan-filters-row` keeps **Date**, **Days**, and Search on one horizontal row. **Date** uses `.hb-plan-date-filter` (caption `label for=` + widget) so the 📅 button is not nested in a wrapping label.
