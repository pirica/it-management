...
| File | Role |
|------|------|
| `hb_booking_form.php` | Shared create/edit form for `hotel_bookings` — customer, room, Last room snapshot (no Hotel field — one hotel per company), check-in/out, payment, **auth2** PIN, portal rate plan (➕ / modal view/edit), planning `booking_color`, segment status FKs, notes, `active` checkbox, audit hidden inputs |
