...
- Planning filters **Date** (`anchor` query) and **Days** sit on one `.hb-plan-filters-row` with Search. Date uses `label for="hb-plan-anchor"` plus the hospitality widget (the 📅 button is not inside that label). Planning loads **`BASE_URL . 'js/hotel-date-input.js'`** (repo `js/`, not `modules/hotel_bookings/js/`). There is no Hotel dropdown: the grid and `ajax_action=planning_grid` always use that company’s hotel. Hide checkboxes stay on a separate `.hb-plan-hide-filters` row (`hb_hide[]`).
