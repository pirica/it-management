...
- Database triggers `trg_workstation_office_audit_insert`, `trg_workstation_office_audit_update`, `trg_workstation_office_audit_delete` on `workstation_office` in `db/03_triggers.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
