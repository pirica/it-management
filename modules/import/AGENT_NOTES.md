...
- **Direct CSV Handling:** Procedural CSV parsing in PHP was removed in favor of unified AJAX/JSON processing. Do not re-introduce it. [Cursor-Valid]
- **Permissions:** Ensure `itm_require_admin` is called at the top of the entry file. [Cursor-Valid]
- **File Size:** Large imports may hit PHP `memory_limit` or `max_execution_time` if processed synchronously; the AJAX approach helps mitigate UI blocking but server-side limits still apply. [Cursor-Valid]
