...
- **No flattened list contract:** single-day report grid omits standard Sort/Pagination and primary ➕ create on `index.php` (reports open by date; `opr_ensure_report()` auto-creates the daily row). **Search** row sits above the page title (`Search (all fields)` + emoji-only 🔙 reset); child table rows filter in PHP via `opr_report_apply_search_filter()` when `?search=` is set. `fields_missing.php` bespoke gate still runs Sort, Pagination, and New button as `[SKIP][fail][reviewed]` (manifest: `scripts/fields_missing_reviewed.php`).
