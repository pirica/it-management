...
- **No flattened list contract:** single-day report grid omits standard Sort/Pagination and primary ➕ create on `index.php` (reports open by date; `opr_ensure_report()` auto-creates the daily row). **Search** row sits above the page title (`Search (all fields)` + **Scope** + **Section**): **This day** filters child rows via `opr_report_apply_search_filter()`; **All dates** lists matching `report_date` values via `opr_search_cross_date_hits()` (scoped by `search_section`, default `all`, labels from `opr_search_cross_date_section_labels()`). Cross-date hit lines render as `{search term} | {linked report_date} — {section labels}` (newest date first). Links open **This day** with the same term and section. Demo seed + screenshot: `php scripts/seed_ops_report_search_demo.php`, `python scripts/verify_ops_report_search_screenshot.py` (see **`scripts/SCRIPTS.md`**). Regression hit-line assertions: `php scripts/verify_ops_report.php`. When a section is selected with an active search, unrelated blocks hide on **This day**. Emoji-only 🔙 reset clears search. `fields_missing.php` bespoke gate still runs Sort, Pagination, and New button as `[SKIP][fail][reviewed]` (manifest: `scripts/fields_missing_reviewed.php`).
