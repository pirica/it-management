...
- **No flattened list contract:** single-day report grid omits standard Sort/Pagination and primary ➕ create on `index.php` (reports open by date; `opr_ensure_report()` auto-creates the daily row). **Search** row sits above the page title (`Search (all fields)` + **Scope**: **This day** filters child rows via `opr_report_apply_search_filter()`; **All dates** lists matching `report_date` values via `opr_search_cross_date_hits()` across `ops_report` header fields and all child `ops_report_*` tables (link opens that day with **This day** + term). Emoji-only 🔙 reset clears search. `fields_missing.php` bespoke gate still runs Sort, Pagination, and New button as `[SKIP][fail][reviewed]` (manifest: `scripts/fields_missing_reviewed.php`).
