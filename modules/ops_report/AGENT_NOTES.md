...
- **Fixed display (not in `report_ui_json`):** report date suffix (`opr_format_date()`, `d.m.y`) from day/month/year selectors; company line is always `Company:` + `companies.company` for the active tenant; date/export toolbar (Day, Month, Year, Go, Export Excel, Export PDF, share actions) in `.opr-controls` with export/share buttons in shared `.itm-toolbar-actions` (`css/styles.css` — same neutral `btn btn-sm` styling as `table-tools.js`, not `btn-success` / `btn-danger`).
...
- **No flattened list contract:** single-day report grid omits standard Sort/Pagination and primary ➕ create on `index.php` (reports open by date; `opr_ensure_report()` auto-creates the daily row). **Search** row sits above the page title (`Search (all fields)` + **Scope** + **Section**): **This day** filters child rows via `opr_report_apply_search_filter()`; **All dates** lists matching `report_date` values via `opr_search_cross_date_hits()` (scoped by `search_section`, default `all`, labels from `opr_search_cross_date_section_labels()`). Cross-date results render in a sortable table (**Report date**, **Sections** — no Actions column) with server-side sort (`sort`=`report_date`|`sections`, `dir`=`ASC`|`DESC`, default `report_date` `DESC`) and pagination from Settings `records_per_page` (`page` query param). Hit rows keep `{search term} | {linked report_date}` in the first column; section labels in the second. Links open **This day** with the same term and section. When a section is selected with an active search, unrelated blocks hide on **This day**. Emoji-only 🔙 reset clears search. Shared list helpers: `scripts/lib/itm_ops_report_search.php`. Regression: `php scripts/verify_ops_report.php`. Gate-excluded UI config lines (+ New Button, main-grid column sort, new-button position/style, bulk delete) are reviewed in `scripts/data/ui_configuration_reviewed.json`; `fields_missing.php` bespoke Sort/Pagination/New button skips remain in `scripts/data/fields_missing_reviewed.json`.
