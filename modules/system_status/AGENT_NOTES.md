...
- **Refresh:** toolbar POST (`refresh_cache` + CSRF) runs `itm_system_status_refresh_all()` for **all** tabs; preserves active `?tab=` on redirect; shows **Last refreshed** from `updated_at` (`dd/mmm/yyyy HH:MM`) on the toolbar side opposite the Refresh control when `new_button_position` is `left`.
