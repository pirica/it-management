<?php
$crud_table = 'automation_rules';
$crud_title = 'Automation Rules';
...
if ($crud_table === 'automation_rules') {
    $uiColumns = array_values(array_filter($uiColumns, function ($col) {
        return !in_array((string)($col['Field'] ?? ''), ['conditions_json', 'actions_json'], true);
    }));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    foreach ($fieldColumns as $col) {
        $name = $col['Field'];
        if (in_array($name, ['last_run_at', 'deleted_by', 'deleted_at'], true)) {
            continue;
        }
        $isTinyInt = str_starts_with($col['Type'], 'tinyint(1)');
        if ($isTinyInt || $name === 'active' || $name === 'enabled') {
    ...
    if ($crud_table === 'automation_rules' && empty($errors)) {
        $ruleName = trim((string)($data['name'] ?? ''));
        if ($ruleName === '') {
            $errors[] = 'Rule name is required.';
        }
        $triggerSlug = trim((string)($data['trigger_slug'] ?? ''));
        $allowedTriggers = itm_automation_rules_trigger_slugs();
        if ($triggerSlug === '' || !in_array($triggerSlug, $allowedTriggers, true)) {
            $errors[] = 'Select a valid trigger.';
        }
        foreach (['conditions_json', 'actions_json'] as $jsonField) {
            $rawJson = trim((string)($data[$jsonField] ?? ''));
            if ($rawJson === '') {
                $data[$jsonField] = '[]';
                $sqlValues[$jsonField] = "'[]'";
                continue;
            ...
            if (!is_array($decoded)) {
                $errors[] = cr_humanize_field($jsonField) . ' must be valid JSON array.';
            }
        }
    }
...
<button type="submit" name="add_sample_data" value="1" class="btn btn-primary">Add sample data</button>
            ...
            <?php elseif (in_array($crud_action, ['create', 'edit'], true)): ?>
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    if ($company_id > 0) {
                        echo '<input type="hidden" name="company_id" value="' . (int)$company_id . '">';
                    }
                    $triggerSlugs = itm_automation_rules_trigger_slugs();
                    $enabledChecked = ((int)($data['enabled'] ?? 1) === 1);
                    ?>
                    <div class="form-group">
                        <label>Name</label>
                        <input type="text" name="name" value="<?php echo sanitize(cr_form_display_value($data['name'] ?? '')); ?>" required>
                    </div>
                    <div class="form-group">
                        <label>Trigger</label>
                        <select name="trigger_slug" required>
                            <option value="">-- Select --</option>
                            <?php foreach ($triggerSlugs as $slug): ?>
                                <option value="<?php echo sanitize($slug); ?>" <?php echo ((string)($data['trigger_slug'] ?? '') === $slug) ? 'selected' : ''; ?>><?php echo sanitize($slug); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Conditions JSON</label>
                        <textarea name="conditions_json" rows="6" placeholder='[{"field":"status_name","op":"equals","value":"Open"}]'><?php echo sanitize(cr_form_display_value($data['conditions_json'] ?? '[]')); ?></textarea>
                    </div>
                    <div class="form-group">
                        <label>Actions JSON</label>
                        <textarea name="actions_json" rows="6" placeholder='[{"type":"notify_employee","employee_id":1,"title":"New ticket","body":"A ticket was created."}]'><?php echo sanitize(cr_form_display_value($data['actions_json'] ?? '[]')); ?></textarea>
                    </div>
                    <div class="form-group">
                        <label>Enabled</label>
                        <label class="itm-checkbox-control">
                            <input type="checkbox" name="enabled" value="1" <?php echo $enabledChecked ? 'checked' : ''; ?>>
                            <span>Enabled <span class="itm-check-indicator" aria-hidden="true"><?php echo $enabledChecked ? '✅' : '❌'; ?></span></span>
                        </label>
                    </div>
            ...
            <?php elseif ($crud_action === 'view'): ?>
                <h1 title="View automation rule">🔎</h1>
                <div class="card">
                    <table>
                        <tbody>
                    ...
                    <?php
                    $ruleRuns = [];
                    $viewRuleId = (int)($data['id'] ?? 0);
                    if ($viewRuleId > 0 && $company_id > 0) {
                        $runsSql = 'SELECT id, status, message, ran_at FROM automation_rule_runs
                                    WHERE company_id = ' . (int)$company_id . ' AND rule_id = ' . $viewRuleId . ' AND deleted_at IS NULL
                                    ORDER BY ran_at DESC, id DESC LIMIT 50';
                        $runsRes = mysqli_query($conn, $runsSql);
                        while ($runsRes && ($runRow = mysqli_fetch_assoc($runsRes))) {
                            $ruleRuns[] = $runRow;
                        }
                    }
                    ?>
                    <?php if (!empty($ruleRuns)): ?>
                        ...
                        <table>
                            ...
                            <tbody>
                            <?php foreach ($ruleRuns as $runRow): ?>
                                <tr>
                                    <td><?php echo sanitize((string)($runRow['status'] ?? '')); ?></td>
                                    <td><?php echo sanitize((string)($runRow['message'] ?? '')); ?></td>
                                    <td><?php echo sanitize((string)($runRow['ran_at'] ?? '')); ?></td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
                        </table>
                    <?php endif; ?>
                    <p style="margin-top:16px;"><a href="index.php" class="btn" title="Back">🔙</a> <a class="btn btn-primary" href="edit.php?id=<?php echo (int)($data['id'] ?? 0); ?>" title="Edit">✏️</a></p>
                </div>
            <?php endif; ?>
        </div>
    </div>
