...
$uiColumns = array_values(array_filter($fieldColumns, function ($col) use ($hideCompanyIdTables) {
    ...
    if (in_array($fieldName, ['active', 'deleted_by', 'deleted_at', 'created_by', 'created_at', 'updated_by', 'updated_at'], true)) {
        return false;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    foreach ($fieldColumns as $col) {
        ...
        // Pre-populate standard metadata columns
        ...
        if ($isTinyInt || $name === 'active') {
            if ($name === 'active') {
                $data[$name] = isset($_POST[$name]) ? (int)$_POST[$name] : 1;
            } else {
                ...
                <form method="POST" class="form-grid" style="max-width:980px;">
                    ...
                    ?>
                        <?php if ($name === 'company_id'): ?>
                            ...
                            <?php continue; ?>
                        <?php endif; ?>
                        <?php if (in_array($name, ['active', 'deleted_by', 'deleted_at', 'created_by', 'updated_by'], true)): ?>
                            <?php
                                if ($crud_action === 'create') {
                                    if ($name === 'active' && $displayVal === '') { $displayVal = '1'; }
                        ...
                        <div class="form-group">
                            ...
                            <?php elseif ($name === 'color'): ?>
                                <div style="display:flex;gap:8px;align-items:center;">
                                    <input type="color" name="<?php echo sanitize($name); ?>" id="cat-color-picker" value="<?php echo sanitize($displayVal !== '' ? $displayVal : '#3b82f6'); ?>" style="width:50px;height:38px;padding:2px;">
                                    <input type="text" id="cat-color-text" value="<?php echo sanitize($displayVal !== '' ? $displayVal : '#3b82f6'); ?>" placeholder="#000000" style="flex:1;">
                                </div>
                                <script>
                                    document.getElementById('cat-color-picker').addEventListener('input', (e) => {
                                        document.getElementById('cat-color-text').value = e.target.value.toUpperCase();
                                    });
                                    document.getElementById('cat-color-text').addEventListener('input', (e) => {
                                        let val = e.target.value;
                                        if (/^#[0-9A-F]{6}$/i.test(val)) {
                                            document.getElementById('cat-color-picker').value = val;
                                        }
                                    });
                                </script>
