...
- Database triggers `trg_workstation_os_versions_audit_insert`, `trg_workstation_os_versions_audit_update`, `trg_workstation_os_versions_audit_delete` on `workstation_os_versions` in `db/03_triggers.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **equipment** (`workstation_os_version_id`). [Cursor-Valid]
...
### Safe SELECT
```php
$stmt = $conn->prepare("SELECT * FROM workstation_os_versions WHERE company_id = ? AND id = ?");
...
### Safe INSERT
```php
$stmt = $conn->prepare("INSERT INTO workstation_os_versions (company_id, name) VALUES (?, ?)");
