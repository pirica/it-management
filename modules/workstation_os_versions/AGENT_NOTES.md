...
- Database triggers `trg_workstation_os_versions_audit_insert`, `trg_workstation_os_versions_audit_update`, `trg_workstation_os_versions_audit_delete` on `workstation_os_versions` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
