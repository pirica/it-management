...
- Database triggers `trg_workstation_os_versions_audit_insert`, `trg_workstation_os_versions_audit_update`, `trg_workstation_os_versions_audit_delete` on `workstation_os_versions` in `database.sql` always write to `audit_logs` on INSERT/UPDATE/DELETE (unconditional DB triggers; not gated by `enable_audit_logs`).
...
- Referenced by **equipment** (`workstation_os_version_id`). [Valid]-[2026-07-15]
