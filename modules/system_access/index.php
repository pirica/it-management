...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_sample_data'])) {
    ...
    $existingRowsStmt = mysqli_prepare($conn, 'SELECT COUNT(*) AS total_rows FROM system_access WHERE company_id = ?');
    $existingRows = 0;
    if ($existingRowsStmt) {
    ...
    $seedError = '';
    $insertedRows = itm_seed_table_from_database_sql($conn, 'system_access', (int)$company_id, $seedError);
            ...
            <?php if ($totalRows > $perPage): ?>
                <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-top:12px;">
                    ...
                    <div style="display:flex;gap:6px;flex-wrap:wrap;">
                        <?php if ($page > 1): ?>
                            <a class="btn btn-sm" href="?<?php echo sanitize(sa_build_query(['search' => $searchRaw, 'sort' => $sort, 'dir' => $dir, 'page' => 1])); ?>" title="First page">⏮️</a>
                        ...
                        <?php if ($page < $totalPages): ?>
                            <a class="btn btn-sm" href="?<?php echo sanitize(sa_build_query(['search' => $searchRaw, 'sort' => $sort, 'dir' => $dir, 'page' => $page + 1])); ?>" title="Next page">▶️</a>
