...
- Database triggers `trg_rj45_speed_audit_insert`, `trg_rj45_speed_audit_update`, `trg_rj45_speed_audit_delete` on `rj45_speed` in `database.sql` write to `audit_logs` when `enable_audit_logs` is enabled.
