...
/**
 * RJ45 Speed Module - Index
 * 
 * Main list view for equipment statuses. Users can view, create, edit, and delete
 * records defined for the company.
 */

$crud_table = $crud_table ?? 'rj45_speed';
$crud_title = $crud_title ?? 'RJ45 Speed';
...
function cr_validate_numeric_value($rawValue, $column, $fieldName, &$normalizedValue, &$error) {
    ...
    if (preg_match('/^(tinyint|smallint|mediumint|int|bigint)\b/', $type, $match)) {
        ...
        if (isset($ranges[$typeName])) {
            $signedMin = $ranges[$typeName][0];
            $signedMax = $ranges[$typeName][1];
            $unsignedMin = $ranges[$typeName][2];
            $unsignedMax = $ranges[$typeName][3];
...
if ($crud_action === 'delete') {
    ...
    itm_require_crud_role_module_permission($conn, 'delete', 'rj45_speed');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($crud_action, ['create', 'edit'], true)) {
    ...
    itm_require_crud_role_module_permission($conn, $crud_action, 'rj45_speed');
    cr_require_valid_csrf_token();
