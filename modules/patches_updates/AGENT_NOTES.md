...
- **Product gaps & opportunities (list):** `index.php` renders **Engage with dashboard and calendar** — set **Due date** on open (non-closed status) rows; shared helper `includes/itm_patches_updates_integrations.php` feeds [Calendar](http://localhost/it-management/modules/calendar/index.php) and the **Patches due in 30 days** smart dashboard widget (`includes/itm_dashboard_widgets.php`). **Equipment view:** [modules/equipment/view.php](http://localhost/it-management/modules/equipment/view.php) shows the linked patch row (status, level, due date, CVE, severity) with view/edit links; create accepts `?equipment_id=` prefill. Regression: [verify_patches_updates_integrations.php?run=1](http://localhost/it-management/scripts/verify_patches_updates_integrations.php?run=1), [verify_equipment_view_related.php?run=1](http://localhost/it-management/scripts/verify_equipment_view_related.php?run=1).
