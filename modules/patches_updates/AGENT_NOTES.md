...
- **patches_updates** → depends on **employees** (`assigned_to_employee_id` assignee dropdown on create/edit; list filter + FK label search).
...
- **Assignee**: Optional `assigned_to_employee_id` → `employees` (tenant-scoped dropdown labeled **Assigned To** on create/edit; list filter; dashboard widget `patches_due_30d` counts rows assigned to the signed-in employee).
...
- **Standard flattened CRUD**: search across visible columns (`$displayFieldColumns` alias), sort (ASC/DESC ▲/▼), server-side pagination (`records_per_page`) with `title="◀️ Previous"` / `title="▶️ Next"` on page links, bulk delete/clear when `$totalRows >= $perPage` (`bulk-delete-selection.js` included in `index.php` HTML), Export Excel/PDF, Import Excel via `table-tools.js`.
...
- **Product gaps & opportunities (list):** `index.php` renders **Engage with dashboard and calendar** — set **Due date** and **Assigned To** on open (non-closed status) rows; shared helper `includes/itm_patches_updates_integrations.php` feeds [Calendar](http://localhost/it-management/modules/calendar/index.php) and the **My patches due in 30 days** smart dashboard widget (`includes/itm_dashboard_widgets.php`, scoped to `assigned_to_employee_id` = session employee). **Equipment view:** [modules/equipment/view.php](http://localhost/it-management/modules/equipment/view.php) shows the linked patch row (status, assignee, level, due date, CVE, severity) with view/edit links; create accepts `?equipment_id=` prefill. Regression: [verify_patches_updates_integrations.php?run=1](http://localhost/it-management/scripts/verify_patches_updates_integrations.php?run=1), [verify_equipment_view_related.php?run=1](http://localhost/it-management/scripts/verify_equipment_view_related.php?run=1).
