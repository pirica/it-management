...
itm_require_crud_role_module_permission($conn, 'create', 'patches_updates');
...
function cr_render_cell_value($table, $field, $value) {
    ...
    if (isset($GLOBALS['fkMap'][$field]) && (string)$value !== '') {
        ...
        if ($field === 'status_id') {
            ...
            if (function_exists('itm_crud_render_status_label_badge')) {
                return itm_crud_render_status_label_badge($statusLabel, (string)$color);
            }
            return sanitize($statusLabel);
        }
    ...
    // Why: Row active is soft-delete mirror only — never show on list/view for status-driven modules.
    if ($field === 'active') {
        return '';
    }
...
$uiColumns = array_values(array_filter($fieldColumns, function ($col) use ($hideCompanyIdTables) {
    ...
    // Why: Soft-delete mirror only — business Active/Inactive is status_id badges.
    if ($fieldName === 'active') {
        return false;
    }
...
$viewColumns = array_values(array_filter($fieldColumns, function ($col) {
    $fieldName = (string)($col['Field'] ?? '');
    if ($fieldName === 'company_id' || $fieldName === 'active') {
        return false;
    }
    return true;
}));
                ...
                <form method="POST" enctype="multipart/form-data" class="form-grid" style="max-width:980px;">
                    ...
                    ?>
                        ...
                        <?php elseif ($name === 'created_by' && ($GLOBALS['crud_table'] ?? '') === 'patches_updates'): ?>
                            <?php $createdByValue = $displayVal !== '' ? $displayVal : (string)(int)($_SESSION['employee_id'] ?? 0); ?>
