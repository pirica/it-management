...
- Including alerts without `end_datetime` — calendar query requires end date for multi-day span rendering.
- Fetching integrated sources when `company_module_access` has disabled the parent module — always gate with `has_module_access()`.
