...
- `rooms/select-rate.php` — **Step 2 of 4** Select a Rate (active `hotel_booking_portal_rate_plans` per hotel; posts `portal_rate_plan_id`; special requests, comments). Nightly **Best available rate** uses `itm_hotel_booking_portal_check_in_display_bar()` (room-type calendar override else `hotel_booking_room_type_base_prices`); stay totals add tax/breakfast. Seed room `id=1` is STD at **75.00** so it matches the hotel calendar “from” BAR.
