...
- **Manage booking:** `users/bookings.php` — last name + **reservation ID** (`hotel_bookings.id`) + **auth2** (12-character code with uppercase, lowercase, numbers, and symbols on new bookings; legacy 4-digit PINs still accepted). Verified by `itm_hotel_booking_fetch_for_guest_manage()`; after PIN match the portal emails a **6-digit OTP** (`itm_hotel_booking_portal_manage_otp_issue()` / `_verify()`) before showing or cancelling a stay. OTP mail uses the reservation **hotel name** as the transactional template header (no gear icon) and a **Manage my booking** footer link to tenant `hotel_booking_settings.urlmybooking`. Lookup, OTP, and cancel POSTs are throttled by **session and client IP** via `itm_hotel_booking_portal_manage_rate_limit_check()` / `_record()` (default 12 attempts / 15 minutes). The confirmation page shows the auth code and links "Manage my booking" to the company-configured `urlmybooking` URL with `target="_blank"`.
