...
- `rooms/room-single.php` — **Step 4 of 4** guest form: locked check-in/out and **locked occupancy** from checkout draft (POST guest counts are ignored when a draft exists so crafted occupancy cannot undercut the quoted total); saves `hotel_bookings.portal_rate_plan_id` from checkout draft; creates the row via `itm_hotel_booking_portal_insert_booking_locked()` (transaction + room `SELECT … FOR UPDATE` + overlap re-check); email (`filter_var`) and phone (E.164 `+` country code) validated server-side.
