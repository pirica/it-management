...
- `rooms/select-rate.php` — **Step 2 of 4** Select a Rate (active `hotel_booking_portal_rate_plans` per hotel; posts `portal_rate_plan_id`; special requests, comments). Guest-facing **/night** and stay totals are **tax-inclusive** (tourist tax for occupancy). Stay-total strikethrough respects `show_discount_strikethrough`. Rate cards use `itm_hotel_booking_portal_rate_plan_offer()` (DB merchandising fields override slug defaults; NR plan discount from `plan_discount_percent`). Free-cancel `{date}` uses plan override or `hotel_booking_settings.free_cancellation_days_before_check_in` (seed **5** for companies 1–5). BAR excl-tax still comes from `itm_hotel_booking_portal_check_in_display_bar()`. Seed room `id=1` is STD at **75.00**; calendar/From merchandising uses `itm_hotel_booking_portal_cheapest_rate_offer_for_hotel()` (skips breakfast add-on).
