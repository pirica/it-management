...
- `rooms/customize.php` — **Step 3 of 4**: main column upgrade card; **View room details** opens the same room-detail modal as Step 1 (no Quick compare link); right column stacks stepper then **Reservation summary** (lists every `room_lines` entry when `rooms>1`; tourist tax €2/guest/night from settings).
