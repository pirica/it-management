...
- `rooms/room-single.php` — **Step 4 of 4** guest details; read-only **Special requests** block from session draft (pet, service animal, additional comments); uses draft for fees and `hotel_bookings.notes`.
- `rooms/payment.php` — confirmation after step 4: portal header/stay bar, success card (confirmation #, guest, dates, total), stepper complete + aside summary; online payment not enabled (pay at hotel copy). **Save booking confirmation** opens `rooms/confirmation-pdf.php?print=1` (print / Save as PDF).
