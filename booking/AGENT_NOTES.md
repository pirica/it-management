...
- `rooms.php` — full **Select a Room** page (step 1, stay bar, room grid, hotel sidebar). **Rooms & guests** modal (`rooms`, `adults`, `children`, `babies` query params); nightly price via `itm_hotel_booking_portal_quote_nightly()`. **Room Filters** (type `filter_tags`) and **Special rates** (`hotel_booking_special_rates`). **View room details** opens two-column modal (`includes/portal_room_detail.php`). **Amenities** on the page and **Hotel amenities** in the room-detail modal use the same emoji + label row as the hotel detail modal on `index.php` (`hb_portal_render_amenities_scroll()` / `hb_portal_amenity_icon()` in `includes/portal_chrome.php`, mirrored in `hotel-booking-public.js` `amenityIcon()`).
