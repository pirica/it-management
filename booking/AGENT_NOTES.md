...
- `index.php` — hotel list (all active `hotel_booking_hotels` across tenants); each card **From** price is **tax-inclusive** (min BAR + tourist tax for 1 adult, per hotel `company_id` settings); card image has gallery **‹ ›** arrows and **`1 / N`** counter when multiple photos exist; **Details** opens the same gallery in the modal (From* also tax-inclusive)
...
- `rooms/select-rate.php` — **Step 2 of 4** Select a Rate (active `hotel_booking_portal_rate_plans` per hotel; posts `portal_rate_plan_id`; special requests, comments). Guest-facing **/night** and stay totals are **tax-inclusive** (tourist tax for occupancy). Rate cards use `itm_hotel_booking_portal_rate_plan_offer()` (DB merchandising fields override slug defaults; NR plan discount from `plan_discount_percent`). Free-cancel `{date}` uses plan override or `hotel_booking_settings.free_cancellation_days_before_check_in` (seed **5** for companies 1–5). BAR excl-tax still comes from `itm_hotel_booking_portal_check_in_display_bar()`. Seed room `id=1` is STD at **75.00**; calendar day `price` = BAR + tax.
...
- Calendar JSON (`booking/calendar.php`) — passes occupancy; `itm_hotel_booking_hotel_calendar_month()` returns `prices_include_tax` and tax-inclusive day `price` (plus `bar_excl_tax`).
- `rooms.php` list cards — PHP quotes tax-inclusive; `js/hotel-booking-select-room.js` `updatePrices()` must add the same tourist tax (`HB_SELECT_ROOM.touristTaxPerPersonPerNight`) or it overwrites server HTML back to BAR-only.
