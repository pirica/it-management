...
- `rooms.php` — **Step 1 of 4** Select a Room; list card **/night** uses cheapest plan discount (usually NR) stacked with special rates, tax-inclusive; strikethrough list = BAR+tax; `HB_SELECT_ROOM.cheapestPlanDiscountPercent` keeps JS `updatePrices()` aligned.
