...
- `rooms/payment.php` — confirmation after step 4: **Full name** (guest `customers.name`), **Number of nights** `(N night(s))`, **Guests** `👤 …` occupancy line, **Reservation notes**, jsPDF download (**Save booking confirmation** — same card as screen, not print preview). Multi-room stays use **one guest-facing confirmation number** (lowest `hotel_bookings.id` in the auth2 group) and list **Your rooms** with type + per-room amount; internal sibling rows are not shown as separate confirmation IDs. Occupancy stored in `hotel_bookings.notes` as `Occupancy: rooms=…` meta line. Confirmation PDF is a rasterized card (html2canvas) plus a **clickable PDF `/URI` annotation** over the inline **Manage my booking** hint (`data-hb-pdf-manage-link`, `hotel-booking-confirmation-pdf.js` → `hbPdfAddManageBookingLink`); action buttons stay `.hb-pdf-exclude`.
