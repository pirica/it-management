...
- `rooms/room-single.php` — **Step 4 of 4** guest form: **requires an active checkout draft** with `occupancy` (POST without draft or without draft occupancy is rejected); locked check-in/out and occupancy from draft only; **charge re-resolves BAR, special-rate discount, and plan discount/surcharge from DB** via `itm_hotel_booking_portal_resolve_step4_charge()` (session draft money fields are not trusted at INSERT); saves `hotel_bookings.portal_rate_plan_id` from resolved plan; creates one or more rows via `itm_hotel_booking_portal_insert_stay_bookings_locked()` (shared `auth2`, split payment across `room_lines`; transaction + per-line room `SELECT … FOR UPDATE`); new bookings get a 12-character **auth2** code; email (`filter_var`) and phone (E.164 `+` country code) validated server-side. Compact step-4 layout (`.hb-checkout-page-step4`): side-by-side dates, compact special-requests card (including staff disclaimer), main-column **Total due**, full sidebar stepper + reservation summary; terms checkbox + **Book and continue to payment** in fixed `.hb-checkout-sticky-cta` (`form="hb-guest-form"`) so legal copy and CTA stay visible without scrolling.
- `rooms/payment.php` — confirmation after step 4: **Full name** (guest `customers.name`), **Number of nights** `(N night(s))`, **Guests** `👤 …` occupancy line, **Reservation notes**, jsPDF download (**Save booking confirmation** — same card as screen, not print preview). Multi-room stays list **Additional rooms** confirmation numbers from `$_SESSION['hotel_booking_last_ids']`. Occupancy stored in `hotel_bookings.notes` as `Occupancy: rooms=…` meta line. Confirmation PDF is a rasterized card (html2canvas) plus a **clickable PDF `/URI` annotation** over the inline **Manage my booking** hint (`data-hb-pdf-manage-link`, `hotel-booking-confirmation-pdf.js` → `hbPdfAddManageBookingLink`); action buttons stay `.hb-pdf-exclude`.
