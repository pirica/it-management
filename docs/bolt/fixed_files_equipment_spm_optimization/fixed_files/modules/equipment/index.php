...
if ($enableSwitchPortManager) {
    // Why: Performance optimization. Fetch only ID and Name for the switcher list.
    // Full equipment details are fetched separately only for the selected switch.
...
foreach ($equipmentRows as $equipmentRow) {
    ...
    $eqTypeNameLower = strtolower(trim((string)($equipmentRow['equipment_type_name'] ?? '')));
    $isSwitchType = strpos($eqTypeNameLower, 'switch') !== false;
...
if ($hasSelectedSwitch && $showSwitchPortManager) {
    // Why: Fetch full equipment details only for the selected switch when Switch Port Manager is active.
                ...
                <table data-itm-db-import-endpoint="index.php">
                    ...
                    <?php if (!empty($equipmentRows)): ?>
                        <?php foreach ($equipmentRows as $row): ?>
                            <?php
                            $rowTypeNameLower = strtolower(trim((string)($row['equipment_type_name'] ?? '')));
                            $isSwitch = strpos($rowTypeNameLower, 'switch') !== false;
