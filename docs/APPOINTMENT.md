...
Comprehensive documentation for the employee self-service IT appointment scheduling system, weekly slot grid generation, multi-concurrency lock mechanisms, API actions, and administrative configuration.

---

## 1. Intent & Purpose

The **Appointment** subsystem (`modules/appointment/`) enables employees to schedule visits with the IT support desk. It streamlines support workflow by offering:
...
sequenceDiagram
    ...
    participant API as appointment/api.php (Backend)
...
The booking UI coordinates with the backend via `modules/appointment/api.php`. Rate limiting and an active user session are required.
...
| Path | Primary Role |
|---|---|
| `modules/appointment/index.php` | Main entry point for booking UI, list review, and delete forms. |
| `modules/appointment/api.php` | API endpoint for week-grid slots and schedule submission. |
| `modules/appointment/list_all.php` | Administrative wrapper routing to the all-bookings list view. |
| `modules/appointment/view.php` | Detail view showing assignment and confirmation flags. |
