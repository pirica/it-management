...
- **docs/report.md** — penetration-test findings ITM-PENTEST-001–022 (read-only static review); per-finding **Date updated** and file/line **Verification** blocks; regression: `php scripts/verify_pentest_report.php` (PHPUnit `--filter PentestReport`). ITM-PENTEST-004/005 documented only (no code fix); ITM-PENTEST-006 remediated (error reporting default off).
