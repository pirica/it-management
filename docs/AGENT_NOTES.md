...
- **docs/security_assessment_report.md** — defensive architecture and deployment hardening guide (attack surfaces, theoretical abuse paths, checklist with `[x]`/`[~]`/`[ ]` evidence). **Not** the live pentest register — use **`docs/report.md`** + `php scripts/verify_pentest_report.php` for ITM-PENTEST-001–022 status. Checklist last synced 2026-09-02.
- **docs/report.md** — penetration-test findings ITM-PENTEST-001–022; remediated **001–003, 006–008, 009–016**; documented posture **`[INFO]`** **004, 004-mitigation, 005**; regression: `php scripts/verify_pentest_report.php` (`[PASS]` / `[INFO]` / `[OPEN]` drift / `[FAIL]` — see report → Regression verifier output). QA browser auth: `php scripts/verify_script_localhost_maintenance_auth.php`.
