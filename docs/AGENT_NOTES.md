...
- **docs/todo.md** — security and platform hardening backlog migrated from retired wiki `TO-DO-3.md` (API key hashing, vault GCM, `escape_sql` cleanup, RBAC chokepoint, prod checklist, config split, PHPStan, PHPUnit growth). Distinct from pentest register (`docs/report.md`) and product gaps (`docs/PRODUCT_GAPS_AND_MISSING.md`). Re-validate item status against codebase when closing items.
