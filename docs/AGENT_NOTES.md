...
- **docs/fixed_files_vulnerability_rate_plans_rce/** and **docs/fixed_files_vulnerability_hotel_booking_portal_rate_plans/** — historical reference patches for cancellation-policy URL RCE; **not** safe full-file replacements for live `includes/itm_hotel_booking.php` (see each folder’s `AGENT_NOTES.md`).
