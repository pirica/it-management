...
- **docs/fixed_files_vulnerability_rate_plans_rce/** and **docs/fixed_files_vulnerability_hotel_booking_portal_rate_plans/** — reference snapshots of `includes/itm_hotel_booking.php` for the cancellation-policy URL RCE finding; keep snapshots synced with live (see each folder’s `AGENT_NOTES.md`). Do not leave stale copies that omit later helpers (e.g. **`auth2`**).
- **docs/database_fixed/** — notes that the RCE fix needed no DB schema change (`AGENT_NOTES.md`).
