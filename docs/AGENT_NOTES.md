...
- **docs/ENV.md** — canonical `.env` reference: database keys, **`ITM_DEV` / `APP_ENV`** deployment profile (local `ITM_DEV=1` + `APP_ENV=development`), relationship to per-employee `enable_all_error_reporting`, drift audit, Apache `SetEnv`. Templates: `.env.example`, `docs/examples/env.*.sample`.
- **docs/examples/env.development.sample** — copy-paste local `.env` template (`ITM_DEV=1`, `APP_ENV=development`, Dunebox DB defaults).
- **docs/examples/env.production.sample** — production-style `.env` template (`APP_ENV=production`, `ITM_APP_URL`, no `ITM_DEV`).
