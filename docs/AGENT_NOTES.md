...
- **docs/VERIFY.md** — static code-only security review register (findings tables with **Status** `OPEN`/`FIXED`/`INFO`; `.env` root `.htaccess` **FIXED** / `ITM-PENTEST-023`; per-user `display_errors` and CLI `bypass_login.php` **INFO**). Includes **Code verification audit (2026-09-03)** with per-finding **Verdict** tables. Distinct from live pentest register (`docs/report.md`) and backlog (`docs/todo.md`).
