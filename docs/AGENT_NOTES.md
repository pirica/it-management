...
- **docs/ENV.md** — canonical `.env` reference: database keys, **`ITM_DEV` / `APP_ENV`** deployment profile (local `ITM_DEV=1` + `APP_ENV=development`), relationship to per-employee `enable_all_error_reporting`, drift audit, Apache `SetEnv`. Templates: `.env.example`, `examples/env.*.sample` (repo root).
- **examples/** — copy-paste `.env` templates (`env.development.sample`, `env.production.sample`); see **`examples/AGENT_NOTES.md`**.
