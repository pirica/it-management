...
## 7. File Structure
- **VAULT.md** — vault/master-key workflows (create, unlock, change, forgotten-key remediation), optional TOTP 2FA (enable/disable, QR setup, enforcement on unlock and master-key change), client-side key generation, one-time display UI, and `itm_send_email()` notification contract (no secrets in mail).
