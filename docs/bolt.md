...
**Verify:** `php scripts/verify_metadata_column_cache.php` — cold batch expects schema `Questions` delta 1–2 (MySQL may count prepare+execute); warm repeat expects schema delta 0. Measurement uses a chained baseline and excludes the trailing `SHOW STATUS` query from each delta. Optional env: `ITM_META_CACHE_TABLE` (default `switch_ports`).
