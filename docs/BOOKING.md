...
**Portal display (admin):** Same settings screen — **Portal display** card: `portal_show_room_number_on_confirmation` (default off) prefixes assigned room numbers on guest summaries and confirmation; `portal_hide_upgrade_upsell_when_multi_room` (default on) hides Step 3 upgrade upsell when `occupancy.rooms > 1`; `portal_money_symbol` (`EUR`/`GBP`/`USD`) with suffix (default on, e.g. `69.50€`) or prefix style for all portal price rendering (`hb_portal_money_format*`, `hotel-booking-money.js`, `itm_hotel_booking_portal_public_settings_for_js()`).
...
**Portal date/time formats (admin):** `portal_date_format` (European DD/MM/YYYY default, US, ISO), `portal_time_format` (24h default / 12h), datetime display enable flags + `portal_datetime_format_default` (European `17/AUG/YYYY HH:mm` default). Portal PHP uses `hb_portal_format_date_display()` / `hb_portal_format_datetime_display()`; JS uses `hotel-booking-date-format.js` + `HB_SETTINGS` keys.
