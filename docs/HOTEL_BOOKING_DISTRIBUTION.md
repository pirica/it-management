...
Set `ITM_DIST_API_KEY` to a channel plain-text API key from [Distribution Channels](http://localhost/it-management/modules/hotel_booking_distribution_channels/index.php). Fresh database imports include deterministic demo keys per tenant — company 1 ITM Demo: `itm_hbd_seed_demo_c0101` (pattern `itm_hbd_seed_demo_c{company}{slot}`; slots 1–3 = `itm_demo` / `booking_com` / `opentravel`). Empty tenants can click **Add sample data** on the channel index to create the same trio via `itm_hotel_booking_distribution_seed_sample_data()`.

For modify/cancel, set `ITM_DIST_EXTERNAL_RESERVATION_ID` to an existing partner reservation id.
