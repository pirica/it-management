...
| `php scripts/list_date_display_formats.php` | **CLI/browser (Admin).** Per-module date display audit: **OK** = UK `dd/mm/yyyy` via `itm_format_date_display()` / `itm_format_cell_scalar_display()` / `itm_format_datetime_display()` or explicit `date('d/m/Y')`; **WARN** = raw ISO echo, `type="date"` / `datetime-local`, `date('Y-m-d')` on output lines, US/text `date()` patterns. Default WARN-only; `--module=` filter; `--all` includes OK rows. Detection only (no auto-fix). Examples: `php scripts/list_date_display_formats.php --module=tickets`; [list_date_display_formats.php?run=1](http://localhost/it-management/scripts/list_date_display_formats.php?run=1). Exit `1` when WARN rows exist. Lib: `scripts/lib/itm_module_date_format_display_audit.php`. |
