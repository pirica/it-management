...
| `php scripts/fast_create_acc.php` | Admin browser form + CLI `--seed-demo-bundle` to create/update demo employees (`lib/itm_demo_module_users_seed.php`): FK selects (company, **role**, employment status, access level, multi-select **departments** `department_ids[]`, position — all with ➕ quick-add where supported) and **multi-select Modules** (`module_slugs[]`), RBAC row per module, `employee_departments` sync, sidebar prefs, `ui_configuration` |
| `php scripts/check_fast_create_acc_select_quick_add.php` | Static audit: FK `<select>` elements in `fast_create_acc.php` include `__add_new__` ➕ (exempt navigation/preset selects) |
