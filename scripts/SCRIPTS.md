...
**Company column (`company_id`) inventory:** after changing `$hideCompanyIdTables`, flattened list column filters, or bespoke modules that may show tenant company on list tables, run `php scripts/check_company_id_ui_column.php`. Static scan of `modules/*/index.php` + `db/01_schema.sql` (no database). Default exit `0` (report). `--list-exposed` prints exposed slugs only; `--strict` exits `1` when scaffold modules omit their table from `$hideCompanyIdTables`. Lib: `scripts/lib/itm_company_id_ui_column_audit.php`.
