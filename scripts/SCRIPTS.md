...
| `php scripts/apply_budget_categories_category_kind.php` | **CLI / browser (Admin).** Backfill `budget_categories.category_kind` from canonical category names (Revenue / Operating Expense / Capital Expense) and insert missing GL **7100** annual + January monthly budgets when Infrastructure cost center exists. Dry-run default; `--apply` or [apply_budget_categories_category_kind.php?run=1&apply=1](http://localhost/it-management/scripts/apply_budget_categories_category_kind.php?run=1&apply=1). Reports also auto-backfill kinds on page load via `itm_budget_category_report_bootstrap.php`. |
| `php scripts/verify_capex_opex.php` | **CLI / browser.** Regression for `modules/capex/` and `modules/opex/`: `budget_categories.category_kind` column and seeds, CAPEX vs OPEX GL isolation (7100 vs 6100/6200), report SQL in full-year and month modes, `default_year` from latest `annual_budgets`. Run when changing `includes/itm_budget_category_report.php` or category kind seeds. |
