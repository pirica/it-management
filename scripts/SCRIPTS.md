...
| `php scripts/fast_create_acc.php` | Admin browser form + CLI `--seed-demo-bundle` to create/update demo employees (`lib/itm_demo_module_users_seed.php`): FK selects (company, **role** with ➕ quick-add, status, access level, multi-select **departments** `department_ids[]` with ➕ quick-add, position) and **multi-select Modules** (`module_slugs[]`), RBAC row per module, `employee_departments` sync, sidebar prefs, `ui_configuration` |
