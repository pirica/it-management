...
| `php scripts/migrate.php --status` | Lists `db/migrations/*.sql` still needing SQL vs `schema_migrations` (checksum drift fails). Schema already satisfied by fresh `db/` import shows Applied even when unrecorded. Browser + CLI (Admin for apply). |
| `php scripts/migrate.php --apply` | Runs only true pending migrations in filename order; records schema-satisfied files without re-executing destructive SQL. Stamps `filename` + SHA-256 `checksum`. Helpers: `includes/itm_database_migrations.php`. |
