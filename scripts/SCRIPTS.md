...
- **No-auth browser scripts:** define `ITM_SCRIPT_NO_AUTH` before `config.php` only for read-only aggregate diagnostics allowlisted in `config/config.php` (`$itmNoAuthScripts`). Currently: `count_db_tables.php`, `test_chatbot.php`, `openapi.php`. Browser access requires `itm_script_browser_no_auth_client_allowed()` — loopback, built-in hosts **`localhost`** + **`myhome.dynip.sapo.pt`**, optional comma-separated **`ITM_SCRIPT_NO_AUTH_ALLOWED_HOSTS`** / **`ITM_SCRIPT_NO_AUTH_ALLOWED_IPS`** (IPv4/IPv6 or IPv4 CIDR; built-in IP **`127.0.0.1`**), or valid **`ITM_MAINTENANCE_TOKEN`** (`?token=` / `X-ITM-Maintenance-Token`). Other clients get HTTP 403. CLI unchanged.
...
| `itm_script_no_auth_allowed_hosts_resolved()` | Built-in `localhost`, `myhome.dynip.sapo.pt` + `ITM_SCRIPT_NO_AUTH_ALLOWED_HOSTS` |
