...
| `fix_crud_record_share_view_buttons.php` | n/a | n/a | Scaffold `index.php` inline view blocks — insert share action buttons |
...
| `php scripts/fast_create_acc.php` | CLI `--seed-demo-bundle` only; browser redirects to `modules/employees/fast_create_acc.php` (Employees list 🚀) |
| `modules/employees/fast_create_acc.php` | Fast demo employee form for active company: modules multi-select, FK selects with ➕ quick-add, RBAC + `employee_departments` via `lib/itm_demo_module_users_seed.php` |
| `php scripts/check_fast_create_acc_select_quick_add.php` | Static audit: FK `<select>` in `modules/employees/fast_create_acc.php` includes `__add_new__` ➕ (exempt `module_slugs[]`) |
...
| `php scripts/fix_crud_record_share_view_buttons.php` | Scaffold `index.php` inline view blocks | Repairs share button markup when view needle pattern drifted (Browser + CLI; dry-run default; `--apply` / `?apply=1`) |
