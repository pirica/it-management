...
GitHub Actions (`.github/workflows/smoke.yml`) runs two active jobs (**smoke**, **database-import**). **tier2** and **phpunit** are defined but skipped (`if: false`).
...
| **tier2** *(disabled in CI)* | `php scripts/run_tier2_checks.php` | Tier 2 static `check_*` batch from `SCRIPTS_TEST_MATRIX.md` (no MySQL) — run locally |
| **phpunit** *(disabled in CI)* | `ITM_SKIP_DB_TESTS=1 php scripts/run_tests.php` | PHPUnit unit suite without live-database tests — run locally |
