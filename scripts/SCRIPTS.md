...
| `php scripts/verify_setup_wizard_database.php` | Regression: setup wizard step 3 database probe/create/reset — `needs_create` for missing schema, `needs_replace_confirm` when tables exist, SQL bundle rewrite for custom `DB_NAME`, full mysqli import with DELIMITER-aware `03_triggers.sql` trigger count (`setup/includes/itm_setup_wizard.php`); requires MySQL |
...
Run `verify_setup_wizard_database.php` when changing setup wizard step 3 database probe, create, replace confirmation, or import guards (`setup/index.php`, `setup/includes/itm_setup_wizard.php`).
