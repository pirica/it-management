...
Use this registry when a **gate-excluded** module (listed in `ui_configuration_excluded_modules.txt` or matching an excluded prefix) intentionally prints `[n/a][pass]`, `[n/a][fail]`, or `[n/a][n/a]` for a UI configuration check and the team has reviewed the gap. Matching lines add **`[reviewed]`** (for example `[n/a][n/a][reviewed]`). Reviewed `[n/a][fail]` lines are omitted from the end-of-run **Gate-excluded modules with unreviewed contract failures** footer; per-line output still shows them with `[reviewed]`. Exit code is unchanged — gate-excluded lines remain informational.
