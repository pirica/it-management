...
| `php scripts/repro_attempts_data_leak_v2.php` | Regression — password-like login identifiers are redacted before `attempts.email` persistence. Disposable secret per run; verifies only the row inserted by this request (immune to stale verbatim rows). Browser + CLI. |
