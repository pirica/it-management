...
| Script | Purpose |
|--------|---------|
| `php scripts/count_db_tables.php` | Counts live tables in `information_schema` for `itmanagement`, echoes the total as plain text, and overwrites `scripts/number_db_tables.txt`. Fresh `db/` import: **205** tables (must match `CREATE TABLE` count in `db/01_schema.sql`). Browser and CLI; **no login** (exempt from `run=1` usage landing — see `itm_script_browser_usage_exempt_basenames()`). |
...
- **Plain output:** tag-free per-scope reference gap lines — `{module}.{column}: schema column not referenced in index|create|edit|view|list_all|includes PHP` — when the column name is not a quoted literal **and** not covered by that scope’s dynamic `$uiColumns` / `$viewColumns` / `$formUiColumns` loop (thin wrappers inherit `index.php` for create/edit/view/list_all). **No line** when the column is intentionally omitted from that scope (`$*ListHidden`, audit list/form hiders, hidden `company_id`). **`includes`** scope is skipped when the column is already referenced in index/create/edit/view/list_all. **`create` / `edit`** scopes are skipped when `modules/{slug}/create.php` or `edit.php` is absent (read-only modules such as `attempts`). Read-only modules (no `create.php` and no `edit.php`) also skip view/edit parity checks. Omits `includes` scope when `modules/{slug}/includes/` is absent. Browser links module slug to `../modules/{slug}/`; CLI gap lines use `{slug}.{column}` and print the module folder URL on its own line when `--module=` is set.
