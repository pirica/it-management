...
| **tier2** | `php scripts/run_tier2_checks.php` | Tier 2 static batch from `SCRIPTS_TEST_MATRIX.md` (`check_*` + `list_raw_columns.php` + `list_date_display_formats.php`; no MySQL) |
...
Browser menu: `scripts/run_tier2_checks.php` → **Run Tier 2 batch**. Optional `PHP_BIN=/path/to/php.exe` (Windows Laragon). Parses `SCRIPTS_TEST_MATRIX.md` for Tier 2 rows (any `*.php`, including `list_raw_columns.php` and `list_date_display_formats.php`); falls back to a built-in list when the matrix is missing. Does not mutate the database.
