...
| `php scripts/migrate.php --status` | Lists pending `db/migrations/*.sql` files vs `schema_migrations` (checksum drift fails). Browser + CLI (Admin for apply). |
| `php scripts/migrate.php --apply` | Runs pending migrations in filename order (one MySQL session); records `filename` + SHA-256 `checksum`. Destructive DROP+CREATE files — back up first. Helpers: `includes/itm_database_migrations.php`. |
...
- `php scripts/migrate.php --status` / `--apply` — `db/migrations/*.sql` runner + `schema_migrations` history (`includes/itm_database_migrations.php`)
