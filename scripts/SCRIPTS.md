...
- **No-auth browser scripts:** define `ITM_SCRIPT_NO_AUTH` before `config.php` only for read-only aggregate diagnostics allowlisted in `config/config.php` (`$itmNoAuthScripts`). Currently: `count_db_tables.php`, `test_chatbot.php`, `openapi.php`. Browser access requires `itm_script_browser_no_auth_client_allowed()` — loopback, built-in hosts **`localhost`**, **`127.0.0.1`**, **`myhome.dynip.sapo.pt`**, optional comma-separated **`ITM_SCRIPT_NO_AUTH_ALLOWED_HOSTS`** / **`ITM_SCRIPT_NO_AUTH_ALLOWED_IPS`** (IPv4/IPv6 or IPv4 CIDR), or valid **`ITM_MAINTENANCE_TOKEN`** (`?token=` / `X-ITM-Maintenance-Token`). Other clients get HTTP 403. CLI unchanged.

### Database table count (`count_db_tables.php`)

| Script | Purpose |
|--------|---------|
| `php scripts/count_db_tables.php` | Counts live tables in `information_schema` for `itmanagement` and echoes the total as plain text (browser or CLI only — no file write). Fresh `db/` import: **248** tables (must match `CREATE TABLE` count in `db/01_schema.sql`; [count_db_tables.php](http://localhost/it-management/scripts/count_db_tables.php) / [verify_database_schema.php?run=1](http://localhost/it-management/scripts/verify_database_schema.php?run=1)). Browser **no login** from loopback or allowlisted IP (see `ITM_SCRIPT_NO_AUTH_ALLOWED_IPS` in `.env.example`; exempt from `run=1` usage landing — see `itm_script_browser_usage_exempt_basenames()`). |
...
| `php scripts/perform_audit.php` | **Exploratory** subprocess audit of Tier 1–3 `scripts/*.php` (excludes Tier 4 MBQA, Tier 5 maintenance, `repro_*`, `verify_*`, `_tmp_*`, `health.php`, and session-mock harnesses `test_ajax.php` / `test_edit.php`). Uses Laragon `php.exe`, `putenv()` DB vars, truncates `docs/error_log.txt`, per-script log deltas, and PHP-specific stdout filters. Report: `scripts/php_error_audit_results.json` (`exit_code`, `cli_errors`, `stdout_hits`, `summary`). Intentional exit-only failures: `scripts/data/perform_audit_allowlist.json`. Exit `0` when no unallowlisted PHP errors or non-zero exits. Optional `--loop` prints top failures and exits `1` for agent triage. **Not a CI gate** — run Tier 1 checks and targeted `verify_*` / `repro_*` separately. |
...
| `php scripts/check_crud_rbac_coverage.php` | Static audit — in-scope `modules/*/index.php` with create/edit/delete handlers and standalone `create.php` / `edit.php` / `delete.php` must call `itm_crud_mutation_guard_entry()`, `itm_crud_enforce_mutation_access()`, or accepted alternate guards (`itm_require_crud_role_module_permission()`, `itm_require_role_module_permission()`, `itm_require_admin()`, `aps_require_permission()`). Skips index wrappers, create.php delegates, redirect stubs, and exempt slugs from `itm_crud_rbac_exempt_module_slugs()`. Exit `1` when missing. Browser: [check_crud_rbac_coverage.php?run=1](http://localhost/it-management/scripts/check_crud_rbac_coverage.php?run=1). |
| `php scripts/check_prod_hardening.php` | Pre-deploy production posture gate (not wired to CI smoke). Fails (exit `1`) when `APP_ENV=production` (default when unset) and seed admin/demo passwords still match import defaults, `ui_configuration.enable_all_error_reporting` is on or `display_errors` is enabled, legacy `error_log.txt` exists under `ROOT_PATH` (canonical log: `docs/error_log.txt`), dev bypass env flags (`ITM_SKIP_FORCE_PASSWORD_CHANGE`, `ITM_DEV`) are set, non-loopback `ITM_SCRIPT_NO_AUTH_*` overrides are present, or HTTP probe finds web-reachable `scripts/bypass_login.php`. Development profile prints `[WARN]` only unless `--enforce` / `?enforce=1`. Optional: `ITM_PROD_HARDENING_BASE_URL` for HTTP probe; `--skip-http` / `?skip_http=1` skips Apache probe. Lib: `lib/itm_prod_hardening_check.php`. Browser: [check_prod_hardening.php?run=1](http://localhost/it-management/scripts/check_prod_hardening.php?run=1) (Administrator). |
| `php scripts/apply_crud_rbac_guards.php` | **Browser + CLI.** Default dry-run; `--apply` / `?apply=1` (Admin). Inserts early `itm_crud_mutation_guard_entry()` on flattened `index.php` routers and strips duplicate per-handler `itm_require_crud_role_module_permission()` calls (idempotent). Browser: [apply_crud_rbac_guards.php?run=1](http://localhost/it-management/scripts/apply_crud_rbac_guards.php?run=1). |
| `php scripts/apply_crud_mutation_bootstrap.php` | **Browser + CLI.** Default dry-run; `--apply` / `?apply=1` (Admin). Inserts `itm_crud_mutation_guard_entry()` on standalone `create.php` / `edit.php` / `delete.php` (skips index wrappers and redirect stubs); strips legacy duplicate guards. Browser: [apply_crud_mutation_bootstrap.php?run=1](http://localhost/it-management/scripts/apply_crud_mutation_bootstrap.php?run=1). |
...
| `php scripts/verify_pentest_report.php` | Static regression for `docs/report.md` ITM-PENTEST-001–023. Output: `[PASS]` remediated (001–003, 006–008, 009–016, 023), `[INFO]` documented posture / positive controls (004, 004-mitigation, 005, 017–022), `[OPEN]` regression drift on formerly remediated findings, `[FAIL]` drift. Invokes `check_sql_injection_coverage.php` and `check_csrf_coverage.php`. PHPUnit: `--filter PentestReport`. Browser: [verify_pentest_report.php?run=1](http://localhost/it-management/scripts/verify_pentest_report.php?run=1) (Administrator). |
| `php scripts/verify_explorer_api_rate_limit.php` | ITM-PENTEST-015 regression: `modules/explorer/api.php` calls `itm_explorer_api_enforce_rate_limit_or_exit()`; rolling-hour cap blocks over-limit requests. Rolling-hour probe temporarily clears `.env` `ITM_EXPLORER_API_RATE_LIMIT_PER_HOUR` so the in-script cap of 3 applies (env otherwise overrides `ui_configuration`). Browser: [verify_explorer_api_rate_limit.php?run=1](http://localhost/it-management/scripts/verify_explorer_api_rate_limit.php?run=1) (Administrator). |
...
| `php scripts/verify_explorer_api_rate_limit.php` | ITM-PENTEST-015: confirms `modules/explorer/api.php` enforces per-employee hourly rate limit via `itm_explorer_api_enforce_rate_limit_or_exit()`; probes rolling-hour block after cap (temporarily clears `.env` `ITM_EXPLORER_API_RATE_LIMIT_PER_HOUR` for the probe). Browser: [verify_explorer_api_rate_limit.php?run=1](http://localhost/it-management/scripts/verify_explorer_api_rate_limit.php?run=1) (Administrator). |
...
**Cached schema consumers (non-scaffold):** `user-config.php` (System Access overview), `scripts/generate_tests.php` (batch PHPUnit scaffold), `scripts/module_browser_qa_runner.php` (`mbqa_table_column_names` / `mbqa_table_column_metas`) — all use `itm_crud_table_columns()` / `itm_crud_fk_map()` from `includes/itm_crud_schema_helpers.php`.
...
**Tier 2 batch (pre-merge static cluster):** run every Tier 2 script from `SCRIPTS_TEST_MATRIX.md` in one pass (`check_*` + `list_raw_columns.php` + `list_date_display_formats.php`):
...
Browser menu: [run_tier2_checks.php?run=1](http://localhost/it-management/scripts/run_tier2_checks.php?run=1) → **Run Tier 2 batch**. Optional `PHP_BIN=/path/to/php.exe` (Windows Laragon). Parses `SCRIPTS_TEST_MATRIX.md` for Tier 2 rows (any `*.php`, including `list_raw_columns.php` and `list_date_display_formats.php` with CLI `--include-inputs`); falls back to a built-in list when the matrix is missing. Does not mutate the database.
...
| `php scripts/apply_budget_categories_category_kind.php` | **CLI / browser (Admin).** Backfill `budget_categories.category_kind` from canonical names and insert CAPEX/OPEX demo rows (GL 6100/6200/7100 annual + January monthly budgets, one Posted 6100 expense) via `itm_budget_category_report_ensure_demo_sample_rows()`. Dry-run default; `--apply` (session company) or `--apply --all-companies` (seed tenants 1–5); browser [apply_budget_categories_category_kind.php?run=1&apply=1](http://localhost/it-management/scripts/apply_budget_categories_category_kind.php?run=1&apply=1) or `&all_companies=1`. CAPEX/OPEX empty states also offer **Add sample data** buttons (admin). |
| `php scripts/verify_capex_opex.php` | **CLI / browser.** Regression for `modules/capex/` and `modules/opex/`: `budget_categories.category_kind`, CAPEX vs OPEX GL isolation (7100 vs 6100/6200), report SQL in full-year and month modes, `default_year` from latest `annual_budgets`, and CAPEX/OPEX rows for seed companies **1–5**. Run when changing `includes/itm_budget_category_report.php` or `db/02_data.sql` finance seeds. [verify_capex_opex.php?run=1](http://localhost/it-management/scripts/verify_capex_opex.php?run=1) |
...
| `php scripts/list_date_display_formats.php` | **CLI/browser (Admin).** Per-module **display** date audit: **OK** = UK `dd/mmm/yyyy` helpers or `module_pass`; **WARN** = raw ISO list/view echo, bespoke `*_field_value()` helpers that return raw scalars without `itm_format_*`, or non-UK `date()` on output; **SKIP** (`module_skip`) = `reports`, `ops_report`, `ops_report_*`, `settings`, `backup_tape_log`, `birthdays`, `resignations`, `calendar`, `explorer`, `hotel*` (does not scan repo `booking/` guest portal). Native `type="date"` / `datetime-local` inputs skipped unless `--include-inputs` / `?include_inputs=1`. Sort: browser column headers (`?sort=module&dir=desc`) or CLI `--sort=module` `--dir=desc`. Default: WARN + `module_pass` + `module_skip` rows; `--module=` filter; `--all` line-level OK; `--no-show-pass` / `--no-show-skips`. Detection only. Examples: `php scripts/list_date_display_formats.php --module=equipment`; [list_date_display_formats.php?run=1](http://localhost/it-management/scripts/list_date_display_formats.php?run=1); inputs: [list_date_display_formats.php?run=1&include_inputs=1](http://localhost/it-management/scripts/list_date_display_formats.php?run=1&include_inputs=1). Exit `1` when WARN rows exist. **Tier 2:** `run_tier2_checks.php` runs CLI `--include-inputs` (same scope as [list_date_display_formats.php?run=1&include_inputs=1](http://localhost/it-management/scripts/list_date_display_formats.php?run=1&include_inputs=1)). Lib: `scripts/lib/itm_module_date_format_display_audit.php`. |
| `php scripts/apply_date_display_format.php` | **CLI/browser (Admin).** One-time/maintenance: patch flattened CRUD `cr_render_cell_value()` to call `itm_format_cell_scalar_display()` (UK list/view cells). **Default = dry-run**; writes with CLI `--apply` or browser `?apply=1`. Idempotent. Verify: `php scripts/list_date_display_formats.php`. |
| `php scripts/apply_crud_uk_date_inputs.php` | **CLI/browser (Admin).** One-time/maintenance: replace scaffold native `type="date"` / `datetime-local` inputs with `itm_render_uk_date_input()` / `itm_render_uk_datetime_input()` and insert `itm_crud_coerce_post_date_value()` in POST handlers. Skips audit-exempt modules (`reports`, `ops_report`, `settings`, `backup_tape_log`, `birthdays`, `resignations`, `calendar`, `explorer`, `hotel*`). **Default = dry-run**; writes with CLI `--apply` or browser `?apply=1`. Idempotent. Verify: `php scripts/list_date_display_formats.php --include-inputs`. Examples: [apply_crud_uk_date_inputs.php?run=1](http://localhost/it-management/scripts/apply_crud_uk_date_inputs.php?run=1). |
...
* `scripts/verify_tickets_sample_data.php` (empty-tenant tickets **Add sample data** — `TCK-0001`, active list visibility, no local employees, stale session `@app_company_id`; per-tenant seed `created_by` / `assigned_to` employee FK checks on `db/02_data.sql` `TCK-0001` rows; company-context probe for cross-tenant Admin at company 4 → tenant seed Admin4).
* `scripts/verify_tickets_create_defaults.php` (tickets create **Created By** default + `itm_ensure_company_context_employee_session()` stale-session remap at company 4 during `config.php` bootstrap; `itm_is_admin()` must live in `itm_company_session.php`; edit `TCK-0001` seed FK probe). CLI: [verify_tickets_create_defaults.php](http://localhost/it-management/scripts/verify_tickets_create_defaults.php) (CLI-only).
...
| 2 | **`error_log`** | Start scope: rename `docs/error_log.txt` to next `docs/error_log-N.txt` when present; else record byte offset (only *new* lines count for this module). |
...
* **`error_log`:** If `docs/error_log.txt` cannot be renamed (e.g. Windows file lock), the runner records the current file size and only attributes **new** lines to the active module — avoids false failures from earlier modules. When rotation succeeds, archives are `error_log-1.txt`, `error_log-2.txt`, … under `docs/` (same directory as `itm_error_log_file_path()`).
...
| [verify_employee_dashboard.php?run=1](http://localhost/it-management/scripts/verify_employee_dashboard.php?run=1) | Regression: employee **dashboard.php** hero + grouped stat cards, **Admin-only** company switcher (`Switch Company` / `itm_switch_active_company_session` from `itm_company_session_login_employee_id()`), Admin tenant remap load-order, smart widget wiring + Chart.js, `includes/itm_employee_dashboard.php` loader |
...
| `php scripts/verify_equipment_view_related.php` | Regression: equipment view **Patches & Updates** + **Related tickets** cards (`modules/equipment/view.php`, `includes/itm_patches_updates_integrations.php`, `includes/itm_equipment_view_related.php`); optional `ITM_TEST_COMPANY_ID` |
...
| `php scripts/verify_setup_wizard_project_root.php` | Regression: setup wizard collapsed Windows `project_root` repair, step 1 session root (no runtime fallback after step 1 complete), step 2 upload subdirectory paths, step 8 finish (`setup/index.php` deletion in target `z_folder` while keeping running `x_folder/setup/index.php` unblocked). MySQL host from `DB_HOST` (fallback `localhost`). Step 8 subprocess JSON is decoded from the last complete `{…}` object in stdout (not first-brace-to-last-brace). (`setup/includes/itm_setup_wizard.php`) |
| `php scripts/verify_setup_wizard_database.php` | Regression: setup wizard step 3 database probe/create/reset — `needs_create` for missing schema, `needs_replace_confirm` when tables exist, SQL bundle rewrite for custom `DB_NAME`, full mysqli import with DELIMITER-aware `03_triggers.sql` trigger count, step 7 skip minimal single-user cleanup (`itm_setup_wizard_apply_minimal_single_user_install`) after full bundle import, plus `itm_setup_wizard_install_sample_data_for_companies()` isolation (company 1 only vs company 3, then batch companies 1 and 3 — fail closed if the extra multi-company schema cannot be created/imported). CLI defines `ITM_CLI_SCRIPT` + `ITM_SETUP_WIZARD` then loads `config/config.php` because sample seed needs `itm_parse_database_sql_inserts()`. (`setup/includes/itm_setup_wizard.php`); requires MySQL |
...
Run `verify_setup_wizard_project_root.php` when changing setup wizard project-root repair, `itm_setup_wizard_project_root()`, step 2 file verification, step 1 session persistence, or Step 8 `itm_setup_wizard_remove_entrypoint()` (`setup/index.php`, `setup/includes/itm_setup_wizard.php`).

Run `verify_setup_wizard_database.php` when changing setup wizard step 3 database probe, create, replace confirmation, import guards, step 7 skip minimal single-user cleanup, or `itm_setup_wizard_install_sample_data_for_companies()` (`setup/index.php`, `setup/includes/itm_setup_wizard.php`).
...
Run [verify_employee_dashboard.php?run=1](http://localhost/it-management/scripts/verify_employee_dashboard.php?run=1) when changing employee `dashboard.php`, the Admin-only company switcher, company-switch identity remap, `includes/itm_employee_dashboard.php`, `includes/itm_employee_dashboard_cards.php`, or smart widget wiring on the landing dashboard.
...
Run `verify_equipment_view_related.php` when changing equipment view patch/ticket summary cards or `includes/itm_equipment_view_related.php`.
...
| Script | Purpose |
|--------|---------|
| `php scripts/verify_reports_hub.php` | Regression for `modules/reports/`: all `api/helpers.php` chart payloads (including CAPEX/OPEX split, budget vs actual, monthly actual), Hotel Operations MTD metrics (`ops_report` / `ops_report_fb_outlet`), budget vs actual / YoY totals, `modules_registry` slug `reports`, and core Chart.js canvas ids in `index.php`. Optional `ITM_TEST_COMPANY_ID` (default 1). Requires `db/` Reports Hub sample seeds (ops_report daily trend, F&B covers, expanded budgets/expenses). Browser + CLI. |
...
| `php scripts/verify_software_license_links.php` | Regression: `software_license_links` junction, `includes/itm_software_license_link.php` sync/list helpers (including `itm_software_license_list_equipment()` for the License Management Equipment tab), bidirectional software ↔ license_management UI linking. Browser: [verify_software_license_links.php?run=1](http://localhost/it-management/scripts/verify_software_license_links.php?run=1) (Admin session) |
...
- `php scripts/verify_pentest_report.php` — static regression for `docs/report.md` ITM-PENTEST-001–023 (`[PASS]` remediated 001–003, 006–008, 009–016, 023; `[INFO]` 004, 004-mitigation, 005, 017–022; `[OPEN]` regression drift; `[FAIL]` drift); invokes SQLi/CSRF static gates. PHPUnit: `--filter PentestReport`. Browser: [verify_pentest_report.php?run=1](http://localhost/it-management/scripts/verify_pentest_report.php?run=1) (Administrator).
- `php scripts/verify_explorer_api_rate_limit.php` — ITM-PENTEST-015: Explorer `api.php` per-employee hourly cap (`itm_explorer_api_enforce_rate_limit_or_exit`); rolling-hour probe clears `.env` override during the cap-of-3 check. Browser: [verify_explorer_api_rate_limit.php?run=1](http://localhost/it-management/scripts/verify_explorer_api_rate_limit.php?run=1) (Administrator).
