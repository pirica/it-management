...
| `php scripts/fast_create_acc.php` | Admin browser form + CLI `--seed-demo-bundle` to create/update demo employees (`lib/itm_demo_module_users_seed.php`): FK selects (company, **role** with ➕ quick-add, status, access level, department, position) and **multi-select Modules** (`module_slugs[]`), RBAC row per module, sidebar prefs, `ui_configuration` |
