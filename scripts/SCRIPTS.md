...
| `php scripts/migrate.php --status` | Probes **live database** for every `db/migrations/*.sql` file (Applied vs Pending). `schema_migrations` is audit/history only (checksum drift fails). Browser `?run=1` HTML table includes per-row **Open SQL** (`?run=1&sql={filename}`, new tab, plain text). Browser + CLI (Admin for apply). |
