...
- **Plain output:** tag-free per-scope reference gap lines — `{module}.{column}: schema column not referenced in index|create|edit|view|list_all|includes PHP` — when the column name is not a quoted literal **and** not covered by that scope’s dynamic `$uiColumns` / `$viewColumns` / `$formUiColumns` loop (thin wrappers inherit `index.php` for create/edit/view/list_all). Columns filtered out of a scope (`$*ListHidden`, audit list/form hiders, hidden `company_id`) may still report gaps. Omits `includes` scope when `modules/{slug}/includes/` is absent. Browser links module slug to `../modules/{slug}/`; CLI prints `http://localhost/it-management/modules/{slug}/` before `.{column}`.
