...
**Page chrome verify:** `php scripts/verify_module_page_chrome.php` — cross-checks `modules/{slug}/index.php`, `create.php`, `edit.php`, and `view.php` with a standalone `<head>` for canonical browser `<title>` (`titles_list.php` / `itm_titles_list_audit.php`) and Settings favicon wiring (`itm_check_module_favicon_link()`). Auxiliary paths (`list_all.php`, `delete.php`, export/import, partials, `join.php`, etc.) print `[SKIP]` — same scope as fields_missing bespoke page chrome, not `[FAIL]`. Exit `1` on in-scope title or favicon failure only.
