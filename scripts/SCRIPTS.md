...
| `php scripts/fields_missing.php` | **All-module** schema/UI audit: every discoverable module with a `database.sql` table compares live MySQL columns to canonical schema; flattened scaffold modules pass UI via dynamic `$uiColumns`; `employees` uses the critical-field matrix; bespoke modules (`docs/list_bespoke_UI.txt`) and other status-driven slugs (`equipment`, `patches_updates`, `tickets`) are **schema-only** when a table exists. Module folders whose slug matches a schema table but `$crud_table` points elsewhere (for example `company_module_access` → `modules_registry`) also run a companion schema pass on the folder-named table. Bespoke modules **without** a schema table (for example `calendar`, `settings`, `roles_permissions`, `is_*` equipment filters) still print a full per-module block with scraped UI form fields from `index.php` and extra entry files. Prints `database.sql` / live / UI-audited / excluded columns as comma-separated inline lists; **UI form fields** one per indented line; `[FAIL]` when schema or UI coverage gaps exist (including global meta columns visible on create/edit forms); **Failure summary** before `Result:` when any checks fail. Footer lists tables without a module folder. CLI `--module=<slug>` / `--json`; browser optional module filter. Exit `1` on schema or non-bespoke UI gaps. |
