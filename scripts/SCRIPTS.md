...
| `php scripts/pitfalls.php` | Aggregated pitfalls and developer traps extracted directly from the `AGENT_NOTES.md` files across all modules and submodules. Automatically creates/backfills missing note files. Supports Browser (Admin) and CLI with options like `-module=<slug>` and `--json`. |
