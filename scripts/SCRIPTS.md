...
- **No-auth browser scripts:** define `ITM_SCRIPT_NO_AUTH` before `config.php` only for read-only aggregate diagnostics allowlisted in `config/config.php` (`$itmNoAuthScripts`). Currently: `count_db_tables.php`, `test_chatbot.php`, `openapi.php`. Browser access requires `itm_script_browser_no_auth_client_allowed()` — loopback, comma-separated **`ITM_SCRIPT_NO_AUTH_ALLOWED_IPS`** (IPv4/IPv6 or IPv4 CIDR), or valid **`ITM_MAINTENANCE_TOKEN`** (`?token=` / `X-ITM-Maintenance-Token`). Other clients get HTTP 403. CLI unchanged.
...
| `itm_script_no_auth_allowed_ips_from_env()` | Parses `ITM_SCRIPT_NO_AUTH_ALLOWED_IPS` (comma-separated IPv4/IPv6 or IPv4 CIDR) |
| `itm_script_browser_no_auth_client_allowed()` | True on loopback, allowlisted IP, or valid `ITM_MAINTENANCE_TOKEN` / `X-ITM-Maintenance-Token` |
...
| `php scripts/verify_pentest_report.php` | Static regression for `docs/report.md` ITM-PENTEST-001–022. Output: `[PASS]` remediated (001–003, 006–007, 010, 012–014, 016), `[OPEN]` documented gaps (004–005, 008–011, 015), `[INFO]` positive controls (017–022), `[FAIL]` drift. Invokes `check_sql_injection_coverage.php` and `check_csrf_coverage.php`. PHPUnit: `--filter PentestReport`. Browser: [verify_pentest_report.php?run=1](http://localhost/it-management/scripts/verify_pentest_report.php?run=1) (Administrator). |
...
- `php scripts/verify_pentest_report.php` — static regression for `docs/report.md` ITM-PENTEST-001–022 (`[PASS]`/`[OPEN]`/`[INFO]`/`[FAIL]` labels; see report → Regression verifier output); invokes SQLi/CSRF static gates. PHPUnit: `--filter PentestReport`. Browser: [verify_pentest_report.php?run=1](http://localhost/it-management/scripts/verify_pentest_report.php?run=1) (Administrator).
