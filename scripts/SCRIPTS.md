...
**Seeded modules:** `attempts` — no `create.php` / `edit.php` (read-only security log). `audit_logs` — immutable read-only trail (`create.php` redirect; no edit/delete entry files). `backup_tape_log` — monthly grid bespoke UI (no `create.php`; Search, Sort, Pagination, bulk toolbar, and create/new-button checks reviewed).
