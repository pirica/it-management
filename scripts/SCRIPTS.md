...
| `php scripts/migrate.php --status` | Probes **live database** for every `db/migrations/*.sql` file (Applied vs Pending). `schema_migrations` is audit/history only (checksum drift fails). Browser + CLI (Admin for apply). |
