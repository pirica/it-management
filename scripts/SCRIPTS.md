...
| `php scripts/fields_missing.php` | **All-module** schema/UI audit: live MySQL vs `database.sql`; **dynamic scaffold** modules print **`[PASS] audited UI column`** per business field (create/edit, view, index); manual modules use legacy `UI covers {field}`; **bespoke** (`docs/list_bespoke_UI.txt`) and **status-driven** slugs run the same business-column checks tagged **`[SKIP][pass]`** / **`[SKIP][fail]`** (deferred — not scaffold `[FAIL]`) plus hidden-meta and scaffold-hybrid gate rules. Expect many `[SKIP][fail]` lines on custom UI modules; zero gaps is not the default. CLI `--module=<slug>` / `--json`. Exit `1` on any `[FAIL]` or `[SKIP][fail]`. |
...
Run `fields_missing.php` after changing `database.sql` table columns or module UI. Use `--module=<slug>` to narrow output. Bespoke modules report business-column gaps as `[SKIP][fail]` (deferred audit); many gaps are expected until custom UI is brought in line.
