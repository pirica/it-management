...
| `php scripts/scaffold_departments_child_table_modules.php` | One-time/maintenance: copy flattened CRUD from `modules/departments/` into child/support table modules (29-table allowlist). **Default = dry-run**; writes with CLI `--apply` or browser `?apply=1` (Admin). Run `php scripts/sync_modules_registry.php` after apply; add sidebar rows in `includes/ui_config.php` when introducing new slugs. |
