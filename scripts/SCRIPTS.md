...
| `php scripts/verify_setup_wizard_database.php` | Regression: setup wizard step 3 database probe/create/reset — `needs_create` for missing schema, `needs_replace_confirm` when tables exist, no raw `Unknown database` message (`setup/includes/itm_setup_wizard.php`); requires MySQL |
