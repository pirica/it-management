...
**Page chrome verify:** `php scripts/verify_module_page_chrome.php` — cross-checks `modules/{slug}/index.php`, `create.php`, `edit.php`, `view.php`, and standalone `list_all.php` / `view_all.php` with a `<head>` for canonical browser `<title>` (`titles_list.php` / `itm_titles_list_audit.php`) and Settings favicon wiring (`itm_check_module_favicon_link()`). Wrapper `list_all.php` / `view_all.php` files that only `require` another entry are skipped (no standalone `<head>`). Other auxiliary paths (`delete.php`, export/import, partials, `join.php`, etc.) print `[SKIP]` — not `[FAIL]`. Exit `1` on in-scope title or favicon failure only.
