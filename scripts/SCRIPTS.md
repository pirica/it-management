...
| `php scripts/titles_list.php` | Scans all PHP files under the `modules/` directory to extract their `<title>` tags. Prints a summary (scanned / with title / match vs not match) for the canonical app suffix `sanitize($app_name ?? itm_ui_config_app_name($currentUiConfig))`, then lists each file; non-matching rows are prefixed `[NOT MATCH]`. |
