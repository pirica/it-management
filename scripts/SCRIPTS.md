...
| `php scripts/list_empty_tables.php` | Lists tenant-scoped tables with zero live rows for the session company (browser Admin) or `--company=N` (CLI). Browser filter: `?company_id=N` (dropdown on page; legacy `?company=N` still accepted). Links to `modules/{table}/index.php` in a new tab when the module folder exists. JSON: `--json` / `?company_id=N&format=json`. |
...
- `php scripts/verify_qr_share_modules.php` — Passwords, Bookmarks, Todo, Events, Private Contacts, Explorer, Floor Plans, and Rack Planner temporary QR/code share sessions (`*_share_sessions`, `join.php`, module `*_share_helpers.php`, shared `includes/itm_qr_share.php`)
