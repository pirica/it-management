...
| `php scripts/fields_missing.php` | **All-module** schema/UI audit: every discoverable module with a `database.sql` table compares live MySQL columns to canonical schema; flattened scaffold modules pass UI via dynamic `$uiColumns`; `employees` uses the critical-field matrix; bespoke modules (`docs/list_bespoke_UI.txt`) and other status-driven slugs (`equipment`, `patches_updates`, `tickets`) are **schema-only** when a table exists (prints `UI coverage audit: skipped` — never a full-column `excluded from UI audit` list). Module folders whose slug matches a schema table but `$crud_table` points elsewhere (for example `company_module_access` → `modules_registry`) also run a companion schema pass on the folder-named table. Bespoke modules **without** a schema table (for example `calendar`, `settings`, `roles_permissions`, `is_*` equipment filters) still print a full per-module block with scraped UI form fields from `index.php` and extra entry files. Per-module sections: `database.sql` / live columns; optional skip line; **Static HTML name= scrape** (literal `name="..."` only); **Inferred form columns** when `$uiColumns` / `cr_manageable_columns` / employees matrix applies; **UI audited columns** / **excluded from UI audit** only when UI coverage ran; **UI form fields other** for non-table controls. CLI legend documents the same contract for every `ui:` mode. `[PASS]` per matching schema column, hidden meta column, and manual UI field coverage; `[FAIL]` when schema or UI coverage gaps exist; **Failure summary** before `Result:` when any checks fail. Footer lists tables without a module folder. CLI `--module=<slug>` / `--json`; browser optional module filter. Exit `1` on schema or non-bespoke UI gaps. |
