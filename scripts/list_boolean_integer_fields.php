 ...
 *
 * Why: Lists all database fields that can be Boolean, int, tinyint, and others,
 * matching tables to modules by table name and providing blank target links.
 *
 * Browser: open while logged in as Admin.
 * CLI: php scripts/list_boolean_integer_fields.php [--source=sql|db|both]
 */
...
<strong>Log in first.</strong> Open <a href="list_boolean_integer_fields.php">list_boolean_integer_fields.php</a>. CLI: <code>php scripts/list_boolean_integer_fields.php [--source=sql|db|both]</code>
    ...
    foreach ($matches as $match) {
        ...
        foreach ($lines as $line) {
            ...
            if (preg_match('/^`([a-zA-Z0-9_]+)`\s+(.*)$/i', $trimmedLine, $colMatch)) {
                ...
                if (preg_match('/\b(tinyint|int|bigint|mediumint|smallint|integer|boolean|bool)\b/i', $colDef)) {
                    $results[] = [
                        'table' => $tableName,
                        'column' => $colName,
...
function queryLiveDatabaseFields(mysqli $conn): array {
    ...
    while ($tableRow = $tablesRes->fetch_row()) {
        ...
        while ($col = $colsRes->fetch_assoc()) {
            ...
            if (preg_match('/\b(tinyint|int|bigint|mediumint|smallint|integer|boolean|bool)\b/i', $fieldType)) {
                $defParts = [];
                $defParts[] = "`" . $fieldName . "`";
                $defParts[] = $fieldType;
                if ($isNull === 'NO') {
...
if (!$isCli) {
    echo "<h2>List Boolean and Integer Fields</h2>";
    echo "<p class=\"report-muted\">Displays all boolean-like and integer fields across the database schema.</p>";
