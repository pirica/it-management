...
foreach ($modules as $module) {
    ...
    $checks = [
        ...
        'New button style' => itm_check_new_button_style($indexContent, is_file($createPath), $createContent),
    ...
    foreach (itm_ui_config_resolve_check_order($checks, $isGateExcluded) as $checkName) {
        ...
        if ($isGateExcluded) {
            ...
            if ($status === 'pass') {
                ...
                $failureLine = "{$checkName}: {$result['details']}";
                if ($isReviewed) {
                    $failureLine .= ' [reviewed]';
                }
                $moduleExcludedFailures[$module][] = $failureLine;
            } else {
                $totals['excluded_n/a']++;
            }
