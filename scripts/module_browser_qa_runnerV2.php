 ...
 *   php scripts/module_browser_qa_runnerV2.php
 *   php scripts/module_browser_qa_runnerV2.php --module=expenses --company=1
 *   php scripts/module_browser_qa_runnerV2.php --pilot-only
 *
 * Browser (Laragon): open scripts/module_browser_qa_runnerV2.php, set options, Run QA.
 */
...
function mbqa_append_ui_click_evidence(string $root, string $module, int $companyId, array $steps): array
{
    ...
    $xlsxBuilt = mbqar_build_runner_xlsx($root, $payload['results'], $pass, $fail, (string)($payload['generated_at'] ?? date('Y-m-d H:i:s')));
    ...
    return [
        ...
        'pass' => $pass,
        'fail' => $fail,
        'xlsx_href' => $xlsxBuilt['ok'] ? ('../qa-reports/' . mbqa_report_xlsx_basename()) : '',
    ];
}
...
function mbqa_echo_browser_url_actions_table(): void
{
    ...
    echo '<tr><td>CLI <code>php scripts/module_browser_qa_runnerV2.php</code></td><td>No <code>run</code> / <code>stream</code> / <code>ajax</code> — always CLI output</td></tr>';
    echo '</tbody></table>';
}
...
function mbqa_render_browser_ajax_required(): void
{
    ...
    echo '<p style="margin-top:20px;"><a href="module_browser_qa_runnerV2.php">← Runner form</a> · ';
    echo '<a href="module_browser_qa_runnerV2.php?help=1">Full help</a></p>';
    echo '</main>';
}
...
function mbqa_render_browser_help(): void
{
    ...
    itm_script_browser_nav_echo();
    echo '<main style="font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Helvetica,Arial,sans-serif;max-width:720px;margin:16px;line-height:1.5;">';
    echo '<h1>Module browser QA V2 — help</h1>';
    ...
    echo '<ol>';
    echo '<li>Open <a href="module_browser_qa_runnerV2.php">module_browser_qa_runnerV2.php</a> (the form, not this help URL alone).</li>';
    ...
    echo htmlspecialchars(
        ...
        . "php scripts/module_browser_qa_runnerV2.php --module=expenses --company=1\n"
        . "php scripts/module_browser_qa_runnerV2.php --pilot-only\n"
        . "php scripts/module_browser_qa_runnerV2.php --ui-click-smoke --module=expenses --company=1 # browser-only guard\n"
        . "php scripts/module_browser_qa_runnerV2.php --help\n",
        ENT_QUOTES,
        'UTF-8'
    );
    echo '</pre>';
...
function mbqa_print_help(): void
{
    ...
    itm_script_output_begin('Module browser QA runner V2');
    mbqa_out("Module browser QA runner V2\n\n");
    mbqa_out("Options:\n");
...
function mbqa_render_browser_form(array $options): void
{
    ...
    echo '<h1>Module browser QA runner V2</h1>';
    ...
    echo '<form id="mbqa-run-form" method="get" action="module_browser_qa_runnerV2.php" style="display:grid;gap:12px;max-width:520px;">';
...
(function () {
  ...
  function showFooter(done) {
    ...
    footerEl.innerHTML =
      ...
      '<a href="module_browser_qa_runnerV2.php">Run QA runner V2</a></p>';
  ...
  function postUiClickEvidence(runId, moduleSlug, companyId, steps) {
    return fetch('module_browser_qa_runnerV2.php?ajax=ui_click_evidence&run_id=' + encodeURIComponent(runId), {
      method: 'POST',
      credentials: 'same-origin',
      cache: 'no-store',
  ...
  function pollProgress(runId) {
    var url = 'module_browser_qa_runnerV2.php?ajax=progress&run_id=' + encodeURIComponent(runId);
  ...
  function requestCancel(runId) {
    var url = 'module_browser_qa_runnerV2.php?ajax=cancel&run_id=' + encodeURIComponent(runId);
...
function mbqa_validate_requested_scope(?string $module, ?int $company, bool $uiClickSmoke, array $allModules, array $companyNames): void
{
    ...
    if ($uiClickSmoke && mbqa_is_cli_sapi()) {
        mbqa_fail_preflight('--ui-click-smoke is browser-only; open scripts/module_browser_qa_runnerV2.php and select one module + one company.');
    }
...
$rerunHref = 'module_browser_qa_runnerV2.php?' . http_build_query($rerunParams);
