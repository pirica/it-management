 ...
 *   php scripts/tickets_related_equipment_delete_test.php
 *
 * Optional env:
...
function tradt_set_audit_context(mysqli $conn, int $companyId, int $employeeId = 1): void
{
    ...
    mysqli_query($conn, "SET @app_user_agent = 'tickets_related_equipment_delete_test'");
}
...
function tradt_run_static_checks(): void
{
    ...
    tradt_assert(function_exists('tickets_repair_sample_equipment_links'), 'tickets_repair_sample_equipment_links is available');
    ...
    tradt_assert(
        ...
        'equipment FK business-key remap includes serial_number for sample equipment_id'
    );
}
...
function tradt_run_db_integration(mysqli $conn): void
{
    ...
    $repaired = tickets_repair_sample_equipment_links($conn, $companyId);
    tradt_out('[INFO] tickets_repair_sample_equipment_links updated ' . (int)$repaired . ' row(s)');

    $linkRes = mysqli_query(
        $conn,
        "SELECT equipment_id FROM tickets WHERE company_id = " . (int)$companyId
        . " AND ticket_external_code = 'TCK-0001' LIMIT 1"
    );
    $linkRow = ($linkRes) ? mysqli_fetch_assoc($linkRes) : null;
    $linkedEquipmentId = is_array($linkRow) ? (int)($linkRow['equipment_id'] ?? 0) : 0;
    tradt_assert(
        $linkedEquipmentId === $equipmentId,
        'sample ticket TCK-0001 equipment_id links to Primary File Server (id ' . $equipmentId . ')'
    );
    ...
    if ($wrongEquipmentId > 0) {
        $wrongStmt = mysqli_prepare(
            $conn,
            "UPDATE tickets SET equipment_id = ? WHERE company_id = ? AND ticket_external_code = 'TCK-0001' LIMIT 1"
        );
        if ($wrongStmt) {
        ...
        $repairedWrong = tickets_repair_sample_equipment_links($conn, $companyId);
        tradt_assert($repairedWrong > 0, 'repair re-links sample ticket after stale wrong equipment_id');
        ...
        tradt_out('[INFO] skipped stale equipment_id repair case (no alternate equipment row)');
    }
    ...
    tradt_assert(
        stripos($deleteError, 'Related Equipment') !== false || stripos($deleteError, 'in use') !== false,
        'delete error mentions ticket Related Equipment / in use: ' . $deleteError
    );

    tradt_delete_sample_ticket($conn, $companyId);
    tradt_out(colorText('[PASS] removed temporary TCK-0001 ticket after assertions', 'pass'));
}
...
$failures = 0;
tradt_out('Tickets Related Equipment / equipment delete regression');
