...
- **ui_configuration_excluded_modules.txt** / **ui_configuration_excluded_prefixes.txt** — modules skipped by UI configuration coverage script. Bespoke read-only dashboards and visual tools (`reports`, `resignations`, `roles_permissions`, `company_module_access`, `org_chart`) are listed explicitly — they are in `docs/list_bespoke_UI.txt` and do not use the flattened CRUD list contract (search/sort/pagination/bulk on `index.php`).
