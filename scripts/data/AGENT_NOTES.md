...
- **ui_configuration_excluded_modules.txt** / **ui_configuration_excluded_prefixes.txt** — modules skipped by UI configuration coverage script. Keep in sync with bespoke slugs in `docs/list_bespoke_UI.txt` that do not use the flattened CRUD list contract (custom grids, dashboards, vault, file manager, designer tools, workflow screens). Scaffold modules that intentionally meet the contract (for example `cable_colors`, `vlans`) stay audited even when listed as bespoke for soft-delete rollout.
