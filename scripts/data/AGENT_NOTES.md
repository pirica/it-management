...
- **scripts-matrix-destroy-log.md** — append-only destroy→fresh-clone log for blanket `scripts/*` verification (`SCRIPTS_TEST_MATRIX.md` protocol). Restore via `verify_database_sql_import.sh` or `import_database_split.sh`.
- **php.ini.dunebox-7.4.template** — full `php.ini` beside Dunebox `php.exe` (`__EXT_DIR__` placeholder). Applied by `scripts/setup_dunebox_php_from_laragon.ps1`. Includes `extension=imap` for inbound email-to-ticket cron (`scripts/run_inbound_email_tickets.php`); ensure `php_imap.dll` is present under Dunebox `ext\`.
- **php.ini.dunebox-xdebug-snippet.ini** — Xdebug block merged into `D:\dunebox-v1.0.6\config\php\php-7.4.ini` for `php74.cmd`.
