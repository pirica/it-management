...
When a script wrecks the local `itmanagement` database or critical trees, add a dated entry **before** restoring from `database.sql`, then mark the matrix run status `DESTROYED_ENV`.
...
- **Symptom:** …
- **Exit / tail:** …
- **Restore:** fresh `database.sql` import via mysql CLI or `bash scripts/verify_database_sql_import.sh`
