...
if (!function_exists('itm_hotel_booking_distribution_demo_api_key')) {
    hbd_fail('seed helper itm_hotel_booking_distribution_demo_api_key missing');
} else {
    hbd_pass('seed helper loaded');
}

$expectedChannelCodes = ['itm_demo', 'booking_com', 'opentravel'];
for ($seedCompanyId = 1; $seedCompanyId <= 5; $seedCompanyId++) {
    $seedCount = itm_hotel_booking_distribution_count_channels($conn, $seedCompanyId);
    if ($seedCount < 3) {
        hbd_fail("company {$seedCompanyId} expects >=3 seeded channels, found {$seedCount} — re-import db/02_data.sql");
        continue;
    }
    $codeStmt = mysqli_prepare(
        $conn,
        'SELECT channel_code FROM hotel_booking_distribution_channels WHERE company_id = ? AND deleted_at IS NULL'
    );
    $foundCodes = [];
    if ($codeStmt) {
        mysqli_stmt_bind_param($codeStmt, 'i', $seedCompanyId);
        mysqli_stmt_execute($codeStmt);
        $codeRes = mysqli_stmt_get_result($codeStmt);
        while ($codeRes && ($codeRow = mysqli_fetch_assoc($codeRes))) {
            $foundCodes[] = (string) ($codeRow['channel_code'] ?? '');
        }
        mysqli_stmt_close($codeStmt);
    }
    $missingCodes = array_diff($expectedChannelCodes, $foundCodes);
    if (!empty($missingCodes)) {
        hbd_fail('company ' . $seedCompanyId . ' missing channel codes: ' . implode(', ', $missingCodes));
        continue;
    }
    hbd_pass("company {$seedCompanyId} has 3 seeded channel codes");

    $demoKey = itm_hotel_booking_distribution_demo_api_key($seedCompanyId, 1);
    $seedChannel = itm_hotel_booking_distribution_lookup_channel_by_api_key($conn, $demoKey);
    if ($seedChannel && ($seedChannel['channel_code'] ?? '') === 'itm_demo' && (int) ($seedChannel['company_id'] ?? 0) === $seedCompanyId) {
        hbd_pass("company {$seedCompanyId} demo API key authenticates itm_demo channel");
    } else {
        hbd_fail("company {$seedCompanyId} demo API key lookup failed for {$demoKey}");
    }

    $mapStmt = mysqli_prepare(
        $conn,
        'SELECT COUNT(*) AS cnt FROM hotel_booking_distribution_mappings m
         INNER JOIN hotel_booking_distribution_channels ch ON ch.id = m.channel_id AND ch.deleted_at IS NULL
         WHERE m.company_id = ? AND m.deleted_at IS NULL AND m.entity_type = \'room_type\''
    );
    $mapCount = 0;
    if ($mapStmt) {
        mysqli_stmt_bind_param($mapStmt, 'i', $seedCompanyId);
        mysqli_stmt_execute($mapStmt);
        $mapRes = mysqli_stmt_get_result($mapStmt);
        $mapRow = $mapRes ? mysqli_fetch_assoc($mapRes) : null;
        mysqli_stmt_close($mapStmt);
        $mapCount = (int) ($mapRow['cnt'] ?? 0);
    }
    if ($mapCount >= 4) {
        hbd_pass("company {$seedCompanyId} has room-type OTA mappings");
    } else {
        hbd_fail("company {$seedCompanyId} expects >=4 room-type mappings, found {$mapCount}");
    }

    $rpStmt = mysqli_prepare(
        $conn,
        'SELECT COUNT(*) AS cnt FROM hotel_booking_distribution_rate_plan_mappings rpm
         INNER JOIN hotel_booking_distribution_channels ch ON ch.id = rpm.channel_id AND ch.channel_code = \'booking_com\' AND ch.deleted_at IS NULL
         WHERE rpm.company_id = ? AND rpm.deleted_at IS NULL'
    );
    $rpCount = 0;
    if ($rpStmt) {
        mysqli_stmt_bind_param($rpStmt, 'i', $seedCompanyId);
        mysqli_stmt_execute($rpStmt);
        $rpRes = mysqli_stmt_get_result($rpStmt);
        $rpRow = $rpRes ? mysqli_fetch_assoc($rpRes) : null;
        mysqli_stmt_close($rpStmt);
        $rpCount = (int) ($rpRow['cnt'] ?? 0);
    }
    if ($rpCount >= 4) {
        hbd_pass("company {$seedCompanyId} has booking_com rate-plan mappings");
    } else {
        hbd_fail("company {$seedCompanyId} expects >=4 rate-plan mappings, found {$rpCount}");
    }
}

$seedSkipCompanyId = 1;
$beforeSeedCount = itm_hotel_booking_distribution_count_channels($conn, $seedSkipCompanyId);
$seedAgain = itm_hotel_booking_distribution_seed_sample_data($conn, $seedSkipCompanyId, 1);
if (!empty($seedAgain['skipped']) && $beforeSeedCount > 0) {
    hbd_pass('runtime seed skips when channels already exist');
} elseif ($beforeSeedCount === 0 && (int) ($seedAgain['channels_created'] ?? 0) === 3) {
    hbd_pass('runtime seed creates 3 channels on empty tenant');
} else {
    hbd_fail('runtime seed idempotency unexpected: ' . json_encode($seedAgain));
}
