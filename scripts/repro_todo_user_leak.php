...
<code>php scripts/repro_todo_user_leak.php</code>
...
itm_script_output_begin('Repro: Todo User Leak');

$root = dirname(__DIR__);
$nl = itm_script_output_nl();

echo "--- Repro: Todo User Leak ---" . $nl;
...
if (!isset($users)) {
    echo colorText("Error: \$users array not set by Todo module.", 'fail') . $nl;
} else {
    // 3. Check if leakUser is in the $users array
    if (isset($users[$leakId])) {
        echo colorText("BUG CONFIRMED: User from Company $company2Id is visible in Company 1 context.", 'fail') . $nl;
    } else {
        echo colorText("SUCCESS: User from other company is NOT visible. User list count: " . count($users), 'pass') . $nl;
    }
}
