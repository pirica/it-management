...
| `itm_ui_list_contract_checks.php` | Shared list/page UI contract helpers used by `check_ui_configuration_coverage.php` and `itm_fields_missing_report.php` bespoke gate (canonical source for Search/Sort/Pagination, MBQA-aligned Actions layout, bulk cancel, pagination titles, index POST CSRF, browser title, favicon via `itm_render_head_favicon_link($favicon_url)` in `<head>`) |
