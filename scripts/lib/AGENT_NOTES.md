...
| `itm_fields_missing_report.php` | Report builder for `fields_missing.php` and `employee_fields_missing.php` (comma-separated column/form lists; per-column `[PASS]` for live schema match, excluded meta hidden, and manual UI coverage; `ui_coverage_audit_skipped` clears fake excluded lists on bespoke/status-driven modules; static HTML scrape vs inferred form columns (`$uiColumns`, `cr_manageable_columns`, employees matrix); employees critical UI; dynamic scaffold; bespoke/schema-only skips; UI-only bespoke pass when no schema table; companion schema pass when module folder slug ≠ `$crud_table`; failure summary) |
