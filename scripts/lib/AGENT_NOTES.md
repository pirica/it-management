...
| `itm_fields_missing_report.php` | Report builder for `fields_missing.php` and `employee_fields_missing.php`: scaffold modules `[PASS]`/`[FAIL]`; bespoke/status-driven `[SKIP][pass]`/`[SKIP][fail]` gated checks (schema match, hidden meta via literal scrape + dynamic `foreach ($fieldColumns|$uiColumns)` loops — parses `cr_manageable_columns()` `in_array($c['Field']|$field, …)`, null-coalesce exclusions, `$hideCompanyIdTables` company_id filtering, bespoke `company_id`/`company_ids[]` branches); static scrape vs inferred form columns; employees critical UI; dynamic scaffold; UI-only bespoke when no schema table |
