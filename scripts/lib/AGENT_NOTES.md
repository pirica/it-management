...
| `itm_fields_missing_report.php` | Report builder for `fields_missing.php` and `employee_fields_missing.php`: dynamic scaffold modules print `[PASS] audited UI column` per business field; bespoke/status-driven modules run the same business-column checks as **`[SKIP][pass]`** / **`[SKIP][fail]`** (deferred audit) plus hidden-meta and scaffold-hybrid gate rules (see `docs/list_bespoke_UI.txt`) |
