...
| `itm_fields_missing_report.php` | Report builder for `fields_missing.php` and `employee_fields_missing.php`: dynamic scaffold modules print `[PASS] audited UI column` per business field (create/edit, view, index) plus `[PASS] excluded UI column` for hidden meta; resolves `require __DIR__ . '/index.php'` wrappers and `$formColumns` loops; manual modules keep legacy `UI covers {field}`; bespoke/status-driven `[SKIP][pass]`/`[SKIP][fail]` gated checks only (see `docs/list_bespoke_UI.txt`) |
