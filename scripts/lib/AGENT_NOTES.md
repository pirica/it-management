...
| `itm_fields_missing_report.php` | Report builder for `fields_missing.php` and `employee_fields_missing.php` (database.sql vs live columns; scrapes create/edit/index `name=` into `ui_form_fields` / `ui_form_fields_other` listed one field per line; employees critical UI; dynamic scaffold; bespoke/schema-only skips; UI-only bespoke pass when no schema table; companion schema pass when module folder slug ≠ `$crud_table`; `[PASS]`/`[FAIL]` per global meta column; failure summary) |
