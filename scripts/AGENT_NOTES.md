...
- **run_tests.php** — central test runner; browser menu (standard vs HTML coverage); resolves CLI `php.exe` via `itm_resolve_phpunit_cli_binary()` (not Apache `php-cgi`); detects Xdebug/PCOV on that binary; post-run link to `phpunit/coverage/html/coverage.html`. Dunebox one-time setup: `scripts/setup_dunebox_php_from_laragon.ps1` (copies Xdebug from Laragon portable into `D:\dunebox-v1.0.6`). Browser coverage URL: `run_tests.php?run=1&mode=coverage`. Full docs: **`scripts/SCRIPTS.md` → PHPUnit test runner**.
