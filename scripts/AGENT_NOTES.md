...
- **fast_create_acc.php** — Admin browser UI at `scripts/fast_create_acc.php` (catalog; no redirect) and `modules/employees/fast_create_acc.php` (🚀); shared **`modules/employees/fast_create_acc_browser.php`**. CLI `--seed-demo-bundle` via **`lib/itm_demo_module_users_seed.php`**.
