...
- **list_db_tables_without_modules.php** — Lists live `DB_NAME` tables with no `modules/{table}/index.php` and no `$crud_table` mapping (policy-hidden internal tables excluded). Shared builder: `scripts/lib/itm_database_tables_modules_report.php`. Browser Admin + CLI; JSON `--json` / `?format=json`; exit `1` when gaps exist. Full bidirectional audit remains **`compare_database_sql_modules.php`** (schema file).
