...
- **verify_db_migrations.php** — globs every `db/migrations/*.sql` and probes live schema/data (Applied / Superseded / Not applied / DML only); lib `itm_verify_db_migrations_report.php` parses `CREATE TABLE` / `CREATE TRIGGER` per file; `--json` / `?format=json`.
