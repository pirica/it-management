...
- **`scripts.php` catalog layout**: responsive card grid (not a wide scrollable table). Each catalog row is a card with script link + access badges (**Browser**, **CLI**, **Dry-run**, **CLI-only**), then **What it does** and **How to use** stacked with section labels. CSS classes: `scripts-catalog-grid`, `scripts-catalog`, `scripts-badge-dryrun`, `scripts-badge-cli-only`. Client-side filter: `#scripts-catalog-filter`. Styles are inline in `scripts.php` (catalog-only).
