...
- **verify_system_status.php** — `modules/system_status/` layout, `modules_registry` row, native API payloads, storage tree + active DB table reports, `information_schema`; Windows also checks `shell_exec`, `.ps1` readability, and `test_*.php` wrappers. Related: `system_status_api.php`, `system_status_phpinfo.php`, `take_screenshots_modules.py` (README `docs/readme/system_status.png`). PHPUnit: `SystemStatusApiTest`.
