...
- **verify_company_empty_sample_data.php** — CLI regression for tenant onboarding: reads `itm_list_empty_tables_collect_report()`, filters the 16-module **Add sample data** allowlist, runs `itm_seed_lookup_parents_for_table()` + `itm_seed_table_from_database_sql()` per empty table; `--company=N` required, optional `--module=slug`.
