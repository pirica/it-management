...
- **migrate.php** — filename-ordered migration runner for `db/migrations/*.sql` (`includes/itm_database_migrations.php`). CLI `php scripts/migrate.php --status` lists pending vs `schema_migrations` (checksum drift fails); schema already satisfied by fresh `db/` import shows **Applied** without a history row. `php scripts/migrate.php --apply` runs only true pending SQL; satisfied migrations are recorded without re-executing destructive files. Browser: `?run=1` status / `?run=1&apply=1` apply (Admin). Bootstrap file `schema_migrations.sql` is excluded from the runner loop (table ensured via `CREATE TABLE IF NOT EXISTS`).
