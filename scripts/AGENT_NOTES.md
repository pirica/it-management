...
- **verify_database_sql_import.sh** — full `database.sql` import + table-count assertion (derived from `CREATE TABLE` lines, currently 130); CI job **database-import** in `.github/workflows/smoke.yml` (also runs `verify_crud_fk_label_search.php`).
- **split_database_sql.php** / **verify_database_split_parity.php** / **import_database_split.sh** — generate and verify `database/01_schema.sql`, `database/03_data.sql`, `database/02_triggers.sql` from `database.sql`; import order 01 → 03 → 02 in one session. Shared lib: `lib/itm_database_sql_split.php`. See `database/README.md`.
