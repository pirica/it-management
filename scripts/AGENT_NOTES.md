...
- **`scripts.php` catalog layout**: responsive card grid. Sticky nav: **← Admin**, **← Dashboard**, **← Home**. Filter row: `?q=` search + **Search** + emoji-only **🔙** clear; empty section cards hide while filtering. Access badges: **Browser** + **CLI** (standard for `scripts/*.php` with a catalog link), or **CLI-only** (`scripts-badge-cli-only`) for bash/Python/bypass helpers. **Documentation** section lists `scripts/data/*.{json,txt,md}` and `scripts/*.md` with **Info** / **Markdown** tags (`scripts-badge-info` / `scripts-badge-md`). PHP rows get table tags only (Info/Markdown are not inferred from source references). Extension filter `*.json` / `*.txt` / `*.md` matches documentation file hrefs. Maintain documentation rows with `apply_script_catalog_documentation_files.php`; tags with `apply_script_catalog_tags.php` / `check_script_catalog_tags.php`; screenshots `verify_scripts_catalog_filter_screenshot.py` → `qa-reports/scripts_catalog_filter/`.
