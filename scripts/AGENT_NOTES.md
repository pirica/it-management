...
- **`scripts.php` catalog layout**: full-width dense **table** (not per-script cards) with `<colgroup>` column sizing: **Script** ~20rem `nowrap`, **Access** / **Tags** narrow, **What it does** takes remaining width; table `min-width: 52rem` with horizontal scroll in `.scripts-catalog-grid`. Sticky top nav + filter toolbar (`?q=` search, kind chips, table-tag `<select>`). **How to use** column hidden in CSS; non-PHP how text inlined under **What it does** (`.scripts-catalog-how-inline`). Long descriptions use `.scripts-cell-what-inner` (`max-height: 6.5rem`, `overflow-y: auto`). Sticky nav: **← Admin**, **← Dashboard**, **← Home**. **Main menu** (Scripts · Pitfalls · Rules) via `itm_script_browser_main_menu_echo()` in `scripts/lib/script_browser_nav.php` on `scripts.php`, `pitfalls.php`, and `rules.php`. Access badges: **Browser** + **CLI** (standard for `scripts/*.php` with a catalog link), or **CLI-only** (`scripts-badge-cli-only`) for bash/Python/bypass helpers. **PHP catalog rows** stub column 5 (`scripts-catalog-how-stub`); **How to use** is on each script’s browser landing (`itm_script_browser_how_to_use()`, dodgerblue info, `run=1` gate). **Documentation** section lists `scripts/data/*.{json,txt,md}` and `scripts/*.md` with **Info** / **Markdown** tags (`scripts-badge-info` / `scripts-badge-md`). PHP rows get table tags only (Info/Markdown are not inferred from source references). Extension filter `*.json` / `*.txt` / `*.md` matches documentation file hrefs. Maintain documentation rows with `apply_script_catalog_documentation_files.php`; tags with `apply_script_catalog_tags.php` / `check_script_catalog_tags.php`; usage migration with `apply_script_catalog_usage_to_php.php` / `check_script_browser_usage.php`; screenshots `verify_scripts_catalog_filter_screenshot.py` → `qa-reports/scripts_catalog_filter/`.
...
- **apply_script_catalog_usage_to_php.php** / **check_script_browser_usage.php** — move **How to use** from catalog into `itm_script_browser_how_to_use()` per `*.php` row; stub catalog column 5; gate browser-capable scripts (`scripts/lib/itm_script_browser_usage.php`, `scripts/lib/itm_script_catalog_usage_migrate.php`).
