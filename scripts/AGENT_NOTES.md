...
- **run_tier2_checks.php** — batch runner for all Tier 2 static scripts from `SCRIPTS_TEST_MATRIX.md` (`check_*` + `list_raw_columns.php`; parse or built-in fallback); CLI flags `--continue`, `--list`, `--only=`; browser menu with `?run=1`. Shared lib: `lib/itm_tier2_check_scripts.php`. Excluded from `perform_audit.php` subprocess scan.
