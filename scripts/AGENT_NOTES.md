...
- **list_empty_tables.php** — Lists tenant-scoped tables with zero live rows for session `company_id` (browser Admin) or `--company=N` (CLI); module links open `modules/{table}/index.php` in a new tab when the folder exists.
