...
- **migrate.php** — filename-ordered migration runner for `db/migrations/*.sql` (`includes/itm_database_migrations.php`). CLI `php scripts/migrate.php --status` lists pending vs `schema_migrations` (checksum drift fails); `php scripts/migrate.php --apply` runs pending SQL in one session and records SHA-256 checksums (destructive DROP+CREATE files — back up first). Browser: `?run=1` status / `?run=1&apply=1` apply (Admin). Bootstrap file `schema_migrations.sql` is excluded from the runner loop (table ensured via `CREATE TABLE IF NOT EXISTS`). Pair with **`verify_db_migrations.php`** for live schema probes.
