...
- **verify_database_sql_import.sh** — full `db/` import + table-count assertion (derived from `CREATE TABLE` lines in `db/01_schema.sql`, currently **126**); CI job **database-import** in `.github/workflows/smoke.yml` (also runs `verify_crud_fk_label_search.php`). Local split alternative: `import_database_split.sh` + `db/AGENT_NOTES.md`.
