...
- **Production Safety**: `debug.php` and other diagnostic tools should be removed or blocked in production. **`debug.php`** — browser requires **Administrator** session (`itm_script_require_admin_script_or_exit`); lists DB tables with total count; **File Permissions** scans every top-level repo folder with writable status and folder count; CLI defines `ITM_CLI_SCRIPT` before `config.php`.

## 5. UI Behavior Requirements
- **Browser vs CLI**: Many scripts provide both a plain-text/HTML browser view and a CLI output mode.
- **`scripts.php` catalog layout**: full-width dense **table** (not per-script cards) with `<colgroup>` column sizing: **Script** ~20rem `nowrap`, **Access** / **Tags** narrow, **What it does** takes remaining width; table `min-width: 52rem` with horizontal scroll in `.scripts-catalog-grid`. Each catalog row with a script link gets a **selection checkbox** in a dedicated narrow **pick** column (client-side: prepends `<col class="scripts-col-pick">`, header `th.scripts-pick-col`, and `td.scripts-cell-pick` with `.scripts-catalog-row-cb`; script name stays in the next `td.scripts-cell-script` with minimal gap). Hidden **How to use** column is `nth-child(6)` after the pick column is inserted. Sticky top nav + filter toolbar (`?q=` search, kind chips, table-tag `<select>`). **How to use** column hidden in CSS; non-PHP how text inlined under **What it does** (`.scripts-catalog-how-inline`). Long descriptions use `.scripts-cell-what-inner` (`max-height: 6.5rem`, `overflow-y: auto`). Sticky nav: **← Admin**, **← Dashboard**, **← Home**. Access badges: **Browser** + **CLI** (standard for `scripts/*.php` with a catalog link), or **CLI-only** (`scripts-badge-cli-only`) for bash/Python/bypass helpers. **PHP catalog rows** stub column 5 (`scripts-catalog-how-stub`); **How to use** is on each script’s browser landing (`itm_script_browser_how_to_use()`, dodgerblue info, `run=1` gate). **Documentation** section lists `scripts/data/*.{json,txt,md}` and `scripts/*.md` with **Info** / **Markdown** tags (`scripts-badge-info` / `scripts-badge-md`). PHP rows get table tags only (Info/Markdown are not inferred from source references). Extension filter `*.json` / `*.txt` / `*.md` matches documentation file hrefs. Maintain documentation rows with `apply_script_catalog_documentation_files.php`; tags with `apply_script_catalog_tags.php` / `check_script_catalog_tags.php`; usage migration with `apply_script_catalog_usage_to_php.php` / `check_script_browser_usage.php`; screenshots `verify_scripts_catalog_filter_screenshot.py` → `qa-reports/scripts_catalog_filter/`.
- **Table Line Breaking**: Audit/report pages such as `crud_tables.php`, `crud_titles.php`, and `crud_actions.php` keep `white-space: nowrap` on table cells with horizontal scroll — see each script’s inline CSS. The `scripts.php` catalog uses `nowrap` only on the **Script** column; **What it does** wraps inside the scrollable inner cell.
...
## 7. File Structure
- **smoke_test.sh** — main shell script for linting and security coverage (steps 1–4: syntax lint, CSRF, SQLi, FK label search static audit). **`check_csrf_coverage.php`** recognizes `itm_try_post_csrf()` (public auth in-page CSRF errors) alongside `itm_require_post_csrf()` / `itm_validate_csrf_token()`; skipped `includes/` hook libraries and `ITM_CLI_SCRIPT` repro scripts are informational only — not failures.
- **run_tier2_checks.php** — batch runner for all Tier 2 static scripts from `SCRIPTS_TEST_MATRIX.md` (`check_*` + `list_raw_columns.php` + `list_date_display_formats.php` with CLI `--include-inputs`; parse or built-in fallback); CLI flags `--continue`, `--list`, `--only=`; browser menu with `?run=1`. Shared lib: `lib/itm_tier2_check_scripts.php` (`itm_tier2_check_scripts_extra_cli_args()`). Excluded from `perform_audit.php` subprocess scan.
- **matrix_safe_run_once.php** — one-shot safe matrix runner (tiers 1-3) that compiles and runs the safe subset of the test matrix, writing results to `qa-reports/scripts-matrix-safe-run-raw.json`. Fully Browser + CLI compliant.																																																									  
...
- **verify_database_sql_import.sh** — full `db/` import + table-count assertion (derived from `CREATE TABLE` lines in `db/01_schema.sql` via `grep -c '^CREATE TABLE'` — not a hardcoded doc number); execs `import_database_split.sh`. CI job **database-import** in `.github/workflows/smoke.yml` sets **`MYSQL_PORT=3306`** (script default is **3307** for Dunebox); also runs `verify_crud_fk_label_search.php`. Local: same bash command; set `MYSQL_PORT` to match MySQL (see `db/AGENT_NOTES.md`).
- **import_database_split.sh** — imports `db/01_schema.sql`, `db/02_data.sql`, `db/03_triggers.sql` in one session (order 01 → 02 → 03). Env: `MYSQL_HOST`, `MYSQL_PORT` (default **3307**), `MYSQL_USER`, `MYSQL_PASSWORD`. See `db/AGENT_NOTES.md`.
- **check_env_vars_in_use.php** — static scan of `getenv()` / `$_ENV` / Python `os.environ` / shell `${VAR}` reads vs `.env.example`; compares live `.env` vs `.env.example` in a table (live column never shows real values — `(Not Empty)` / `(empty)` only). Shared lib `lib/itm_env_vars_audit.php`. Browser + CLI (Admin in browser); default informational; `--strict` exits `1` on example-only or undocumented app drift; `--json` machine output (`env_compare` included).
...
- **verify_setup_wizard_project_root.php** / **verify_setup_wizard_database.php** — CLI setup-wizard regression (project-root repair, Step 8 `setup/index.php` unlink, step 3 import, sample-data company isolation). Catalog + run notes: **`scripts/SCRIPTS.md`**. Browser landings: [verify_setup_wizard_project_root.php?run=1](http://localhost/it-management/scripts/verify_setup_wizard_project_root.php?run=1), [verify_setup_wizard_database.php?run=1](http://localhost/it-management/scripts/verify_setup_wizard_database.php?run=1) (no login; CLI SAPI preferred).
- **check_prod_hardening.php** — pre-deploy production posture gate (not wired to `.github/workflows/smoke.yml`). Lib: `lib/itm_prod_hardening_check.php`. Enforces failures when `APP_ENV=production` (default when unset); development prints `[WARN]` unless `--enforce` / `?enforce=1`. Probes: `ITM_SKIP_FORCE_PASSWORD_CHANGE` / `ITM_DEV`, non-loopback `ITM_SCRIPT_NO_AUTH_*`, legacy `ROOT_PATH/error_log.txt` (canonical log `docs/error_log.txt` via `itm_error_log_file_path()`), `ui_configuration.enable_all_error_reporting` + `display_errors`, canonical seed passwords on `Admin`–`Admin5` / `demo1`–`demo5`, HTTP `scripts/bypass_login.php` via `ITM_PROD_HARDENING_BASE_URL` or `BASE_URL` (`--skip-http` to skip). Browser: [check_prod_hardening.php?run=1](http://localhost/it-management/scripts/check_prod_hardening.php?run=1) (Administrator).
- **check_create_post_prepared_statements.php** — static inventory of `modules/*/create.php` POST save SQL: `escape_sql()`, flattened CRUD `$sqlValues` string INSERT/UPDATE, bespoke string saves vs `mysqli_prepare` / MySQLi OOP `$conn->prepare` + `bind_param`. Lib: `lib/itm_create_post_prepared_statement_audit.php`. Default collapses scaffold rows to counts; `--list-scaffold` / `--verbose` expand listings; `--json` for machine output. Default informational; `--strict` exits `1` when legacy paths remain. Complements `check_sql_injection_coverage.php` (which treats `mysqli_real_escape_string` as safe in proximity windows). Browser: [check_create_post_prepared_statements.php?run=1](http://localhost/it-management/scripts/check_create_post_prepared_statements.php?run=1) (Administrator).
- **verify_crud_fk_label_search.php** — MySQL regression for FK label list search (employees, license_management, switch_ports, todo, notes, private_contacts, ip_subnets, bookmarks); isolated HTML probes for todo/notes/bookmarks set `$_SESSION['vault_key']` (Admin master key hash) so list output is not replaced by the vault lock screen. CI **database-import** job after schema import.
...
- **apply_crud_audit_soft_delete.php** / **check_crud_audit_soft_delete.php** — scaffold soft-delete + audit meta UI rollout (`docs/list_soft-delete.txt`). Apply is **Browser + CLI**; default run is always dry-run; writes only with `--apply` (CLI) or `?apply=1` (browser, Admin); after the count summary prints inventory / skipped / missing / needing-patch / compliant module lists with real newlines (not browser `<br><br>`); skips status-driven modules; idempotent when already compliant. List gate accepts `itm_alerts_build_scoped_where_sql()` (alerts delegates `deleted_at IS NULL` via `includes/alerts_visibility.php`). See **`scripts/SCRIPTS.md` → Scaffold audit columns + soft-delete**.
- **apply_crud_sample_data_live_row_gate.php** / **check_crud_sample_data_live_row_gate.php** — **Add sample data** row templates live in **`db/02_data_sample.sql`** (`itm_seed_table_from_database_sql()`). These scripts only audit/patch the PHP empty-table gate (`itm_seed_tenant_row_count()` vs legacy `COUNT(*)`). Check is finance slugs only; apply can patch all matching legacy blocks (dry-run default). Modules that already use `itm_crud_append_not_deleted_predicate()` on the sample gate are unchanged.
- **All `scripts/apply*.php` maintenance tools** — shared contract via **`lib/itm_apply_script_bootstrap.php`**: Browser + CLI, dry-run default, `--apply` / `?apply=1` (Admin) to write, named target lists after the count summary. Catalog: `scripts/scripts.php`. **`apply_module_sample_data_seed.php`** — propagates sample `INSERT`s into `db/` for every seeded `company_id` (single-row and multi-row `VALUES` blocks); **mirror mode** detects `INSERT … SELECT N, … FROM table WHERE company_id = 1` and appends only to the source tenant VALUES (e.g. `knowledge_base`); browser errors via `itm_seed_fwrite_stderr()`; built-in help lists dry-run/apply URLs. **`apply_new_company_module_share_capable_seed.php`** — live DB backfill for `company_module_share` capable matrix (`lib/itm_apply_new_company_module_share_seed.php`); mirrors `db/migrations/company_module_share_capable_seed.sql`; uses `$boot['conn']` from `itm_apply_script_bootstrap()` only (no top-level `config.php` — duplicate load breaks browser Admin `?apply=1`); optional single-company scope. **`sample_bookings.php`** — seeds 10 planning-demo `hotel_bookings` per company 1–5 (guest names + lifecycle statuses vs today); notes `[sample_bookings]`; replaces prior sample rows on apply.
- **verify_audit_columns.php** — schema gate for mandatory audit columns including soft-delete fields on live tables (including `live_chat_typing`). `live_chat_messages` / `live_chat_typing` remain private-data exempt from `audit_logs` triggers.
- **verify_audit_logs_disclosure.php** — three-step employees audit disclosure regression (static `db/03_triggers.sql` trigger scan, live disposable UPDATE probe, retro scan); prints each step with `[PASS]`/`[FAIL]`.
- **run_tests.php** — central test runner; browser menu optional **PHPUnit filter** (`filter` query param → `--filter`); browser **HTML coverage** spawns a detached CLI job (`scripts/lib/itm_run_tests_browser_coverage.php`, log `qa-reports/run_tests_browser_coverage.log`, status `?coverage_job=1`) to avoid Apache gateway timeouts; PHPUnit subprocess uses **`-d memory_limit=512M`** (env **`ITM_PHPUNIT_MEMORY_LIMIT`**) for HTML report generation. **Standard** mode streams PHPUnit in-page. Resolves CLI `php.exe` via `itm_resolve_phpunit_cli_binary()`. Dunebox setup: `scripts/setup_dunebox_php_from_laragon.ps1`. Full docs: **`scripts/SCRIPTS.md` → PHPUnit test runner**.
...
- **apply_script_catalog_tags.php** / **check_script_catalog_tags.php** — compute and gate catalog table tags on `scripts/scripts.php` cards (`scripts/lib/itm_script_catalog_tags.php`, manifest `scripts/data/script_catalog_tags.json`). Apply is dry-run default; `--apply` / `?apply=1` patches `data-tags` + tag pills. Check exits `1` on drift. Re-run after catalog or script SQL changes.
- **apply_script_catalog_usage_to_php.php** / **check_script_browser_usage.php** — move **How to use** from catalog into `itm_script_browser_how_to_use()` per `*.php` row; stub catalog column 5; gate browser-capable scripts (`scripts/lib/itm_script_browser_usage.php`, `scripts/lib/itm_script_catalog_usage_migrate.php`). **`count_db_tables.php`** is exempt (no-auth plain-text probe; catalog **What it does** for login-free table totals only).
...
- **check_database_sql_company_name_uniques.php** — tenant UNIQUE audit over `db/` (via `includes/database_sql_unique_audit.php`). Skips intentional duplicate-name / junction tables including `bookmark_folders` and `floor_plan_item_tags`, private `events` and vault `todo` (title TEXT / encrypted — scope key is `(company_id, created_by, id)`), and ephemeral `*_share_sessions` tables (see `includes/itm_qr_share.php`, `modules/events/AGENT_NOTES.md`).
- **verify_select_options_escalation.php** — regression for Select Options API table whitelist (`includes/itm_select_options_policy.php`); see **`scripts/SCRIPTS.md` → Select Options API verification**. Isolated subprocess uses `itm_resolve_cli_php_binary()` so browser catalog runs do not spawn php-cgi (which returns `302` to `login.php` instead of whitelist JSON).
...
- **migrate.php** — filename-ordered migration runner for `db/migrations/*.sql` (`includes/itm_database_migrations.php`). Live DB probe authoritative; browser `?run=1` renders an HTML results table (Status, file, Message, verify/fix links, **Open SQL**, **Delete** 🗑️ with JS confirm — Admin removes file from disk). `--apply` / `?run=1&apply=1` (Admin).
- **verify_db_migrations.php** — globs every `db/migrations/*.sql` and probes live schema/data (Applied / Superseded / Not applied / DML only); schema probe only — applied history is **`migrate.php`** + **`schema_migrations`**. Lib `itm_verify_db_migrations_report.php` parses `CREATE TABLE` / `CREATE TRIGGER` per file; `--json` / `?format=json`.
...
- **verify_maintenance_scripts_rbac.php** — browser Admin gate on MBQA runner, compare_database_sql_modules, list_db_tables_without_modules, and test_sql_injection.
...
- **verify_demo_module_restrictions.php** — demo users `demo1`–`demo5` single-module access (`has_module_access`, RBAC `can_view`, subprocess `index.php` probes) and seed admin `password_verify` / `itm_is_admin()` checks; contract: **`lib/itm_demo_module_restrictions_contract.php`**.
- **verify_employee_contact_email.php** — regression for at least one of `work_email` / `personal_email` (`includes/itm_employee_contact_email.php`); employees create/edit/import and `fast_create_acc` email fields.
- **fast_create_acc.php** — Admin browser UI at `scripts/fast_create_acc.php` (catalog; no redirect), **`scripts/fast_create_acc_browser.php`** (alias to same UI), and `modules/employees/fast_create_acc.php` (🚀); shared **`modules/employees/fast_create_acc_browser.php`** (include only). CLI `--seed-demo-bundle` via **`lib/itm_demo_module_users_seed.php`**.
...
- **verify_request_password.php** — RBAC, env-backed HMAC approval links (`ITM_REQUEST_PASSWORD_APPROVAL_SECRET`), list contract markers, creator-only soft-delete. Catalog: **`scripts/scripts.php`**; PoC: `repro_request_password_bypass.php`.
...
- **verify_command_palette_search.php** / **verify_command_palette_sidebar_slugs.php** — global command-palette regression; sidebar slug audit via `scripts/lib/itm_command_palette_sidebar_verify.php`. Catalog: **`scripts/scripts.php`**.
...
- **repro_audit_token_leak.php** / **repro_rbac_bypass.php** / **repro_vulnerabilities.php** / **repro_esa_vulnerability.php** / **repro_auth_bypass_v3.php** / **repro_employee_companies_leak.php** / **repro_employee_companies_bac.php** / **repro_contacts_idor.php** / **repro_select_options.php** / **repro_status_leak.php** / **repro_visitors_bac.php** / **repro_visitors_sqli.php** / **repro_birthdays_resignations_rbac.php** — security repro scripts; subprocess spawns use `escapeshellarg()` and inherit `mysqli.default_socket` when set; **repro_rbac_bypass.php** seeds a disposable expense with tenant-scoped `paid_status_id`, `gl_account_id`, `posting_date`, and unique `invoice_number`, uses Laragon CLI `php.exe` (not `php-cgi`), restores session before `config.php`, and falls back to permission-helper + row retention when subprocess auth redirects to login; audit token repro uses prepared statements for disposable test-user `reset_token` updates. **repro_contacts_idor.php** uses shared disposable employees and clears `@app_employee_id` before create (stale/0 audit actors caused `audit_logs_ibfk_employee` FK failures).
- **repro_destructive_import.php** — employees import destructive-delete repro for company 1; browser + CLI dry-run default; browser <code>?run=1</code> dry-run / <code>?run=1&amp;apply=1</code> (Admin; <code>?apply=1</code> deep-link sets <code>run=1</code>); CLI <code>--apply</code> seeds disposable Keep/Delete Me employees via <code>itm_script_test_employee_create()</code>, imports only Keep Me through <code>modules/employees/index.php</code>, asserts Delete Me row survives (<code>deleted_at IS NULL</code>), tears down via <code>itm_script_test_employee_delete()</code>.
...
- **check_crud_view_edit_field_parity.php** — static view/edit form parity for dynamic scaffold modules (`$viewColumns` vs `$uiColumns` / `$*ListHidden` / edit-section helpers). Tag-free per-scope reference gap lines; exit `1` on parity gaps via JSON `failure_count`. Shared lib: **`itm_crud_view_edit_field_parity_audit.php`**. Catalog: **`scripts/scripts.php`**; workflow: **`SCRIPTS.md`** → *CRUD view/edit field parity*.
...
- **verify_user_config_profile.php** — `user-config.php` profile field regression: home-company UPDATE vs tenant switcher, birthday/theme/emergency round-trip, profile photo URL must be app-absolute Explorer proxy (not `../../modules/…`); cross-tenant `file.php` profile read denied via isolated probe; static cache-bust (`itm_get_ui_configuration(..., true)`) and `$ui_config` reassignment inside the `update_sidebar` and `update_dashboard_widgets` success paths (`strpos` on each action block, not a 1200-char regex).
- **verify_explorer_profile_photo_acl.php** — Explorer `file.php` profile ACL: same-tenant peer read allowed; cross-tenant without grant denied; non-profile Private owner-scoped.
- **verify_sidebar_preferences.php** — `employee_sidebar_preferences` save/load round-trip, Settings SideMenu access-gate static wiring, Personalized Sidebar `$ui_config` refresh contract.
...
- **list_active_and_checkboxes.php** — Audits `active` field UI for modules whose resolved `$crud_table` has an `active` column (`information_schema`). Skips bespoke modules (`docs/list_bespoke_UI.txt` + `scripts/data/crud_tables_skip_modules.txt` via `itm_crud_tables_load_skip_module_slugs()`) and orphan auto-scaffold stub directories (`itm_module_dir_is_standard_crud_scaffold()`). Flags forbidden text inputs, non-compliant scaffold checkboxes (`itm-checkbox-control`), and status-driven modules with visible row `active` on forms. Shared builder: `scripts/lib/itm_list_active_and_checkboxes_report.php`. CLI exits `1` on violations; `--all` lists compliant files.
...
- **check_company_id_ui_column.php** — static inventory of modules whose list/view may show **Company** (`company_id`); scans every `modules/{slug}/**/*.php` file (recursive). Browser report links each module slug to `../modules/{slug}/index.php` (relative, new tab). CLI `--list-exposed` / `--strict`; lib `scripts/lib/itm_company_id_ui_column_audit.php`. Catalog: **`scripts/scripts.php`**.
- **check_crud_has_company_from_field_columns.php** — static audit for flattened CRUD modules where `$hasCompany` is derived from UI-filtered `$fieldColumns` after `company_id` was removed for that module (breaks tenant `WHERE company_id=?` and Add sample data). Scans `modules/*/index.php` and `list_all.php`. CLI `--strict-warn`; lib `scripts/lib/itm_crud_has_company_field_columns_audit.php`. Catalog: **`scripts/scripts.php`**.
- **check_crud_boolean_cell_display.php** — static audit for visible `tinyint(1)` list/view columns missing badge/emoji handling in `cr_render_cell_value()` (`active` → badges; other checkbox columns → `itm_crud_render_checkbox_boolean_cell_value()`, bespoke ✅/❌, badge labels, `in_array(...)` badge blocks; duplicate `list_all.php` / `view.php` inherit from sibling `index.php`). Schema from `db/01_schema.sql`; lib `scripts/lib/itm_crud_boolean_cell_display_audit.php`. Catalog: **`scripts/scripts.php`**.
...
- **perform_audit.php** — Exploratory subprocess auditor for Tier 1–3 `scripts/*.php` (excludes Tier 4/5, `repro_*`, `verify_*`, `_tmp_*`, `health.php`, `test_ajax.php`, `test_edit.php`). Shared runner: `scripts/lib/itm_perform_audit.php`. Truncates `docs/error_log.txt` (`itm_error_log_file_path()`), isolates per-script log deltas, resolves Laragon `php.exe`, and writes `scripts/php_error_audit_results.json` with `exit_code` / `cli_errors` / `stdout_hits` / `summary`. Intentional exit-only failures: `scripts/data/perform_audit_allowlist.json`. Not a CI gate.
...
- **scaffold_departments_child_table_modules.php** — copies flattened CRUD from `modules/departments/` into the 24 child/support table allowlist; dry-run default (`?run=1` prints a compact summary — use `--verbose` for per-slug lists); `--apply` / `?run=1&apply=1`. Shared lib: `scripts/lib/itm_scaffold_departments_child_table.php`.
- **verify_scaffold_departments_child_table_modules.php** — three checks (module folders, schema coverage, sidebar catalog); exit `1` on failure.
- **list_modules_without_share.php** — Lists `modules_registry` rows not in `itm_qr_share_capable_module_slugs()` (Share Modules **No share UI**). Shared builder: `scripts/lib/itm_list_modules_without_share_report.php`. Browser links module **names** to `modules/{slug}/index.php` when the folder exists; CLI/JSON; optional `--active-only` / `?active_only=1`.
...
- **list_db_tables_sample_data.php** — Parses `db/01_schema.sql` and `db/02_data_sample.sql` (no MySQL). Lists all schema tables with module slug links (`target="_blank"`), `sample_data` yes/no/exempt/n/a, and `[INFO]` summary on CLI; filter `--sample=no` / `?sample=no`.
- **list_db_tables_sample_data_company.php** — Same schema/sample map plus live tenant row counts for one company; multi-company filter `--company=N` / `?company=N` (browser dropdown); tenant filter `--tenant=empty|populated`; requires MySQL.
...
- **titles_list.php** — Scans all PHP files under `modules/` and extracts `<title>` tags. Shared helpers: `scripts/lib/itm_titles_list_audit.php`. Prints a summary count (match vs not match) for the canonical title `<title><?= sanitize($crud_title) ?> - <?php echo sanitize($app_name ?? itm_ui_config_app_name($currentUiConfig)); ?></title>`; non-matching rows are prefixed `[NOT MATCH]`. Module sidebar emoji in the tab is applied at runtime via `itm_crud_apply_module_icon_to_browser_title()` before the tag (not part of the static audit string). Gate: `check_crud_browser_title_module_icon.php`; bulk inject: `apply_crud_browser_title_module_icon.php` (`scripts/lib/itm_crud_browser_title_apply.php`).
- **titles_list_show.php** — Same scan and summary as `titles_list.php`, listing rendered inner title text; non-matching rows are prefixed `[NOT MATCH]`. Rendered preview uses static `$crud_title` assignment from source — it does not simulate runtime icon prepend.
...
- **Test Runner EGPCS Support** — Added `-d variables_order=EGPCS` to `scripts/run_tests.php` to guarantee that the spawned PHPUnit process can correctly read `.env` configuration keys, resolving database connection failures.
- **PHPUnit Argument Forwarding** — Updated `scripts/run_tests.php` to transparently forward extra CLI arguments (such as `--filter <pattern>`) to the underlying PHPUnit process.
- **Auto-scaffolding probe cleanup** — `CompanyModuleAccessDiscoveryTest` tears down via `itm_sidebar_discovery_probe_cleanup()` (recursive removal of `modules/mbqa_phpunit_sidebar_probe/`). Do not use blanket `git clean -fd modules/`.
