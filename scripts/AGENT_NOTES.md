...
- **All `scripts/apply*.php` maintenance tools** — shared contract via **`lib/itm_apply_script_bootstrap.php`**: Browser + CLI, dry-run default, `--apply` / `?apply=1` (Admin) to write, named target lists after the count summary. Catalog: `scripts/scripts.php`. **`apply_module_sample_data_seed.php`** — propagates sample `INSERT`s into `database.sql` for every seeded `company_id` (single-row and multi-row `VALUES` blocks); browser errors via `itm_seed_fwrite_stderr()`; built-in help lists dry-run/apply URLs.
