...
- **Test sessions vs Admin**: `scripts/lib/itm_script_bootstrap.php` (loaded from `config.php`) clears disposable test-user cookies on normal web pages. In-process session tests must use **`itm_script_with_test_session_context()`** — not the signed-in Admin session.
