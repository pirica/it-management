...
- **schema_report.php** / **validate_DB_schema.php** / **test_employee_id-foreign_keys.php** / **validate_delete_employee.php** — Database schema validation suite. **`schema_report.php`** — Admin browser HTML report via `itm_schema_collect_validation_issues()`; uses scoped `.schema-report-wrap` (not global `.container` from `styles.css`).
