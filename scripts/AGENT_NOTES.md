...
- **migrate.php** — filename-ordered migration runner for `db/migrations/*.sql` (`includes/itm_database_migrations.php`). Live DB probe authoritative; browser `?run=1` renders an HTML results table (Status, file, Message, verify/fix links). `--apply` / `?run=1&apply=1` (Admin).
