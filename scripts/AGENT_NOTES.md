...
- **run_tests.php** — central test runner (`phpunit/phpunit.phar`, `phpunit/phpunit.xml`, `phpunit/tests/`); browser menu (standard vs HTML coverage); CLI `--verbose` and `--coverage`.
...
```
Browser: open `scripts/run_tests.php` and choose **Standard** or **HTML coverage**.
