...
- **verify_db_migrations.php** — live DB probes for each `db/migrations/*.sql` (Applied / Superseded / Not applied); lib `itm_verify_db_migrations_report.php`; `--json` / `?format=json`. Sidebar seed probe scopes to `Admin%` preference rows only (excludes `demo1`–`demo5` per-user layouts).
