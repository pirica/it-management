...
- **check_database_sql_company_name_uniques.php** — tenant UNIQUE audit over `database.sql` (via `includes/database_sql_unique_audit.php`). Skips intentional duplicate-name / junction tables including `bookmark_folders` and `floor_plan_item_tags`, and ephemeral `*_share_sessions` tables (see `includes/itm_qr_share.php`).
