...
- **verify_audit_columns.php** — schema gate for mandatory audit columns including soft-delete fields; ephemeral `live_chat_typing` is exempt from soft-delete meta (`company_id` only) so private chat presence is not forced into soft-delete / audit_logs patterns.
