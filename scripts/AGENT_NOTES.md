...
- **`scripts.php` catalog layout**: responsive card grid. Access badges: **Browser** + **CLI** (standard for `scripts/*.php` with a catalog link), or **CLI-only** (`scripts-badge-cli-only`) for bash/Python/bypass helpers. No separate dry-run badge — dry-run behaviour is documented in **How to use** (`--apply` / `?apply=1`). CSS: `scripts-badge-web`, `scripts-badge-cli`, `scripts-badge-cli-only`. **Table tags:** each card shows `scripts-badge-tag` pills (`Codebase`, `Python`, `Server`, `Info` for `scripts/data` or `scripts` `.json`/`.txt` refs, `Markdown` for `scripts` `.md` refs including bare `AGENT_NOTES.md`, table names, or `Mixed`) from static scan in `scripts/lib/itm_script_catalog_tags.php`. Data manifests set Info/Markdown only — not mined for `Mixed`. Five-column card grid: Script / Access / **Tags** / What / How (`td:nth-child(4)` and `nth-child(5)` for What/How — do not map `nth-child(3)` to What). Chip bar includes alias chips `*.json`, `*.txt`, `*.md` (filter Info / Markdown rows). Text search matches row copy and `data-tags` (`.json` / `.txt` / `*.json` / `*.txt` → Info; `.md` / `*.md` → Markdown). Screenshot regression: `python scripts/verify_scripts_catalog_filter_screenshot.py` → `qa-reports/scripts_catalog_filter/` (PNG + row-count assertions). Maintain with `apply_script_catalog_tags.php` / `check_script_catalog_tags.php`; regression `verify_scripts_catalog_filter.php`; manifest `scripts/data/script_catalog_tags.json`.
