...
- **apply_crud_audit_soft_delete.php** / **check_crud_audit_soft_delete.php** — scaffold soft-delete + audit meta UI rollout (`docs/list_soft-delete.txt`). Apply is **CLI-only**, dry-run default, `--apply` writes; skips status-driven modules; idempotent when already compliant. See **`scripts/SCRIPTS.md` → Scaffold audit columns + soft-delete**.
