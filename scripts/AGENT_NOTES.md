...
- **fields_missing.php** — all-module schema/UI audit via the same shared lib: schema drift per `$crud_table` module, dynamic scaffold UI pass, employees critical-field matrix, bespoke/status-driven schema-only. Prints `database.sql` / live / UI-audited / excluded column lists per module. Browser module filter; CLI `--module=` / `--json`. Catalog: **`scripts/scripts.php`**.
...
- **generate_tests.php** — Generates PHPUnit tests from metadata; browser dry-run (Admin), `?apply=1` writes; CLI writes immediately.
