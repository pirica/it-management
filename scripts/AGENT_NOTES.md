...
- **scaffold_departments_child_table_modules.php** — copies flattened CRUD from `modules/departments/` into the 24 child/support table allowlist; dry-run default; `--apply` / `?apply=1`. Run `sync_modules_registry.php` after apply; sidebar rows live in `itm_sidebar_base_structure()` (`includes/ui_config.php`).
