...
- **All `scripts/fix_*.php` repair tools** — same dry-run/apply bootstrap as **`apply*`** plus mandatory report sections via **`lib/itm_fix_script_report.php`**: dry-run status (`Dry-run: no broad SQL fixes needed.` / `Dry-run complete — nothing to change.` / `Dry-run: Still need fixes.`), then **`List Actual live DB`**, **`List db/*.sql data`**, **`List Fix`**. All six repair scripts use the contract; aggregate dry-run: **`fix_all.php`** (`--all` passed to `fix_scaffold_active_checkbox.php`). Full contract: **`SCRIPTS.md` → Fix scripts (`scripts/fix_*.php`)**.
...
- **fix_all.php** — dry-run aggregate for every `scripts/fix_*.php` (except itself); per-script PASS / needs-fixes summary. Contract: **`SCRIPTS.md` → Fix scripts (`scripts/fix_*.php`)**; shared lib **`lib/itm_fix_script_report.php`**.
