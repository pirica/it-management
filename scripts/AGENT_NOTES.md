...
- **check_crud_boolean_cell_display.php** — static audit for visible `tinyint(1)` list/view columns missing badge/emoji handling in `cr_render_cell_value()` (`active` → badges; other checkbox columns → `itm_crud_render_checkbox_boolean_cell_value()` or bespoke ✅/❌). Schema from `db/01_schema.sql`; lib `scripts/lib/itm_crud_boolean_cell_display_audit.php`. Catalog: **`scripts/scripts.php`**.
