...
- **verify_employees_equipment_search_coverage.php** — MySQL regression for employees + equipment index search: `first_name`, `last_name`, `username`, full name, FK codes (`FNB`, `LOC-NY-01`, `SUP-001`, `RACK-A`), position description, manager username, equipment assignee identity; disposable rows via `itm_script_test_employee.php`; optional `ITM_TEST_COMPANY_ID`.
