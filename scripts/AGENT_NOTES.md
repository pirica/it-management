...
- **`scripts.php` catalog layout**: responsive card grid (not a wide scrollable table). Each catalog row is a card with script link + access badges, then **What it does** and **How to use** stacked with section labels. CSS classes: `scripts-catalog-grid`, `scripts-catalog`. Client-side filter: `#scripts-catalog-filter`. Styles are inline in `scripts.php` (catalog-only).
