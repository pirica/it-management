...
- **All `scripts/fix_*.php` repair tools** — same dry-run/apply bootstrap as **`apply*`** plus mandatory report sections via **`lib/itm_fix_script_report.php`**: dry-run status (`Dry-run: no broad SQL fixes needed.` / `Dry-run complete — nothing to change.` / `Dry-run: Still need fixes.`), then **`List Actual live DB`**, **`List db/*.sql data`**, **`List Fix`**. Aggregate dry-run: **`fix_all.php`**. Full contract: **`SCRIPTS.md` → Fix scripts (`scripts/fix_*.php`)**.
