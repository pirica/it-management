...
- **fields_missing.php** — all-module schema/UI audit via the same shared lib: schema drift per `$crud_table` module, dynamic scaffold UI pass, employees critical-field matrix, bespoke/status-driven schema-only. Browser module filter; CLI `--module=` / `--json`. Catalog: **`scripts/scripts.php`**.
