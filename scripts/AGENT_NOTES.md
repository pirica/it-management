...
- **titles_list.php** — Scans all PHP files under `modules/` and extracts `<title>` tags. Shared helpers: `scripts/lib/itm_titles_list_audit.php`. Prints a summary count (match vs not match) for the canonical suffix `<?php echo sanitize($app_name ?? itm_ui_config_app_name($currentUiConfig)); ?>`; non-matching rows are prefixed `[NOT MATCH]`.
