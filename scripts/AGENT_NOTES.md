...
- **migrate.php** — filename-ordered migration runner for `db/migrations/*.sql` (`includes/itm_database_migrations.php`). **Applied/Pending always from live DB probes** (`scripts/lib/itm_verify_db_migrations_report.php`); `schema_migrations` is audit/history only (checksum drift), never sole proof of apply. `--apply` runs SQL only when the live probe fails; satisfied migrations are recorded without re-executing destructive files. Browser: `?run=1` / `?run=1&apply=1` (Admin).
