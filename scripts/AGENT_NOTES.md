...
- **migrate.php** — filename-ordered migration runner for `db/migrations/*.sql` (`includes/itm_database_migrations.php`). Live DB probe authoritative; browser `?run=1` renders an HTML results table (Status, file, Message, verify/fix links, **Open SQL** per row via `?run=1&sql={filename}` in a new tab). `--apply` / `?run=1&apply=1` (Admin).
