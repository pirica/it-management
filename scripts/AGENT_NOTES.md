...
- **Production Safety**: `debug.php` and other diagnostic tools should be removed or blocked in production. **`debug.php`** — browser requires **Administrator** session (`itm_script_require_admin_script_or_exit`); lists DB tables with total count in the section header; CLI defines `ITM_CLI_SCRIPT` before `config.php`.
