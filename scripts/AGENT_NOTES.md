...
- **verify_source_utf8_mojibake.php** / **apply_utf8_mojibake_fix.php** — UTF-8 / mojibake static audit and optional repair via `scripts/lib/itm_mojibake_audit.php`. Browser + CLI; `--path=` / `?path=` scope; apply dry-run default. Browser FAIL output links each `modules/<slug>/…` path to the module index via `itm_script_format_modules_file_link()`. Catalog: **`scripts/scripts.php`**.
- **explorer_human_test.php** — Human-flow Explorer API regression (ACL, CRUD, explorer table soft-delete sync, audit). Browser (Admin) via `itm_script_regression_entry.php`; disposable user via `itm_script_with_test_session_context()`; standard coloured `<pre>` log. Mutates DB/filesystem via temporary company teardown. See **`scripts/SCRIPTS.md` → Explorer**.
