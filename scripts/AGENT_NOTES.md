...
- **generate_FK_employee_id.php** / **generate_reassignment.php** / **delete_clone_employee.php** — Employee data maintenance tools.
- **transfer_data_from_employee.php** — Admin browser form + CLI. Default **dry-run** (single transaction rolled back). `--apply` / `?apply=1` creates a clone and copies all tenant tables with `employee_id` (probe per table on apply; full transaction rollback on dry-run).
