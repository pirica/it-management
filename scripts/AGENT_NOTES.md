...
- **repro_select_options_unauthorized_v2.php** — regression that <code>companies</code> is blocked from Select Options quick-add; CLI subprocess when available; browser fallback uses policy whitelist + no-row check and still prints <code>[PASS] companies quick-add blocked for regular users.</code> when confirmed (not a false “still permitted” on login redirect).
