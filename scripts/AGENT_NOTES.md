...
- **apitest_tier_free.php** / **apitest_tier_basic.php** — disposable `ui_configuration` tier rate-limit regressions; slot employees (`apitest-user-{id}`) seed via **`scripts/lib/itm_api_tier_test_helpers.php`** with audit `@app_employee_id` cleared before `employees` / `ui_configuration` writes; Free HTTP probe publishes CLI `PHPSESSID` via `itm_apitest_publish_http_session()`.
