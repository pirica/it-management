...
- **run_tests.php** — central test runner; browser **HTML coverage** spawns a detached CLI job (`scripts/lib/itm_run_tests_browser_coverage.php`, log `qa-reports/run_tests_browser_coverage.log`, status `?coverage_job=1`) to avoid Apache gateway timeouts; **Standard** mode streams PHPUnit in-page. Resolves CLI `php.exe` via `itm_resolve_phpunit_cli_binary()`. Dunebox setup: `scripts/setup_dunebox_php_from_laragon.ps1`. Full docs: **`scripts/SCRIPTS.md` → PHPUnit test runner**.
