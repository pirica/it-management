...
- **generate_FK_employee_id.php** / **generate_reassignment.php** — Employee data maintenance tools.
- **delete_clone_employee.php** — Admin browser form + CLI. Default **dry-run** (row counts only). `--apply` / `?apply=1` hard-deletes all `employee_id` tables (excludes `audit_logs`, `attempts`) then the `employees` row inside a transaction.
