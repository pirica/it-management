...
- **crud_actions.php** — scans `index.php` plus CRUD wrapper entry files for `$crud_action`. Skip: `is_*` equipment shortcuts + same bespoke list via `itm_crud_actions_should_skip_module()` in `scripts/lib/itm_crud_tables_audit.php`; other modules with no assignment remain **N/A**. Browser nav comes from `itm_script_output_begin()` — do not call `itm_script_browser_nav_echo()` again in custom HTML (duplicate ← Scripts index).
