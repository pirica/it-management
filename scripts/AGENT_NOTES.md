...
- **All `scripts/apply*.php` maintenance tools** — shared contract via **`lib/itm_apply_script_bootstrap.php`**: Browser + CLI, dry-run default, `--apply` / `?apply=1` (Admin) to write, named target lists after the count summary. Catalog: `scripts/scripts.php`. **`apply_module_sample_data_seed.php`** — propagates sample `INSERT`s into `database.sql` for every seeded `company_id` (single-row and multi-row `VALUES` blocks); **mirror mode** detects `INSERT … SELECT N, … FROM table WHERE company_id = 1` and appends only to the source tenant VALUES (e.g. `knowledge_base`); browser errors via `itm_seed_fwrite_stderr()`; built-in help lists dry-run/apply URLs.
...
- **employee_fields_missing.php** — compares `employees` columns in `database.sql` and live MySQL with create/edit/view/index coverage in `modules/employees/`; fails on schema or critical UI gaps (including `termination_date`). Uses `scripts/lib/itm_fields_missing_report.php`.
- **fields_missing.php** — all-module schema/UI audit via the same shared lib: schema drift per `$crud_table` module, dynamic scaffold + manual UI pass (`[PASS] audited UI column …` per business field on create/edit, view, and index; `[PASS] excluded UI column …` for hidden meta), employees critical-field matrix, bespoke/status-driven gated UI (`[SKIP][pass]` / `[SKIP][fail]` / `[SKIP][fail][reviewed]`). Prints `database.sql` / live / UI-audited / excluded column lists per module. Browser module filter + optional strict gate (`?strict_gate=1`); CLI `--module=` / `--json` / **`--strict-gate`** (exit `1` on unreviewed bespoke `[SKIP][fail]`). Reviewed exceptions: **`scripts/data/fields_missing_reviewed.json`** (manifest **`fields_missing_reviewed.php`**). Catalog: **`scripts/scripts.php`**.
- **fields_missing_reviewed.php** — read-only manifest viewer/validator for reviewed bespoke `[SKIP][fail]` gate lines consumed by `fields_missing.php`. Data file: `scripts/data/fields_missing_reviewed.json`. Browser or CLI `--json`; exit `1` on invalid registry shape.
