...
- **fast_create_acc.php** — Admin browser UI + CLI `--seed-demo-bundle` to seed/update demo employees via **`lib/itm_demo_module_users_seed.php`** (FK form fields including tenant **Role** select with ➕ quick-add, **Work Email** + **Personal Email**, multi-select **Modules** / `module_slugs[]`, RBAC per module, sidebar, `ui_configuration`).
