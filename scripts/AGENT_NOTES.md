...
- **check_env_vars_in_use.php** — static scan of `getenv()` / `$_ENV` / Python `os.environ` / shell `${VAR}` reads vs `.env.example`; shared lib `lib/itm_env_vars_audit.php`. Browser + CLI (Admin in browser); default informational; `--strict` exits `1` on example-only or undocumented app drift; `--json` machine output.
