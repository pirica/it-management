...
- **run_tests.php** — central test runner; browser menu lists required PHPUnit extensions (`dom`, `json`, `libxml`, `mbstring`, `tokenizer`, `xml`, `xmlwriter`) and Xdebug/PCOV for HTML coverage; resolves CLI `php.exe` via `itm_resolve_phpunit_cli_binary()` (not Apache `php-cgi`). Dunebox one-time setup: `scripts/setup_dunebox_php_from_laragon.ps1`. Full docs: **`scripts/SCRIPTS.md` → PHPUnit test runner**.
