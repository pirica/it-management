...
{"ok": true, "inserted": 2}</code></pre>
    ...
    <div class="card">
        ...
        <ul>
            <li><strong>2026-05-15:</strong> Root <code>README.md</code> now includes a <strong>Screenshots</strong> section with localhost-captured PNGs under <code>docs/readme/</code> (dashboard + Equipment module list).</li>
            <li><strong>2026-05-15:</strong> IDF <code>device.php</code> modals (<code>Edit Port</code>, <code>Create Cable Link</code>, quick-add rows) now use HTML entities for the plus (&#x2795;) and table sort caret glyphs so labels render correctly when the PHP source is not UTF-8–safe.</li>
            <li><strong>2026-05-15:</strong> IDF device <code>Edit Port</code> / <code>Create Cable Link</code> hydration now loads live <code>switch_ports</code> metadata by primary key (<code>modules/idfs/api/switch_port_row.php</code>) when <code>switch_ports.id</code> is known, infers rack equipment from <code>connected_to</code> hostname / device name when the position has no declared asset id, and fixes equipment port matching when RJ45 and SFP share the same port number (<code>modules/idfs/device.php</code>, <code>modules/idfs/idf_ports_sync.php</code>).</li>
