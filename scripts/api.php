 ...
 * IDF `idf_collect_port_slots_for_position` now synthesizes only fiber slots missing after counting live typed SFP rows in `switch_ports`, preventing duplicate RJ45-tail placeholders when `switch_ports` already defines SFP 1..N beside higher RJ45 numbers.
 * IDF rack `view.php`, `device.php`, and Ports metadata prefer `switch_ports` fiber numbering for linked equipment (`idf_switch_fiber_port_numbers_for_equipment`) with switch-port-merge filtering in `device.php` so hostname-backed SFP rows win over orphaned tail placeholders; port grids still prune/cap hydration to declared `sfp_count`.
 ...
 * Equipment create/edit now persists Workstation Office via equipment.workstation_office_id, exposes Network Details for all equipment types, adds equipment-level RJ45 Cable selection from rj45_speed.cable_type, and Switch Port Manager now loads/saves RJ45 cable selections into switch_ports/idf_ports sync.
...
{"ok": true, "inserted": 2}</code></pre>
    ...
    <div class="card">
        ...
        <ul>
            <li><strong>2026-05-17:</strong> Smoke step 5 (<code>check_ui_configuration_coverage.php</code>) now documents exemptions in <code>scripts/data/ui_configuration_excluded_modules.txt</code> and <code>scripts/data/ui_configuration_excluded_prefixes.txt</code>; all <code>is_*</code> equipment-type façades (for example <code>is_switch</code>, <code>is_server</code>) are skipped because they only <code>require modules/equipment/index.php</code> with <code>$equipmentAllowCreate = false</code> and have no local index-table chrome for this audit. CI runs the UI config check by default (no <code>SMOKE_SKIP_UI_CONFIG</code>).</li>
            <li><strong>2026-05-17:</strong> Added <code>scripts/check_audit_logs_coverage.php</code> (CLI + browser via <code>lib/script_cli_output.php</code>) to verify INSERT/UPDATE/DELETE traceability via <code>itm_run_query</code>, <code>itm_log_audit</code>, bulk helpers, or <code>trg_{table}_audit_*</code> triggers in <code>database.sql</code>; supports <code>--module=</code> and <code>--json</code>.</li>
            <li><strong>2026-05-17:</strong> <code>rack_planner</code> audit coverage: <code>database.sql</code> now defines <code>trg_rack_planner_audit_insert</code>, <code>trg_rack_planner_audit_update</code>, and <code>trg_rack_planner_audit_delete</code> after the <code>rack_planner</code> table block so fresh imports create the table before triggers.</li>
            <li><strong>2026-05-17:</strong> Floor Plans mutations now write <code>audit_logs</code> on confirmed DELETE/UPDATE for plans, folders, and tags (<code>modules/floor_plans/index.php</code>, <code>gallery_helpers.php</code>) using <code>itm_log_audit</code> / <code>itm_fetch_audit_record</code> when audit logging is enabled.</li>
            <li><strong>2026-05-17:</strong> <code>scripts/index.html</code> catalog lists static-analysis tools (CSRF, SQLi, index-table compliance, UI config, audit logs coverage) with Browser vs CLI badges; <code>scripts/idfs_api_payload_dry_run.php</code> is documented as CLI-only.</li>
            <li><strong>2026-05-17:</strong> <code>scripts/check_index_table_compliance.php</code> is smoke step 4 (import endpoint, actions markers, POST CSRF on list handlers); baseline: <code>scripts/data/index_table_compliance_baseline.txt</code>.</li>
            <li><strong>2026-05-17:</strong> Audit coverage scanner treats dynamic CRUD modules using <code>cr_escape_identifier($crud_table)</code> as mutation-capable; UTF-8 BOM removed from affected module includes; <code>wiki/Security.md</code> documents <code>ITM_APP_URL</code> and <code>IP2WHOIS_*</code> env vars.</li>
            <li><strong>2026-05-17:</strong> <code>wiki/Modules.md</code> documents non-CRUD entry points (Settings, Budget report, IDFs, Floor Plans) and which smoke audits they skip.</li>
            ...
            <li><strong>2026-05-17:</strong> Removed obsolete standalone repair script <code>scripts/fixes/2026-05-07_restore_sidebar_emojis.sql</code>; emoji values for new installs are defined in <code>database.sql</code> seeds, so ad-hoc updates are unnecessary in the repo snapshot.</li>
            <li><strong>2026-05-18:</strong> Fixed IDF <code>Create Cable Link</code> 500s when saving with a VLAN selection: <code>mysqli_stmt_bind_param</code> type-string for the mirrored <code>switch_ports</code> sync UPDATE had an extra specifier; also added <code>spt_any</code> join fallbacks on that sync query for <code>SFP</code> type matching parity with <code>device.php</code> (<code>modules/idfs/api/link_create.php</code>).</li>
