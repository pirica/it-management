...
echo "=== Todo Module Bug Verification ===\n";

function verify_multi_assign_visibility($conn) {
    echo "\n[1/2] Verifying multi-assign visibility fix...\n";
    $company_id = 1;
    ...
    if (!$stmt) die("ERROR: " . $conn->error . " in SQL: $sql\n");
    $stmt->bind_param("ii", $user_id, $user_id);
    $stmt->execute();
    $res = $stmt->get_result();

    if ($res->num_rows === 0) {
        echo "FAIL: Task assigned to multiple users is NOT visible to them (Bug confirmed).\n";
        $success = false;
    } else {
        echo "PASS: Multi-assigned task is visible.\n";
        $success = true;
    }
...
function verify_action_permission_enforcement($conn) {
    echo "\n[2/2] Verifying action permission enforcement fix...\n";
    $company_id = 1;
    $owner_id = 1;
    $attacker_id = 2;
    ...
    $fetchBlocked = !$data;
    if ($fetchBlocked) {
        echo "PASS: Attacker cannot fetch private task via ID.\n";
    } else {
        echo "FAIL: Attacker can fetch private task via ID (Bug confirmed).\n";
    }
    ...
    if ($updateBlocked) {
        echo "PASS: Attacker cannot modify private task via ID.\n";
    } else {
        echo "FAIL: Attacker can modify private task via ID (Bug confirmed).\n";
    }
...
$success1 = verify_multi_assign_visibility($conn);
$success2 = verify_action_permission_enforcement($conn);

echo "\nSummary: " . ($success1 && $success2 ? "ALL PASS" : "FAILURES DETECTED") . "\n";
