(function () {
    ...
    var selectAllBtn = document.getElementById('sms-select-all');
    var cancelSelectBtn = document.getElementById('sms-cancel-select');
    var unselectAllBtn = document.getElementById('sms-unselect-all');
    var enableSelectedBtn = document.getElementById('sms-enable-selected');
    var disableSelectedBtn = document.getElementById('sms-disable-selected');
    var filterInput = document.getElementById('sms-matrix-filter');
    var matrixTable = document.getElementById('sms-share-matrix');
    ...
    function shareToggles() {
        return Array.prototype.slice.call(document.querySelectorAll('.sms-share-toggle:not(:disabled)'));
    }

    function selectionCheckboxes() {
        return Array.prototype.slice.call(document.querySelectorAll('.sms-selection-checkbox'));
    }
    ...
    function updateSelectionUi() {
        ...
        document.querySelectorAll('.sms-selection-cell').forEach(function (cell) {
            cell.style.display = selectionMode ? '' : 'none';
        });
    }

    function ensureSelectionColumn() {
        if (document.querySelector('.sms-selection-header')) {
            return;
        }
        ...
        th.className = 'sms-selection-header';
        ...
        matrixTable.querySelectorAll('tbody tr').forEach(function (row) {
            ...
            td.className = 'sms-selection-cell';
            td.style.display = 'none';
            var moduleId = '';
            var toggle = row.querySelector('.sms-share-toggle');
            if (toggle) {
                moduleId = toggle.getAttribute('data-module-id') || '';
            }
            td.innerHTML = '<input type="checkbox" class="sms-selection-checkbox" data-module-id="' + moduleId + '">';
    ...
    function postToggle(companyId, moduleId, enabled, checkbox) {
        ...
        body.set('ajax_action', 'toggle_share');
        body.set('csrf_token', window.ITM_SMS_CSRF || '');
        body.set('company_id', String(companyId));
        body.set('module_id', String(moduleId));
        body.set('enabled', enabled ? '1' : '0');

        return fetch(window.ITM_SMS_ENDPOINT, {
            method: 'POST',
    ...
    function bulkToggle(enabled) {
        ...
        matrixTable.querySelectorAll('tbody tr').forEach(function (row) {
            ...
            var selection = row.querySelector('.sms-selection-checkbox');
            if (!selection || !selection.checked) {
                return;
            }
            row.querySelectorAll('.sms-share-toggle:not(:disabled)').forEach(function (toggle) {
                pairs.push({
        ...
        var body = new URLSearchParams();
        body.set('ajax_action', 'bulk_toggle_share');
        body.set('csrf_token', window.ITM_SMS_CSRF || '');
        body.set('enabled', enabled ? '1' : '0');
        body.set('pairs_json', JSON.stringify(pairs));

        fetch(window.ITM_SMS_ENDPOINT, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
            body: body.toString(),
            credentials: 'same-origin'
            ...
            pairs.forEach(function (pair) {
                var selector = '.sms-share-toggle[data-company-id="' + pair.company_id + '"][data-module-id="' + pair.module_id + '"]';
    ...
    shareToggles().forEach(function (toggle) {
        toggle.addEventListener('change', function () {
            ...
            postToggle(companyId, moduleId, desired, toggle).catch(function () {
                ...
                alert('Could not update module share setting.');
            }).finally(function () {
    ...
    if (filterInput) {
        filterInput.addEventListener('input', function () {
            ...
            matrixTable.querySelectorAll('tbody tr').forEach(function (row) {
                var haystack = (row.getAttribute('data-sms-search') || '').toLowerCase();
