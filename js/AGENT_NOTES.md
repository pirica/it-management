...
- **itm-short-url.js** — Short URLs module (`modules/short-url/`): paste-to-shorten, copy public URL, feature-card panels (custom code, password, expiry, QR). Uses document-level event delegation (no per-element `forEach` listeners). Loaded from `modules/short-url/includes/partials/render.php`.
