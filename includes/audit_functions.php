...
/**
 * Redacts sensitive fields from audit payload to prevent data exposure.
 */
function itm_audit_redact_sensitive_fields($values) {
    if (!is_array($values)) {
        return $values;
    }
    ...
    foreach ($values as $key => $value) {
        if (in_array(strtolower((string)$key), $sensitiveFields, true)) {
            ...
            $values[$key] = itm_audit_redact_sensitive_fields($value);
        }
    }
...
function itm_log_audit($conn, $table, $record_id, $action, $old_values = null, $new_values = null) {
    ...
    $old_values = itm_audit_redact_sensitive_fields($old_values);
    $new_values = itm_audit_redact_sensitive_fields($new_values);
