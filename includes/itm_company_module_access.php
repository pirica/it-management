...
if (!function_exists('has_module_access')) {
    function has_module_access($conn, $company_id, $module_slug, $forceRefresh = false)
    {
        ...
        $initialized_company = (int)itm_module_access_shared_static_cache('initialized_company');

        // Why: Pre-fetch all module access and icons for the company in one query to avoid N+1 sidebar performance issues.
        if ($initialized_company !== $company_id || $forceRefresh) {
            ...
            $sql = 'SELECT mr.id, mr.module_slug, mr.module_name, mr.icon AS registry_icon, mr.is_system_module, mr.active AS registry_active,
                           COALESCE(cma.enabled, 0) AS enabled, cma.icon AS company_icon
        ...
        if (!isset($registry_cache[$module_slug])) {
            ...
            itm_module_access_shared_static_cache('registry', $module_slug, $regData);

            $enabled = itm_module_access_company_enabled_for_slug($conn, $company_id, $module_slug);
            itm_module_access_shared_static_cache('access', $module_slug, $enabled);

            // Fetch company icon for this specific slug since it was not in the batch
            $companyIcon = '';
            $stmtIcon = mysqli_prepare($conn, 'SELECT icon FROM company_module_access WHERE company_id = ? AND module_id = ? LIMIT 1');
            if ($stmtIcon) {
                $moduleId = (int)($row['id'] ?? 0);
                mysqli_stmt_bind_param($stmtIcon, 'ii', $company_id, $moduleId);
                mysqli_stmt_execute($stmtIcon);
                $resIcon = mysqli_stmt_get_result($stmtIcon);
                $rowIcon = mysqli_fetch_assoc($resIcon);
...
if (!function_exists('itm_module_access_cached_icon')) {
    /**
     * Internal helper to access the static icon cache from has_module_access.
     */
    function itm_module_access_cached_icon($module_slug)
    {
        return itm_module_access_shared_static_cache('icon', $module_slug);
    }
}

if (!function_exists('itm_module_access_shared_static_cache')) {
    /**
     * Centralized per-request static storage for module access and icons.
     */
    function itm_module_access_shared_static_cache($type, $slug = null, $value = null)
    {
        static $cache = [
            'access' => [],
            'registry' => [],
            'icon' => [],
            'initialized_company' => 0
        ];
        ...
        if ($type === 'clear') {
            ...
            $cache['registry'] = [];
            $cache['icon'] = [];
            $cache['initialized_company'] = (int)$slug;
            return null;
        }
...
if (!function_exists('itm_set_module_access_cache')) {
    /**
     * Manual injection into the per-request sidebar cache (for tests).
     */
    function itm_set_module_access_cache($type, $slug, $value)
    {
        itm_module_access_shared_static_cache($type, $slug, $value);
    }
}
...
if (!function_exists('itm_resolve_module_sidebar_icon')) {
    function itm_resolve_module_sidebar_icon($conn, $company_id, $user_id, $module_slug)
    {
        ...
        // Why: Use pre-fetched icons from has_module_access() to avoid N+1 queries during sidebar rendering.
        if (function_exists('has_module_access')) {
            has_module_access($conn, $company_id, $module_slug);
        }
