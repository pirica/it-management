...
if (!function_exists('itm_explorer_is_allowed_extension')) {
     ...
     */
    function itm_explorer_is_allowed_extension($filename)
    {
        $ext = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
        $allowedExtensions = array('jpg', 'jpeg', 'png', 'gif', 'webp', 'pdf', 'txt', 'md', 'log', 'json', 'xml', 'csv', 'zip', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx');
...
if (!function_exists('explorer_extract_zip_safely')) {
    ...
    function explorer_extract_zip_safely(ZipArchive $zip, string $destinationDir): bool
    {
        ...
        for ($index = 0; $index < $zip->numFiles; $index++) {
            ...
            // Why: Enforce extension whitelist even during ZIP extraction to prevent RCE.
            if (!$isDirectory) {
                if (!itm_explorer_is_allowed_extension($relative)) {
