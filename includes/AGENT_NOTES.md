...
- **itm_login_attempt_identifier.php** — `itm_normalize_login_attempt_identifier()` redacts non-email/username values before `attempts.email` persistence.
...
- **itm_employee_employment_status.php** — login/dropdown eligibility via `employment_status_id` → `employee_statuses.name` = Active (`itm_employee_active_employment_status_join_sql()`, `itm_employee_active_employment_status_predicate_sql()`, `itm_employee_has_active_employment_status()`). Replaces deprecated `employees.active`.
