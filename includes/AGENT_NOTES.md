...
- **itm_script_entry_guard.php** — `itm_skip_http_entry_unless_direct()`, `itm_skip_view_partial_unless_context()`, PHPUnit processing detection. CLI subprocess contract tests set `ITM_HTTP_ENDPOINT_CONTRACT_TEST` so `includes/update_port.php` can run under `scripts/verify_update_port_zero_row.php`.
