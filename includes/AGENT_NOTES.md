...
- **itm_role_module_permissions.php** — `itm_resolve_active_company_id()`, `itm_user_has_role_module_permission()`, `itm_require_role_module_permission()` for server-side RBAC checks against `role_module_permissions`; `itm_mysqli_stmt_fetch_assoc()` falls back to `bind_result` when `mysqli_stmt_get_result` is unavailable and logs `mysqli_stmt_store_result` / `mysqli_stmt_result_metadata` failures via `error_log()`. Loaded from `config/config.php`.
...
- **itm_system_status_cache.php** — `system_status` table cache get/save/refresh (`itm_system_status_cache_get()`, `itm_system_status_refresh_all()`); collectors for Monitoring, PHP Settings, and Database tab payloads; `cache_get()` returns null when prepare/execute fails.
