...
- **itm_role_module_permissions.php** — `itm_resolve_active_company_id()`, `itm_user_has_role_module_permission()`, `itm_require_role_module_permission()` for server-side RBAC checks against `role_module_permissions`; `itm_mysqli_stmt_fetch_assoc()` and `itm_mysqli_stmt_fetch_all_assoc()` fall back to `bind_result` when `mysqli_stmt_get_result` is unavailable and log `mysqli_stmt_store_result` / `mysqli_stmt_result_metadata` failures via `itm_mysqli_log_stmt_fallback_failure()` (context label, optional `errno`, correlation id — no raw `mysqli_stmt_error()` text). Loaded from `config/config.php`.
