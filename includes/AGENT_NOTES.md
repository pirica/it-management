...
- **itm_script_entry_guard.php** — `itm_skip_http_entry_unless_direct()`, `itm_skip_view_partial_unless_context()`, PHPUnit processing detection. CLI subprocess contract tests define **`ITM_HTTP_ENDPOINT_CONTRACT_TEST`** (scripts only — never from HTTP input) so `includes/update_port.php` can run under `scripts/verify_update_port_zero_row.php`.
